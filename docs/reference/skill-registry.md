...
| **Pf-compose-articulator** | BRONZE (target) | Active in staging — installs a hand-off articulated chapter (e.g. a ChatGPT/Gemini rewrite) into a Sessions-lane book's Compose tab through the Composer's own save path; runs the pipeline's own `revoice_gates` before writing, Sessions-lane only, engine `scripts/podcast/compose_articulate.py` | `skills-staging/pf-compose-articulator/SKILL.md` |
| **Pf-compose-fix** | BRONZE (target) | Active in staging — checks and repairs one book's chapters on the Compose tab using the pipeline's own post-articulation rules, without a re-compose; detectors shared with the apparatus's `defect-scan` step, engine `scripts/podcast/compose_fix.py` | `skills-staging/pf-compose-fix/SKILL.md` |
