...
This creates the `<Bucket>/my-book-slug` branch off develop (bucket from content_profile, e.g. `Islamic/my-book-slug`), copies the source to `content/<Bucket>/my-book-slug/_system/source/`, and writes the initial `orchestrator-state.json`. Series-config.yaml is scaffolded — fill in `audience`, `source_tradition` (default `islam`; ismaili/shia/sunni/twelver/sufi alias to it), `host_dynamic`, `per_chapter_cost_cap_usd` (default 5.0 — raise per book if needed), `enable_slide_decks` (default true).
...
python3 scripts/podcast/orchestrate_book.py \
    content/*/my-book-slug/_system/source/raw.pdf
```
...
Read `content/<Bucket>/my-book-slug/_system/series-plan.md`. Confirm chapter list, length tier, episode_format per chapter (deep_dive / debate / walkthrough / monologue / interview / recap / narrative — see [_rules.py:EPISODE_FORMAT_ALLOWED](../../scripts/podcast/_rules.py)). Edit series-plan.md if needed.
...
jq '{phase, phase_status, completed_slugs: .phases."per-chapter".completed_slugs,
   ...
   content/*/my-book-slug/_system/orchestrator-state.json
```
...
G1–G5+G7 gates validate; on PASS, flips `status: published` IN PLACE at `content/<Bucket>/my-book-slug/` (status-flag model — nothing is copied; G6 wipe-safety is obsolete). The orchestrator then merges `<Bucket>/my-book-slug` → `develop` with --no-ff, pushes, and the book branch becomes deletable per the "ONE branch per active book" rule.
