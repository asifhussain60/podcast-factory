...
jq '{phase, phase_status, last_completed_phase, last_error}' \
    content/drafts/<my-book>/_system/orchestrator-state.json
```
...
`content/drafts/<book>/_system/orchestrator-state.json` is the
...
`content/drafts/<book>/**` for the book(s) listed as
