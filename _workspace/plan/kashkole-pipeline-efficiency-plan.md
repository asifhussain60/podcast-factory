...
| Phase 3 — Challenge | `tools/content_challenger/kashkole/challenge_auto.py` | `claude -p` (Max plan) | **~$0.43** |
...
The `anthropic` package is already installed in the kashkole venv (`anthropic>=0.104`).
...
The batch driver already exists at `_workspace/plan/_drivers/kashkole_adapt_all.py` —
...
| `tools/content_challenger/kashkole/challenge_auto.py` | Same — replace `claude -p` subprocess with direct API call |
| `_workspace/plan/_drivers/kashkole_adapt_all.py` | Add `--max-chapters` cap; optionally refactor to submit Batches API |
| `_workspace/plan/_drivers/kashkole_challenge_all.py` | Same cap + optional batch submission |
