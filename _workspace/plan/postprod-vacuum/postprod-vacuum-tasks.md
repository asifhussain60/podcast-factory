...
- **Next halt point:** T4 (folder-name decision), T5 (vacuum-plan review before --apply). T14 (`m4a/v1/` policy) was decided 2026-07-31 — delete — and executed.
...
| T11 | ✅ | ~~**BUG (re-scoped 2026-05-26)**: investigate SKIP/BLOCK decision-path~~ — **RESOLVED 2026-05-26.** Root cause was a timing race: commit `5fe5d58` (F30 scaffold-retirement, 2026-05-25 16:00) deleted the chapter spine scaffolds mid-pipeline while `per-chapter-slides` was running (13:58–16:46). Four chapters processed before the deletion (density 0.000 from unfilled `[LLM-FILL]` templates → justified-skip flow worked); two processed after (density returned 1.0 from missing spine → full convergence loop → spine pre-condition raised → BLOCKED). Final outcomes after this session's work: 5 SKIPPED (verified content-grounded justifications) + 1 SHIP-READY (ch.5 deck authored after Probe-7 found it warranted slides). | — (parallel) | Final state cited in T19. Bug fixes in `_slide_convergence.py` prevent the crash going forward (Fix 1: challenger retry; Fix 2: missing-spine clean BLOCKED). |
...
| T14 | ✅ | **DECIDED 2026-07-31: delete.** Asif chose deletion over archiving when the last surviving `m4a/v1/` came up in a repo cleanup — `content/Islamic/ayyuhal-walad/m4a/v1/` (4 files, the 5 June generation, 270 MB), superseded by the 13 July render that has since been delivered to Google Drive. Removed; no `v1/` directory remains anywhere under `content/`. The vacuum spec's existing "remove legacy mp3s" line is now backed by a decision rather than an assumption. Note the spec calls them mp3s and they were `.m4a` — a legacy generation is identified by the `v1/` directory, not by extension. | T4 | Was a halt point because the choice is irreversible, and it was: the v1 files existed nowhere else. Original context in [postprod-vacuum-review.md](postprod-vacuum-review.md) §3.3 + open question 2. |
...
| T19 | ✅ | **Slide-deck convergence — bug fixes + completion** — (a) Two `_slide_convergence.py` patches: challenger Probe-7 retry on transient failure + missing-spine clean BLOCKED return (no more silent crashes). (b) ch.2 re-verified via Probe-7 against existing substantive justified-skip → SHIP-READY → flipped BLOCKED to SKIPPED. (c) ch.5 Probe-7 found it warrants a full deck (NOT a skip) — surfaced 9 explicit structural candidates from the chapter prose. (d) ch.5 deck pair authored (19 slides × 5 diagram types) + verified SHIP-READY by Slide Deck Challenger on first pass. Final state: 5 SKIPPED + 1 SHIP-READY across all six chapters of M&D. Book satisfies corpus-level slide-deck mandate. | T11 | Rule clarification ([feedback_slide_decks_required.md](~/.claude/projects/-Users-asifhussain-PROJECTS-podcast-factory/memory/feedback_slide_decks_required.md)) updated in parallel: slide decks mandatory at book/lecture level, per-chapter skips OK when content explicitly resists illustration. M&D is now the first book proven through the clarified rule end-to-end. |
