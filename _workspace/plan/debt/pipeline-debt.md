...
| F38 | `_chunking.py:304` and `tighten_source.py:366` use `subprocess.run(["claude", "-p", ...])` in the live pipeline — violates the DR-015 "no `claude -p` in unattended pipeline code" rule and the standing `feedback_no_claude_p_token_burn` operator rule. `_chunking.py` is the core Phase 0b–0e window driver; `tighten_source.py` is the Phase I noise pre-pass. Both pre-date Wave J and are not regressions. Migration target: replace both `subprocess` callsites with direct `anthropic.Anthropic().messages.create()` SDK calls (same pattern already used in `rewrite_chapters.py`, `_review_ai.py`, `noise_router.py`). Prompt-caching should be applied to the repeated system prompt. DR-015 applies: these scripts fire >10 unattended calls per book. | 2026-06-02 (post-Wave-J audit — first formal tracking) | **High** | **CLOSED 2026-06-09** (Phase 4): verified — no `claude -p` shellout remains; `_chunking.py` uses `make_sdk_invoke_fn` (Anthropic SDK) and `tighten_source.py`/`_tighten_helpers.py` use `spawn_claude` (SDK). Prompt-caching applied to the stable instructions block in `make_sdk_invoke_fn` (semantically identical two-block user message). Pinned by `test_no_claude_p_in_unattended.py` grep gate. Max-vs-metered pool choice documented in framework.md. | — |
...
| F39 | Duplicated `_run`/`_err`/`_info` helpers across 13+ phase modules (no shared util). | 2026-06-09 (holistic-hardening Phase 4) | Medium | **CLOSED 2026-06-09** (Phase 4): extracted to `scripts/podcast/_subprocess.py` (single `run()`/`info()`/`err()`); 12 phase modules now alias the imports back to their private names — behaviour byte-identical, full suite green. | — |
| F41 | Live intake classifier not wired — Phase 6 Screen 2 (`SmartForm.tsx`) already ACCEPTS a classifier `proposed` map (pre-selects dropdown values + shows rationale), but no endpoint produces it, so the smart form ships with defaults pre-selected rather than source-derived suggestions (Q9 auto-fill). Fix: add `POST /api/intake/classify` that spawns the existing `podcast-blueprint` Layer-1 classifier (Haiku — cheap) on the staged primary source's text → returns `{content_profile, audience_profile, host_dynamic, episode_planning_mode, length_tier, video_style}` each with a one-line rationale, shaped to SmartForm's `Proposal` type. LLM-spend surface (small), so it's a deliberate increment, not an autonomous build. Wire: classify on primary-source staging, apply work-wide, per-volume override. | 2026-06-09 (Wave M6 — built SmartForm proposal-ready, classify endpoint deferred) | Medium | Open (increment — SmartForm already proposal-ready) | — |
| F40 | Two oversized functions remain: `chapter_driver._drive_per_chapter_and_after` (~250 lines) and `_chapter_design.author_phase_0d` (~771 lines). Plan Phase 4 proposed splitting each into named steps. **DEFERRED (not skipped):** these are pure-refactor (no behaviour fix) on the pipeline's hot path; `author_phase_0d` has no live characterization harness, so a blind split risks regressing already-shipped books — which is the hard zero-regression constraint. Do this WITH a live characterization run (record real I/O of one book through each function, assert byte-identical pre/post) rather than blind. The `_drive_*` split is lower-risk (covered by `test_run_supervision.py`) and can lead. | 2026-06-09 (holistic-hardening Phase 4 — risk-weighted deferral) | Low | Open (deferred — needs characterization harness) | — |
...
| ID | Gap | Severity | Note |
|---|---|---|---|
| F-MET1 | `compose_fix --vowel` spends Gemini and records nothing | **Python side FIXED 2026-08-11**; TS side open |  `_compose_fix_vowel.vowel_chapters` calls `vowel_book.vowel_text` directly and never calls `vowel_book.record_spend`, so neither `cost-ledger.jsonl` nor `model-provenance.jsonl` sees the pass. `record_spend`'s own docstring exists because this was already fixed once on the whole-book path. The Book Composer's Diacritics button reaches the same engine and should be checked with it. `stats` already carries `in_chars`/`out_chars`, so the fix is one call per chapter. A concrete instance of the class F-AUD3 described speculatively. **Python path fixed 2026-08-11**: spend and provenance recorded per chapter BEFORE the no-change check (a chapter the gate refuses outright still costs a call and carries the refusals worth reading), and `book-vowelling.json` written with a `scope` key naming the chapters covered. Pinned by `test_compose_fix_vowel_ledger.py`. **Still open:** the Book Composer's Diacritics button is a SECOND, TypeScript implementation (`plan-dashboard/src/pages/api/studio/vowelling.ts`) calling Gemini directly, and there is no cost-ledger writer on the TS side to call — the dashboard only ever reads that file. Needs a writer before it can be closed. |
