...
**Full context document:** `_workspace/KASHKOLE-COPILOT-BRIEF.md` — read §18 first, then §§1–12 for corpus detail.
...
Only ingest chapters whose `kashkole-challenger-report.md` verdict is PASS or WARN. Skip FAILs.
...
# Canonical ID:  doctrine:kashkole:<binder_id>:<chapter_id>:<chunk_index>
# Example:       doctrine:kashkole:24:650:0
...
### Step 3 — Build `kashkole_ingest_knowledge.py`

Create at: `_workspace/plan/_drivers/kashkole_ingest_knowledge.py`

This is a standalone batch driver — it does NOT go through `orchestrate_book.py`. Logic:

```
For each chapter directory under _workspace/kashkole-ksessions/extracted/kashkole/**/{chapter}/:
  ...
  LAYER 1 — Quran atoms:
    ...
    - Emit atom: type="quran", id="quran:S:A", source="kashkole/<binder>/<chapter>",
...
Run with:
```bash
$VENV _workspace/plan/_drivers/kashkole_ingest_knowledge.py
$VENV _workspace/plan/_drivers/kashkole_ingest_knowledge.py --binder 24   # single binder
$VENV _workspace/plan/_drivers/kashkole_ingest_knowledge.py --dry-run      # counts only
```
...
- Dedup: exact-match on canonical ID (`doctrine:kashkole:<b>:<c>:<i>`)
...
> **This is the canonical topic classification.** Every topic in the KASHKOLE database was manually tagged by the compiler with a `TopicTypeID` from the `Lookup_TopicTypes` table. This is more precise than binder-level inference — use it as the **primary** `topic_tags` source during ingestion. The full mapping is saved at `_workspace/kashkole-ksessions/topic-type-map.json`.
...
```python
# In kashkole_ingest_knowledge.py:
...
import json
with open("_workspace/kashkole-ksessions/topic-type-map.json") as f:
...
The JSON file at `_workspace/kashkole-ksessions/topic-type-map.json` contains both the taxonomy and all 223 per-topic assignments — **no database access needed** during ingestion.
  ...
  {NN}-{binder-slug}/
    {NN}-{chapter-slug}/
      ...
      _system/source/text/
        ...
        kashkole-challenger-report.md ← verdict filter (PASS/WARN = ingest, FAIL = skip)

_workspace/plan/
  kashkole-gate-report.md   ← lists the 18 FAIL chapters
  kashkole-podcast-scope.yaml

_workspace/KASHKOLE-COPILOT-BRIEF.md  ← full context document (read §18)
```
