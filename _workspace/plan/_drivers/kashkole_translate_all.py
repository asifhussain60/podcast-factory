#!/usr/bin/env python3
"""Batch translation driver for all KAHSKOLE chapters.

Runs translate for every chapter in binder order (smallest first).
Idempotent: skips chapters already at stage translated/adapted/challenged.
Enforces per-session cost cap. Commits per binder. Logs failures and continues.

Usage:
    python _workspace/plan/_drivers/kashkole_translate_all.py
    python _workspace/plan/_drivers/kashkole_translate_all.py --dry-run
    python _workspace/plan/_drivers/kashkole_translate_all.py --binder 35
"""
...
def _get_stage(binder_id: int, chapter_id: int) -> str:
    # Find the bundle.yml for this chapter
        ...
        for book_dir in shelf_dir.iterdir():
            ...
            for line in text.splitlines():
                ...
                if in_book and line.strip() == f"id: {chapter_id}":
                    has_chapter = True
                    break
                if in_book and line.startswith("  ") is False and line.strip() and not line.startswith("book:"):
                    in_book = False
...
def _translate_chapter(binder_id: int, chapter_id: int, dry_run: bool) -> float:
    """Run translate on one chapter. Returns cost_usd (0 if skipped/failed)."""
    cmd = [str(VENV), "-m", "tools.content_translator", "translate"]
    if dry_run:
        cmd.append("--dry-run")
    ...
    if result.returncode != 0:
        _log_failure(binder_id, chapter_id, f"translate failed: {output[-400:]}")
        return 0.0

    if "SKIPPED" in output:
        return 0.0

    # Parse cost from output line "  cost:     $X.XXXX"
...
def _log_failure(binder_id: int, chapter_id: int, reason: str) -> None:
    ...
    with FAILURE_LOG.open("a") as f:
        f.write(f"[{ts}] translate failed binder={binder_id} chapter={chapter_id}: {reason}\n")
...
def _commit_binder(binder_id: int, name: str, chapters: int, cost: float, dry_run: bool) -> None:
    if dry_run:
        ...
        return
    msg = (
        f"feat(kashkole-translate): binder {binder_id} — {name} "
...
def main() -> None:
    ...
    if args.binder:
        binders = [(b, n, c) for b, n, c in BINDER_ORDER if b == args.binder]
        if not binders:
            print(f"Binder {args.binder} not in order list")
            sys.exit(1)

    for binder_id, binder_name, _ in binders:
        ...
        for ch_id in chapters:
            ...
            print(f"  ch {ch_id}: translating...", end=" ", flush=True)
            t0 = time.time()
            cost = _translate_chapter(binder_id, ch_id, args.dry_run)
            elapsed = time.time() - t0

            if cost == 0.0 and not args.dry_run:
                # Check if it actually succeeded (zero-cost = skipped or failed)
                stage_after = _get_stage(binder_id, ch_id)
                if stage_after == "translated":
        ...
        print(f"\n  Binder {binder_id} done: {done} translated, {skipped} skipped, {failed} failed")
