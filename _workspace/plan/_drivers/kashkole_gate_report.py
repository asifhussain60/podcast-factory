...
def generate_report(out: Path | None = None) -> str:
    ...
    lines += [
        ...
        f"```",
        f"Then navigate to `http://localhost:4321/kashkole` to review any chapter.",
        f"",
        f"## Next Step",
        f"",
        f"After review:",
...
def main() -> None:
    ...
    ap.add_argument("--out", type=Path, default=REPO / "_workspace/plan/kashkole-gate-report.md")
    args = ap.parse_args()
