...
- The 3 snapshot JSONs in `plan-dashboard/src/data/` — regenerated via `cd plan-dashboard && npm run snapshot` (falls back to `python3 plan-dashboard/scripts/regenerate-snapshots.py` automatically; corrected 2026-07-18 — personal machine, no org policy, `npm install` works).
...
cd plan-dashboard && npm run snapshot                            # regenerate dashboard JSONs (Python fallback built in)
...
### 2026-06-04 — Claude session #9 (production-readiness sweep: pipeline + site)

**Purpose:** Full architectural / infrastructure / functionality / quality audit of both the podcast-factory Python pipeline and the Podcast Factory Astro Site, then fix findings holistically in risk-ordered waves to get the app production-ready.

**Method:** Ran four parallel auditors (html-view-challenger on the site; three Explore passes on infra, pipeline code, pipeline architecture), then RE-VERIFIED every load-bearing finding against the actual code before acting. This caught several sub-agent false positives that were explicitly excluded: the "dry-run mutates meta.yml" claim (the meta write is correctly behind the dry-run guard), the "DashboardTabs/AgentHoverBadge are dead" claim (status-badges.ts → PlanDesign.tsx is live), and a pydantic recommendation (the pipeline is hard stdlib-only).

**What changed (6 commits a723620 → 8773be7 on develop):**
- Wave 0 — resolved a committed merge conflict in `plan-dashboard/src/data/infrastructure-snapshot.json` that broke `astro build`; gitignored the stray `plan-dashboard/knowledge.db`. (Note: that snapshot is generated by the podcast-planner agent, NOT regenerate-snapshots.py, so the regenerator does not clear its conflicts.)
- Wave 1 — fixed the REAL `--dry-run` mutation bug: the G4 build-clean gate ran `build_episode_txt.py` (which writes `episodes/*.txt` + mints section depths) before the dry-run guard. Added a `--check` mode to the builder; G4 passes it under dry-run. Plus: 120s timeout on `orchestrate_book._run` (git) + 10s on the `security` keychain fetches; `is_phase_stale()` stale-running→failed auto-recovery in the resume dispatcher; PDF-exists + slug-format validation at launch; `write_state()` stops mutating the caller's dict.
- Wave 2 — UNIFIED the phase registry to one source of truth (`_progress.PHASES`). This fixed a LATENT P0: `"0literary"` (initial_driver) and `"publish"` (publish_driver) were emitted by drivers but absent from PHASES, so `update_phase()` raised ValueError when they ran — the full e2e suite was already RED on develop because of it. `orchestrate_book.CANONICAL_PHASES` now aliases PHASES; resume_dispatcher imports it; `_phases.py` (a never-adopted P5.4 StrEnum) became a thin re-export; `test_phases.py` rewritten as a real drift guard.
- Wave 3 — Cortex MUSTs on the site: numbered the two orphan architecture.astro sections (06/07); `scroll-margin-top` on `:where(section[id], .stack[id])`; moved the SVG `role`/`aria-labelledby` triple onto `<svg>` in OverviewSystemMap + InfraDeployment.
- Wave 4 — `render-mermaid.mjs` degrades gracefully when the Playwright chromium binary is absent (reuses cached SVGs, never hard-fails the build); corrected retired `_workspace/Books/` path drift across the orchestrator + challenger agent specs, SKILL.md, and orchestrate_book.py; documented the home-logo ornament as a REQ-027 decorative exception.
- Wave 5 — new `scripts/podcast/tests/test_publish_gates.py` (14 tests) covering G1-G3/G4/G6 + the dry-run no-mutation invariant; final repo regression sweep + html-view-challenger re-gate.

**Deliberately NOT changed (verified already-sound; avoided churn on working code):** state schema (`update_phase` already validates phase + status), episode-format contract (unwired formats already degrade to a P1 build warning via `EPISODE_FORMAT_FULLY_WIRED`), convergence constants (already named module-level), core bare-excepts (all re-raise or safe-default; rest carry `# noqa: BLE001`), file-splitting the >600-line files (high churn / low value).
