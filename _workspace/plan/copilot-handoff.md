...
- The 3 snapshot JSONs in `plan-dashboard/src/data/` — regenerated by Claude via `python3 plan-dashboard/scripts/regenerate-snapshots.py` (npm registry is blocked by org policy; Node-only `npm install` fails; `npm run snapshot` now falls back to Python automatically).
...
python3 plan-dashboard/scripts/regenerate-snapshots.py           # regenerate dashboard JSONs (npm install blocked by org)
