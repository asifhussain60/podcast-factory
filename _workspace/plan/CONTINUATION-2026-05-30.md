...
| ◐ | 4+ | **Split.** `extractor.py` claude→Gemini fix DONE (BLOCKER-C). Doctrine atoms already ingested (425 in knowledge.db). Quran verified (ch01: 7 refs). **Hadith/etymology/poetry atom ingest DEFERRED to pre-Slice-7** — genuine blocker: `knowledge-report.json` records `pending_ingestion:['hadith reference DB']`, no hadith corpus ingested, `wisdom_ingest` needs PyYAML (absent from system+venv), B5 extraction unbuilt. By dependency 4+ feeds Slice-7 OUTPUT (past the editor gate), so deferral is correct. augmented.md needs NO regen (Claude-authored layer, semantically intact, no meaning change from 2-fix). | source-library MCP | ~$0 | $0 |
...
| ◐ | 5b | **CORE shipped (ui-reviewer ship-ready, build clean).** New `/studio` page supersedes throwaway `/studio-poc`: 3-pane shell (editorial cockpit rail + editor/inspector). All 6 cards (Name Resolution, Key Focus, Tone & Register, Forbidden Terms, Required Elements, Audience Calibration) render + persist book-canonical + per-chapter override (`_system/editorial/book.json` + `<ch>.json`). Generic `kind`-based card model (best-judgment: north-star extensibility — adding a card/tradition = data entry). Cockpit follows editor via `studio:chapter-change` event. **DEFERRED enhancement layer:** dnd-kit reorder, cmdk corpus search, NLP name-extraction, weight/source, collapsible toggle. **← review checkpoint: run `npm run dev` → `/studio`.** | Copilot | ~$0 | $0 |
