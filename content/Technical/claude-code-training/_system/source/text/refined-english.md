...
This episode answers the question every Copilot user has when they first hear about Claude Code: "Where does this fit in what I already do?" Use the audience's Copilot knowledge as a foundation to build on — not as a foil to argue against. The goal is not to displace Copilot in the listener's workflow; it is to show them what Claude Code adds and where it fits alongside what they already use.
...
**Narrative arc for this episode:** Start from Copilot fluency as shared ground, build through the architectural difference — what the tools are actually designed to do — and land on the insight that reframes the comparison: these tools are not rivals. They are complementary, and sophisticated teams already use both.
...
- Runs in CI/CD (GitHub Actions, GitLab CI/CD)
...
1. **Inline suggestions (autocomplete):** "Autocomplete-style suggestions from Copilot in supported IDEs." Surrounding code is combined with contextual information from open tabs and sent to a language model. Suggestions appear as "ghost text" and are added only when the user explicitly accepts them.
...
3. **Copilot cloud agent:** Operates "autonomously in a GitHub Actions-powered environment." Hard limit: 59 minutes per session. Works only on "repositories hosted on GitHub."

---

## PART 2 — THE MENTAL MODEL SHIFT (TEACH THIS BEFORE THE COMPARISON TABLES)

This section comes before the feature tables. Without this frame, the tables read as a spec sheet. With it, every row makes sense.
...
**Key limitation:** Terminal commands are *suggested* to the user for approval, not executed fully autonomously. The cloud agent has a hard 59-minute cap per session.

### Claude Code's Agentic Model

- **Sub-agents:** Claude spawns parallel agents (Task tool) to work on different parts simultaneously
...
When subscription limits are exhausted, users can continue at standard API token rates. This requires explicit opt-in to billing.
...
This is not a footnote — it is the clearest expression of what these tools actually are relative to each other. They share underlying infrastructure. The question was never "which one" — it was always "which job."

**Claude Code has no inline autocomplete.** This is a deliberate architectural choice, not a gap. Developers who need real-time inline completion while writing, plus agentic task execution for larger work, use both tools. That is the mainstream pattern at engineering teams that have adopted Claude Code.
...
**In practice:** A developer using both keeps Copilot active in VS Code for the writing experience, and opens Claude Code in a terminal (or the VS Code panel) when they have a well-scoped task to hand off. The two sessions are independent — they do not interfere with each other.
...
- Sub-agents (Task tool): Copilot has specialized agents but no user-defined parallel sub-agent spawning within a task. This is advanced capability; do not oversell it for day-one use.
...
CLAUDE.md is a plain markdown file at your project root — either `./CLAUDE.md` or `./.claude/CLAUDE.md`. Claude Code **reads it at the start of every session** and loads it into context before your first prompt.
...
Claude Code also has auto-memory: Claude writes notes to `~/.claude/projects/<project>/memory/MEMORY.md` on its own, saving things like build commands, debugging insights, and preferences it has discovered. The first 200 lines (or 25KB) load into every session. The file is editable and deletable at any time.

**Important architecture note:** CLAUDE.md content is "delivered as a user message after the system prompt, not as part of the system prompt itself." It shapes behavior but is not a hard enforcement layer. For enforcement, use Hooks — covered in Part 5.
...
From the official commands reference (`code.claude.com/docs/en/commands`):
...
Custom slash commands have been merged into the Skills system. A `SKILL.md` file in `.claude/skills/<name>/` creates a `/<name>` command. **Key design choice:** CLAUDE.md is always-on context — it loads every session. Skills are on-demand context — they load only when invoked. This keeps the context window efficient.
...
MCP servers can be scoped three ways:
...
Rules are evaluated in order: **deny → ask → allow.** First match wins. Deny always beats allow.
...
Claude Code is **token-billed** for API usage. Every agentic loop iteration — every tool call and its result — passes through the context window. A single "fix the auth tests" request might involve 10–20 tool calls, each contributing to the context.

**The critical framing for Copilot users:** Copilot charges a flat monthly subscription per seat for unlimited completions. Claude Code's agentic loops consume more tokens per task, but they also accomplish tasks that would otherwise require many back-and-forth completion cycles. The comparison is task completion vs. text prediction, not tokens-for-tokens.
...
**Teacher (male voice):** In practical mode. This episode is a walkthrough. He has guided many Copilot users through the first session and knows exactly where the friction points are. He anticipates them before the student hits them — and when she does hit one, he validates it rather than dismissing it.

**Student (female voice):** She is ready to try it. She has the mental model from Episodes 1 and 2 and wants to go from understanding to doing. Her questions this episode are practical: "what do I do first?", "what's going to feel strange coming from Copilot?", "what should I set up before I start on a real task?" Her energy is forward-moving — she is ready, not impatient.
...
3. If the browser does not open, press `c` to copy the login URL
...
The welcome screen shows session information, recent conversations, and the latest updates.
...
```
Claude Code will: (1) find the appropriate file, (2) show proposed changes as a diff, (3) ask for approval, and (4) make the edit.
...
In plan mode, Claude presents its plan and asks how to proceed. The options are:
...
**Auto memory:** Claude Code v2.1.59+ writes its own notes to `~/.claude/projects/<project>/memory/MEMORY.md`. The first 200 lines or 25KB load every session. Manage it via `/memory`.
...
**2. Human-in-the-loop defaults.** Claude Code defaults to requesting permission before each file change. Copilot users are accustomed to accepting inline suggestions instantly. Use `acceptEdits` mode (`Shift+Tab`) to reduce friction, then review `git diff` afterward.
...
**4. GitHub workflows use standard git and gh CLI, not native Copilot integrations.** Copilot is deeply embedded in GitHub's web UI — PR summaries, reviewer suggestions, and commit messages all happen inside GitHub. Claude Code does the same things through standard git commands and the `gh` CLI. If you already know git and gh, the translation is direct: "summarize these changes" becomes a prompt, and "create a PR" becomes a `gh pr create` call that Claude Code can make on your behalf. The experience is terminal-first rather than web-native, but the underlying capability is equivalent for most workflows.

**5. Cost model difference.** Copilot is a flat subscription per seat. Claude Code on Pro/Max includes usage within plan limits, and if you hit those limits, the session pauses. Claude Code via the Console API is pay-per-token with no soft rate limit.

**6. Context window management is your job.** Copilot manages context automatically with a small sliding window. With Claude Code, as the context window fills, performance degrades. Use `/clear` between unrelated tasks. Use plan mode to scope exploration. Use sub-agents for large investigations.
...
**8. Rate limits on subscriptions.** Heavy agentic use on Pro/Max can hit rate limits, at which point the session pauses until limits reset. The Console API has no soft rate limit.

**9. Model is always Claude.** Unlike Copilot, which lets you switch to GPT-4o, Gemini, and others, Claude Code only uses Claude models (Sonnet, Haiku, Opus).
...
**Custom slash commands:** Put `.md` files in `.claude/commands/` and they become slash commands available via `/commandname`. Commit them to git for team sharing. Use `$ARGUMENTS` for parameters.
...
2. **Authenticate:** run `claude` and complete browser login with your Claude.ai subscription account
...
5. **Initialize memory:** `/init`, then review and refine the generated CLAUDE.md
6. **Explore first:** `give me an overview of this codebase`
7. **Make a small change in plan mode:** press `Shift+Tab` twice to enter plan mode, describe a change, and review the plan before Claude touches any files
8. **Set acceptEdits for iterative work:** press `Shift+Tab` once, then run `git diff` to review afterward
