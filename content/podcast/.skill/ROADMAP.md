...
| A14 | **Orchestrator + trainer specs reconciled onto the `_learning/` substrate (v2 plan, Phases A + C + D).** Trainer agent spec rewritten end-to-end: inputs = `_learning/patterns.md` + `_learning/proposals/`; regression = `test_challenger.py` (no separate `regression-suite/`); ledger = `findings.jsonl` (no separate `trainer-ledger.md`); promotion = move proposal to `_learning/promoted/` with decision log; archive = move to `_learning/archive/` with reason. Orchestrator spec narrowed convergence cap 5×5→3×5 (15 max passes/chapter, fits the $50 cap); dropped `PROGRESS.md` / `orchestrator-log.md` / `needs-human-review.md` references (state.json + challenger-report.md cover everything); explicit `_learning/` references and never-touch list. Phase 0f gate aligned to chapter-list + tier only. New `scripts/podcast/sync-agent-wrappers.sh` (sync + `--check` for CI) keeps `infra/claude-agents/*.md` byte-identical to `.github/agents/*.agent.md` for the 6 active wrappers. SKILL.md Phase 4 gains a "two operating modes" preamble. New `scripts/podcast/_progress.py` (atomic state.json writer) and `scripts/podcast/orchestrate_book.py` Phase A driver (pre-flight + branch + scaffold + Azure ingest + 0f halt; 0b–0e LLM authoring via `_authoring.py` stubs). | `.github/agents/podcast-{orchestrator,trainer}.agent.md`, `infra/claude-agents/podcast-{orchestrator,trainer}.md`, `scripts/podcast/{sync-agent-wrappers.sh,_progress.py,orchestrate_book.py,_authoring.py}`, `skills-staging/podcast/SKILL.md` Phase 4 preamble | 2026-05-18 |
