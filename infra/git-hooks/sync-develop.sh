...
# ref moves, so the Podcast Factory Astro Site and the Podcast Factory Library dev server
...
#   2. the Podcast Factory Library's local D1 schema (a migration, not a file)
#
...
# --- Podcast Factory Library local schema ----------------------------------------------------
...
if [ -f "$LISTENER_DIR/package.json" ] && command -v npx >/dev/null 2>&1; then
  if OUTPUT="$(cd "$LISTENER_DIR" && npx wrangler d1 migrations apply podcast-listener --local 2>&1)"; then
    if grep -qi "no migrations to apply" <<<"$OUTPUT"; then
      ...
      echo "sync-develop: Podcast Factory Library local D1 schema updated"
    fi
  else
