...
| I5 | **No authorial-apparatus noise** (R-NOISE-AUTHORIAL-APPARATUS) — the chapter/framing carries no circulation/copyright notice, ijazat-to-record + treasury-deposit provenance, or colophon (meta about the book-OBJECT rather than its teaching). Distinct from I4 (biographical): this is about how the book was recorded/authorized/circulated. PROTECT: wilayah/allegiance doctrine + inherited-from-prophets epistemics are teaching, never apparatus. | Apply `R_NOISE_APPARATUS_PATTERNS` from `_rules.py` to locate candidates, then judge in context. | Flag (P1) — authoring decision; never auto-fix. Cross-surface sweeps belong to the [`noise-auditor`](noise-auditor.md) agent (NZ-* findings). |
