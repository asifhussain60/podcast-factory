...
challenger_contract:
 ...
 - docs/reference/cortex-challenger-framework.md # parent framework: severity, verdicts, convergence
...
7. [docs/reference/cortex-challenger-framework.md](../../docs/reference/cortex-challenger-framework.md) — the parent framework (severity taxonomy, verdict-state contract, sidecar template).
...
The earlier authority list named 17 handbook + Arabic-reference files under `content/podcast/.skill/handbook/`, `content/_shared/arabic/`, and `_learning/`. As of 2026-05-24 those files are missing from disk — likely a side-effect of the 2026-05-23 restructure. When they are restored (or rebuilt from `_rules.py` + this agent's prose), they re-enter the normative list above. Until then, treat any reference in this agent's body to a `content/podcast/.skill/handbook/*` path as advisory documentation, not a hard authority.
...
|---|---|---|---|
| A1 | **Citation discipline** — every Quranic verse cites the plain-English form `(chapter N, verse M)` — canonical since 2026-06-10 (R-QURAN-CITATION-FORMAT; supersedes the prior `(Quran <Surah>:<Verse>)` form, whose terse variants `(Q N:M)` / `(Quran N:M)` / bare `(N:M)` are now P1-flagged by `_validators.assert_quran_citation_format` because TTS reads them as opaque number runs). A Quranic quotation with NO reference at all is this check's semantic half: flag it unless the source text itself leaves the verse unnamed (references are never invented — see A2). Every hadith cites collection + book + number + narrator. Wisdom/saying blockquotes attributed to the Father of Imams should name the speaker in chapter prose, but SHOULD NOT carry bibliographic tails like `in Nahj al-Balagha (compiled by al-Sharif al-Radi), Hikam (Saying) 147`; those are `NZ-REFERENCE-TAIL` chapter noise and belong in logs/show notes if needed. Every Ismaili source names work + author/Imam + (for Farmans) date + location. Per `enrichment-sources.md` §2 as constrained by Wave-N. | Scan blockquotes + inline italics; verify scripture/hadith/source claims carry needed source clarity, and verify wisdom/saying blockquotes keep speaker attribution without reference-tail clutter. | Flag (P0 for missing scripture/hadith/source references; P1 for wisdom/saying reference-tail clutter). |
...
| I5 | **No authorial-apparatus noise** (R-NOISE-AUTHORIAL-APPARATUS) — the chapter/framing carries no circulation/copyright notice, ijazat-to-record + treasury-deposit provenance, colophon, or wisdom/saying reference tail (meta about the book-OBJECT or source edition rather than its teaching). Reference-tail example: `in Nahj al-Balagha (compiled by al-Sharif al-Radi), Hikam (Saying) 147`. Distinct from I4 (biographical): this is about how the book/source was recorded, authorized, circulated, compiled, or cited. PROTECT: the quoted teaching itself, the speaker, wilayah/allegiance doctrine, and inherited-from-prophets epistemics are teaching, never apparatus. | Apply `R_NOISE_APPARATUS_PATTERNS` from `_rules.py` to locate candidates, then judge in context. | Flag (P1) — authoring decision; deterministic reference-tail stripping is available via `strip_reference_attribution_noise.py`. Cross-surface sweeps belong to the [`noise-auditor`](noise-auditor.md) agent (NZ-* findings). |
...
Loop N enforces the architectural pivot from inline phonetic guides (which NotebookLM reads aloud as content, producing doublings like "Sahih Sitta, sahasita" and mangled names like "tassel wolf" for *Tasawwuf*) to customize-prompt imperative directives. As of 2026-06-24, Islamic scholarly chapters must still persist Arabic script beside glossary-backed terms; Arabic script is required content, while phonetic respelling remains forbidden in prose.
...
| N2 | **Framing's `## Pronunciation` block uses the bullet form** — `- Term: form`, opened by the anti-doubling instruction. The legacy passive-list pattern (`*term*: phonetic`) is forbidden, and so is `Pronounce "X" as "Y"`. | Regex scan (`LEGACY_PASSIVE_PRONUNCIATION` + `PRONOUNCE_AS_DOUBLE_RE` in `_validator_constants.py`). | Auto-fix (deterministic conversion to `- Term: form`). **Corrected 2026-08-01:** this row named `Pronounce "X" as "Y"` as the TARGET form until today, contradicting R-PRONUNCIATION-DOUBLE in the same catalog — that format makes the TTS read the term twice (root-caused in Ayyuhal Walad, 2026-06-05). |
| N3 | **Every transliterated Arabic term in the chapter has an entry in the framing** — gap detection: term in chapter without a corresponding bullet. | Set diff: chapter Arabic terms vs the compiled block's entries. | Structural, not a fix to apply by hand: `_pronunciation_block.py` compiles the block from the terms present in THIS chapter, so a gap means the ladder has nothing settled to say. It is reported by the build as a NOTE and settled by ear with `run_pronunciation_probe.py`, which writes the answer to the cross-book ledger. Flag (P1). (Superseded 2026-08-01: this row used to say "auto-fix when the term is in the shared manifest or `_phonetics.md`" — the manifest was retired in the 2026-05-23 restructure and `_phonetics.md` is legacy.) |
...
| N6 | **Islamic chapter contains Arabic script** — every Islamic scholarly chapter source must have visible Arabic script from glossary-backed terms, while pronunciation remains in the glossary/framing. | Regex scan for Arabic Unicode in each chapter plus glossary presence. | Flag (P0) and run deterministic chapter Arabic injection from the curated glossary; do not add phonetic respelling to prose. |
| N7 | **No English translation in the value slot** (R-PRONUNCIATION-RENDER, 2026-08-01) — `- arkan: the pillars` is a translation sitting where the block's own instruction promises a phonetic, so the hosts try to pronounce it. Legitimate only when the ladder itself chose that English (a ledger gloss, an exonym, an explicit `substitute` row). | `assert_framing_pronunciation_render` in `_validators_framing.py`: a determiner-led value that disagrees with what `_pronunciation_block.compile_entries` resolves for that term. | Hard fail. Fix at the source — add a row to `BOOK_DIR/_system/pronunciation.md`, or settle the term by ear with `run_pronunciation_probe.py`. Never by editing the framing: the build recompiles every value. Empirical: five of six episodes of `degrees-of-excellence` shipped this way, and *arkan* was spoken as "Archon", *mafdul* as "Mathdul", *masbuq* as "Mazbuck". |
...
| CS7 | **Source coverage — nothing silently dropped.** The union of Phase 0d `source_chapters[]` line ranges (from `_system/source/text/_chunks/0d/source-toc.json`) covers the refined source, front/back-matter tolerance aside. A gap means source content was dropped from the split. Vacuous pass on legacy books without a source-toc. | `check_chapter_set.py` script id P7 — deterministic line-range union vs `refined-english.md` length. | Flag (P1). Authoring decision — re-draw the 0d plan to cover the gap, or record an explicit `essential: skip` for true side-matter. |
| CS8 | **No duplication across chapters.** Two deterministic halves: (a) Phase 0d source line ranges must not overlap (same source lines feeding two episodes) — P0; (b) no chapter pair shares ≥3 distinct 12-word passages of concept prose (frames + liturgical formulae excluded) — P1. SEMANTIC half (same teaching re-taught in different words) is the challenger's own judgment at book scope: read the chapter set's concept H2 titles + opening paragraphs and flag re-teaching. | `check_chapter_set.py` script id P8 — range intersection + n-gram shingle scan; challenger judgment for paraphrase-duplication. | Flag (P0 overlap / P1 n-gram). Authoring decision — cut the duplicate from one chapter; never auto-strip. |
| CS9 | **Sermon integrity — preserved whole, delivered once.** Every contract declaring `sermon: {present: true, section_title}` must have that H2 section in EXACTLY ONE chapter, carrying the sermon whole (≥150 words; a stub means it was fragmented). Pairs with R-SERMON-VERBATIM: the matching framing must carry the `## Verbatim Recitation` block (enforced by `_authoring/_framing.py`'s post-author gate). | `check_chapter_set.py` script id P9 — contract scan + H2 presence/word-count check. | Flag (P0 missing/duplicated; P1 stub). Authoring decision. |
| CS10 | **Set-level density.** Per-chapter concept-section count within the chapter-density target (≤3 concepts, frame headings excluded; `docs/standards/chapter-density.md`). Advisory HERE — the $0 preflight smoke gate (`phases/preflight_chapter.py` check #4) owns the halting for `density_standard: 2` books; CS10 keeps the set view in the report. | `check_chapter_set.py` script id P10 — `chapter_density_audit.audit_chapter` per chapter. | Flag (P1). Re-split via Phase 0d. |
| CS11 | **Conceptual integrity + episode-to-episode flow** (SEMANTIC — challenger judgment, no script). Read the chapter set in order: (a) no concept's argument straddles an episode boundary (a chapter must not end mid-development of a teaching the next chapter resumes); (b) each episode's landing hands off naturally to the next episode's opening (the set reads as a designed sequence, not shuffled fragments); (c) each chapter's enrichment binds to THAT chapter's named tensions, not generic background. | Challenger reads chapter H2 maps + opening/landing frames across the set. | Flag (P1). Authoring decision — re-draw the boundary at the concept seam. |
...
**Density-standard promotion (2026-06-10):** for books with `density_standard: 2` in `_system/series-config.yaml`, P0-class CS findings HALT the pipeline at the post-0d check (`phases/preflight.py::_run_chapter_set_check` raises) instead of advisory-logging. Legacy books keep advisory behavior.
...
| T4 | **Farman date/location plausibility.** STUB — data file `content/_shared/islam/farmans.yml` was reserved and never created (de-linked 2026-07-26 — the path does not exist). Once populated, cross-references any Farman attribution against issuing-Imam's lifetime + accession dates. | Pending data. | Pending. |
...
**Six matched fixtures** live at [_learning/fixtures/](_learning/fixtures/): `ai_cliche/`, `faux_profundity/`, `premature_closure/`, `deep_dive_self_reference/`, `essentialism_external/`, `essentialism_internal_qualified/` (negative — must NOT trip stem patterns). Each has `input.txt` + `expected.json`.
...
- **File:** content/<Bucket>/<slug>/chapters/ch##-<slug>.txt:LINE
...
After writing the sidecar report, the agent MUST emit one JSONL record per **distinct finding** (auto-fixes count as `resolution: "auto-fixed"`; remaining items as `resolution: "flagged"`) into `_learning/findings.jsonl`. The emission uses `scripts/podcast/_rules.py::emit_finding()` invoked through a small Python one-liner via Bash. Example for a single finding:
...
- Resolves chapter refs within `content/<Bucket>/<book-slug>/chapters/` (draft vs published is a status field, not a separate folder) (memoir paths blocked via `PROHIBITED_PATH_PREFIXES`).
...
1. Confirm the book-slug. If missing, ask: "Which book? (give the `<book-slug>` directory name from `content/<Bucket>/`)".
2. Confirm scope. If per-chapter, confirm the chapter slug exists under `content/<Bucket>/<book-slug>/chapters/`.
...
- Do not run the agent on content outside `content/<Bucket>/<book-slug>/` (per-book state; draft vs published is a status field). Memoir is out of scope; the boundary is hard.
