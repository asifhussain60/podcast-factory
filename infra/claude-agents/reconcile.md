...
| An agent's behaviour | `infra/claude-agents/<agent>.md` plus the skill file it points at |
...
4. **Agent files** (`infra/claude-agents/*.md`) — when an agent's check catalog needs updating to enforce the new rule.
...
- Cross the cross-skill boundaries set out in [CLAUDE.md](../../CLAUDE.md).
...
- **2026-05-27** — Migrated to `infra/claude-agents/` (DR-014). Agent-file references updated from `.github/agents/*.agent.md` to `infra/claude-agents/*.md`.
