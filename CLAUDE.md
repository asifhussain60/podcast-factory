...
- **Plan-first execution gate (LOCKED 2026-05-27).** No pipeline work, code change, or new feature executes without: (1) a corresponding entry added to plan.yaml + plan.md, (2) plan-dashboard views regenerated via `cd plan-dashboard && npm run snapshot`, and (3) Asif's explicit approval. This applies to ALL tiers — even Tier 0 code changes require a plan entry and view update before execution. Per user direction 2026-05-27.
...
- **Two-agent operating model — Claude Code + GitHub Copilot in parallel (LOCKED 2026-05-30).** This repo is worked by two agents at once; the split is **by directory** (directory ≈ skill), validated by a regression audit on 2026-05-30. **Claude owns:** `scripts/podcast/**` (the Python pipeline), all prompts, `_rules.py`/`_doctrinal.py`, the `infra/claude-agents/*.md` specs, `docs/standards/*.md`, the plan (`_workspace/plan/refactor/plan.{yaml,md}`), the three `plan-dashboard/src/data/*-snapshot.json` files, and `CLAUDE.md` — i.e. the pipeline + the architecture/coordination surface. **Copilot owns:** `plan-dashboard/**` (the Astro site — React/TSX, TipTap, CSS, the cockpit + intake UI). **Three load-bearing rules:** (1) **Claude is sole owner of the plan + snapshots** — only Claude runs `npm run snapshot`; Copilot never edits plan files (the snapshot-regen hook does not fire for Copilot, so a Copilot plan edit silently staleness-violates). (2) **The `_system/` JSON files are the editor↔pipeline API and Claude owns their schema** (the pipeline is the consumer of record, wired at Slice 6); Copilot consumes, never forks a schema. (3) **TS↔Python mirror files are a shared seam** — `plan-dashboard/src/lib/content-paths.ts`↔`_paths.py`, `peq-scores.ts`↔`_quality.py`+`challenger_scoring.py`; when Claude changes the Python source it MUST update or flag the TS mirror (and vice-versa) so they never diverge. Async coordination runs through `_workspace/plan/copilot-handoff.md` (Claude writes Copilot's tickets there; Copilot reads at session start, appends a session log at end). Copilot's mirror of these rules lives in `.github/copilot-instructions.md` §Two-agent model. Note: the git `pre-commit` hook is NOT installed in every clone, and Claude's hooks (snapshot-regen, cortex-view-guard, ui-reviewer-stop) do NOT fire for Copilot — so Copilot must run `npm run lint:views` manually before committing a view. Tier-2 actions (orchestrator launch, publish, `develop→main`, force-push, branch deletion) remain Asif's regardless of agent.
