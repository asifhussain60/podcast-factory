"""tests/test_wisdom_ingest.py — B0 acceptance tests for the Kashkole ingestion driver.
...
pytestmark = pytest.mark.skipif(
    not _IMPORT_OK,
    reason=f"wisdom_ingest_knowledge not importable: {_IMPORT_OK or _IMPORT_ERR}",  # type: ignore[name-defined]
)
    ...
    for ch_dir, _bd, _cd in _iter_chapter_dirs():
        ...
        if v == "WARN":
            ...
            row = conn.execute(
                ...
                (f"wisdom:{b['binder_id']}:{b['chapter_id']}",),
            ).fetchone()
