...
Before returning control to the caller, regenerate the three snapshot JSON files that drive the Podcast Factory Astro Site. These are the contract between the agent and the dashboard — the dashboard never reads repo state directly, only these snapshots.
