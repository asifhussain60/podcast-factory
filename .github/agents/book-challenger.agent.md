...
| BK-N7 | **Register and tone** | **P1** | The prose departs from the register the frame implies: **lecture voice** under a third-person frame (REQ-BA-125, `R-NO-LECTURE-VOICE`) — narration that turns to the reader ("you", "your") or directs an audience ("Hold that frame", "Do not pass over that phrase lightly", "consider the grammar"), and commentary about the discourse itself ("this is the heart of it", "as we shall see"). Report the RECAST, never a deletion — the move carries a thought. Second person inside quoted speech, verses, hadith, prayers and block quotations is correct under every frame and is never a finding; neither is any of this under a first-person frame, where addressing the reader is the form. Also **navigation apparatus** (REQ-BA-126, `R-NO-NAVIGATION-APPARATUS`) — prose locating itself in the SOURCE's division scheme ("the fourth chapter of the first gate of the first canopy") or reciting it ("five canopies, each of five gates"), and any SECOND explanation of what a suradiq/bab/fasl IS. The edition has numbered chapters and no canopies, so the scheme names nothing a reader can turn to; report the RECAST, never a deletion, and never flag a heading or Arabic line the source itself prints. Also: authorial intimacy under a third-person frame, archaic diction, ornament that makes the translator visible, or **elegant variation** — the same technical term rendered differently on different occurrences. In this genre terminological consistency is the prose virtue and synonym variety is a defect, because a reader must be able to trace one term through the whole argument. |
