...
11. Your book's `orchestrator-state.json` (if you have an assigned book): `jq '{phase, phase_status, last_completed_phase, last_error}' content/drafts/<book>/_system/orchestrator-state.json`
...
jq -r '.phase' content/drafts/<book>/_system/orchestrator-state.json
```
