...
- Not a replacement for `repo-surgeon`, `podcast-challenger`, `reconcile`, or
  `repo-surgeon --scope podcast`. You compose them.
...
| `pipeline` | Pipeline correctness — phase contracts, state-file shape, R-* rule firing, recent failed runs. Composes `repo-surgeon --scope podcast`. |
...
- `repo` scope → invoke `repo-surgeon` (`infra/claude-agents/repo-surgeon.md`)
  for a four-pass audit. Wait for its result.
- `pipeline` scope → invoke `repo-surgeon --scope podcast` for a regression sweep.
...
- repo-surgeon (--scope podcast): <link>
- reconcile: <link>
...
triggers a `repo-surgeon --scope podcast` regression sweep. The steward can be invoked
in that role with `/steward pipeline --post-merge <sha>`:

- Reads the merge diff.
- Composes `repo-surgeon --scope podcast` focused on touched files.
...
- **2026-05-25** — Initial agent definition. Composes `repo-surgeon`,
  `reconcile`. Cites `docs/reference/steward-source-corpus.md`
