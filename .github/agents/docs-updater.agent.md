...
7. **Category T — doctrinal accuracy** — T1 (canonical attribution), T2 (Imam lineage), T3 (forbidden naming phrases — the canonical examples pair the leadership-title with the personal name of the Father of Imams and the parallel pairing with Fatima → P0 each; full list in `content/_shared/islam/naming-conventions.yml`), T4/T5 (stubs). D3 decision tree showing input chapter → 5 checks → outcomes. Pull from `_doctrinal.py` + `content/_shared/islam/*.yml`.
