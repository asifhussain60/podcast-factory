...
check(
  "a book link redirects so the recipient can sign in",
  (await get("/book/ayyuhal-walad")).status,
  302,
);
...
check(
  "the About page redirects, it is not public",
  (await get("/about")).status,
  302,
);
...
check(
  "its data endpoint hands back a redirect, not the page",
  aboutBody.includes("SingleFetchRedirect"),
  true,
);
...
check(
  "and none of the page's own content",
  aboutBody.includes("What’s new"),
  false,
);

console.log("\nunmatched path (middleware never runs here)");
check(
  "renders a 404 rather than throwing 500",
  (await get("/nonexistent")).status,
  404,
);
check(
  "still 404 with a session",
  (await get("/nonexistent", admin)).status,
  404,
);
...
check(
  "admin sub-page is 404",
  (await get("/admin/content", outsider)).status,
  404,
);
check(
  "case-variant admin is 404",
  (await get("/Admin/content", outsider)).status,
  404,
);
...
check(
  "a chapter is 404",
  (await get(`/book/ayyuhal-walad/read/${CHAPTER}`, outsider)).status,
  404,
);
check(
  "the slide deck is 404",
  (await get("/book/ayyuhal-walad/slides", outsider)).status,
  404,
);
check(
  "a media file is 404",
  (await get("/media/ayyuhal-walad/book.pdf", outsider)).status,
  404,
);
...
check(
  ...
  (await get("/media/ayyuhal-walad/degrees-of-excellence/book.pdf", admin))
    .status,
  404,
);
...
check(
  "denied for someone without the grant",
  (await get(filtered, outsider)).status,
  404,
);
...
const readFiltered = `/book/ayyuhal-walad/read/${CHAPTER}.data?_routes=routes%2Fbook.%24slug.read.%24chapter`;
check(
  "denied on the chapter route too",
  (await get(readFiltered, outsider)).status,
  404,
);
check(
  "control: granted user gets the chapter",
  (await get(readFiltered, admin)).status,
  200,
);
...
check(
  ...
  strip(await denied.text(), "ayyuhal-walad") ===
    strip(await missing.text(), "no-such-book-exists"),
  true,
);
...
check(
  "and the chapter carries no companion note",
  readerPage.includes(MARKER),
  false,
);
...
check(
  "their right-hand drawer is their own notes",
  readerPage.includes("Open your notes"),
  true,
);
check(
  "and no Companion panel is rendered",
  readerPage.includes("Open companion"),
  false,
);
...
check(
  "control: the administrator's copy of the same page carries it",
  adminPage.includes(MARKER),
  true,
);
check(
  "control: and their drawer is the Companion",
  adminPage.includes("Open companion"),
  true,
);
check(
  "control: which REPLACES the notes drawer, not joins it",
  adminPage.includes("Open your notes"),
  false,
);
...
check(
  ...
  (await get("/book/degrees-of-excellence", `${outsider}; ${forged(ADMIN)}`))
    .status,
  404,
);
...
check(
  "the administrator simulating loses the admin screens",
  (await get("/admin", asThem)).status,
  404,
);
...
check(
  "but is no longer shown the companion",
  (await simulated.text()).includes(MARKER),
  false,
);

// The way out cannot be behind the gate the simulation just closed.
check(
  "stopping is reachable while simulating",
  (await post("/stop-simulating", asThem)).status,
  302,
);
...
check(
  "and neither is a signed-out visitor",
  await linkShown("/sign-in", ""),
  false,
);
    ...
    const out = String(
      execFileSync(
        ...
        [
          ...
          "--json",
          "--command",
          `SELECT note, quote, user_email FROM annotation WHERE id = '${OWNED}'`,
        ],
        { encoding: "utf8" },
      ),
    );
...
check(
  "control: the administrator's search finds the other book",
  adminSearchBody.includes(OTHER_BOOK),
  true,
);
...
check(
  "but the other book is not in their results",
  readerSearchBody.includes(OTHER_BOOK),
  false,
);
...
check(
  "their own book is still searchable",
  (await ownSearch.text()).includes("Ayyuha"),
  true,
);
...
const searchData = await get(
  "/search.data?_routes=routes%2Fsearch&q=intellect",
  outsider,
);
check(
  "the data endpoint leaks nothing either",
  (await searchData.text()).includes(OTHER_BOOK),
  false,
);
  ...
  const out = String(
    execFileSync(
      ...
      [
        ...
        "--json",
        "--command",
        "SELECT id FROM search_passage WHERE slug = 'spiritual-ethos' LIMIT 1",
      ],
      { encoding: "utf8" },
    ),
  );
...
check(
  "revoked person is sent to no-access",
  (await get("/", outsider)).status,
  302,
);
