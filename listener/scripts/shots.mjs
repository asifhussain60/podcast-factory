...
const STATES = {
  reader: [
    ...
    {
      // The widest of the three page widths, which is the whole point of having
      // them: the setting moves the SHEET as well as the column, and a shot of
      // the default can only ever show that the buttons exist. Below 900px the
      // control is hidden — the page is already window-capped there — so this
      // state is a no-op on the phone and tablet widths rather than a failure.
      name: "widest",
      act: async (page) => {
        await page.click('[aria-label="Widest page width"]').catch(() => {});
    ...
    },
    {
      ...
      act: async (page) => {
        ...
        await writeNote(
          page,
          "The debt of gratitude is the hinge of the whole passage.",
        );
        ...
        await page
          .waitForSelector(".pf-drawer--end", { timeout: 2000 })
          .catch(() => {});
      },
    },
  ],
    ...
    },
    {
      ...
      act: async (page) => {
        ...
        await page
          ...
          .catch(() => {});
        await page
          .waitForSelector(".pf-player-panel", { timeout: 3000 })
          .catch(() => {});
  ...
  "admin-people": [
    ...
    },
    {
      ...
      act: async (page) => {
        ...
        await page
          .getByRole("link", { name: /No access yet/ })
          .click()
          .catch(() => {});
        await page
          .waitForSelector(".pf-table", { timeout: 3000 })
          .catch(() => {});
        await page
          .locator(".pf-table")
          .scrollIntoViewIfNeeded()
    ...
    },
    {
      ...
      act: async (page) => {
        ...
        await row
          .locator('[aria-label^="Rename"]')
          .click()
          .catch(() => {});
        await page
          .waitForSelector(".pf-rowedit", { timeout: 3000 })
          .catch(() => {});
    ...
    },
    {
      ...
      act: async (page) => {
        ...
        await row
          ...
          .catch(() => {});
        await page
          .waitForSelector(".pf-confirm", { timeout: 3000 })
          .catch(() => {});
        await row.scrollIntoViewIfNeeded().catch(() => {});
      },
    },
    {
      ...
      act: async (page) => {
        ...
        await page
          .waitForSelector(".pf-facts", { timeout: 3000 })
          .catch(() => {});
      },
    },
  ],
  "admin-content": [
    { name: "plain", act: async () => {} },
    {
      ...
      act: async (page) => {
        ...
        await page
          .waitForSelector(".pf-grant", { timeout: 3000 })
          .catch(() => {});
      },
    },
  ],
...
async function openComposer(page) {
  ...
  await page
    .waitForSelector("#pf-note-draft", { timeout: 2000 })
    .catch(() => {});
}
...
async function writeNote(page, text) {
  ...
  await page
    .waitForSelector(".pf-selbar", { state: "detached", timeout: 2000 })
    .catch(() => {});
}
...
if (book === null)
  console.log("  !     no published book — book and reader surfaces skipped");
...
const routes = [
  ...
  ...(book === null
    ? []
    : BOOK_ROUTES.filter(
        (r) => r.expect === 200 && !r.path.endsWith("/marks"),
      )),
  ...(book === null
    ? []
    ...
    for (const theme of themes) {
      for (const route of routes) {
        ...
        await context.addInitScript(
          `localStorage.setItem("pf-theme", ${JSON.stringify(theme)})`,
        );
        ...
        for (const state of STATES[route.label] ?? [
          { name: "plain", act: async () => {} },
        ]) {
