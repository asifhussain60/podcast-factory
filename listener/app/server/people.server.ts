/**
 * Administering PEOPLE: invitations, names, and the grants they hold.
 *
 * Split out of `access.server.ts` on 2026-08-17. That file answers one question
 * on every reader request — may this person see this thing — and it had grown to
 * carry six hundred lines of provisioning CRUD reachable only from `/admin`. The
 * two are different jobs on different paths, and the resolver is the one that
 * must stay small enough to read in full.
 *
 * The same reasoning already produced `companion.server.ts`: a file whose header
 * says nothing in it asks who may see what should not be where that question is
 * answered, and the inverse holds here.
 *
 * What did NOT move, deliberately:
 *
 *   - `hasLiveInvite` stays in `access.server.ts`. It is the per-request sign-in
 *     gate, imported by both `middleware/authed.ts` and Better Auth's hooks, and
 *     it belongs with the resolver it guards rather than with the screens that
 *     administer invitations.
 *   - `event` stays there too, and is imported here. Every mutation below writes
 *     its audit row inside the SAME `db.batch` as the row it describes, which is
 *     what stops the two disagreeing; a second copy of that helper would be a
 *     second way to write history.
 *
 * Every function here is reachable exclusively from behind the admin gate in
 * `app/middleware/admin.ts`. Nothing here is on a reader's path.
 */

import { event, type Grant } from "./access.server";
