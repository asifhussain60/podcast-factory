...
- **Canonical app names (LOCKED 2026-05-29; second app added 2026-08-03).** The repo has **two** web apps, and they are not interchangeable.
  ...
  - **The Podcast Factory Listener** — the AUDIENCE-facing site. Directory `listener/`; React Router 8 + Vite on Cloudflare Workers, invite-only Google sign-in, its own D1 database, its own R2 bucket, its own subdomain, and its own Verdigris palette in the `--l-*` / `bg-pf-*` namespace. Dev server on **:5273** (`cd listener && npm run dev`) — never 4322, which belongs to the Astro site. It imports NOTHING from `plan-dashboard/` at runtime; its single build-time dependency is `renderMarkdown` from `plan-dashboard/src/lib/reader/markdown.ts`, called by the publish step so the on-screen reader and the print PDF cannot diverge, and pinned by a golden-file fixture in `listener/test/`.
