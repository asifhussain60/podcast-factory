...
What this phase CORRECTS:
 ...
 - **Authorial-apparatus noise** (R-NOISE-AUTHORIAL-APPARATUS, Wave-N): strip the author's own NON-teaching meta about the book-OBJECT — circulation/copyright notices (do-not-email/share/store, copy-is-a-sin, circulation-punishment threats), ijazat-to-record + treasury-deposit provenance, colophons. The denoise prompt carries `R_NOISE_APPARATUS_DIRECTIVE` (`_rules.py`) so this is stripped at the root. PROTECT (never strip — it is teaching): wilayah/allegiance doctrine and the inherited-from-prophets/saints epistemics. Cross-surface detection of any that leaks through is the [`noise-auditor`](../../infra/claude-agents/noise-auditor.md) agent's job (`NZ-*` findings).
