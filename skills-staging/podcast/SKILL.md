...
**PODCAST_ROOT** = `<REPO_ROOT>/content/` — the parent of the type-first content buckets (`Islamic/`, `Technical/`, `Fiction/`, `Guides/`). Draft vs published is a STATUS field (`status` in `_system/orchestrator-state.json`, mirrored to `publication.status` in `meta.yml`), not a folder — the 2026-05-23 `drafts/` + `published/` split was retired 2026-06-04. The `_system/` (book-agnostic references) layer is `content/_shared/` + per-script defaults.
**SHARED_ARABIC** = `<REPO_ROOT>/content/_shared/arabic/` — **RETIRED, does not exist on disk** (2026-05-23 restructure; confirmed absent 2026-08-05). Every `SHARED_ARABIC/*` reference below is dead — do not attempt to read one, and do not treat it as a gate. The real, current authority for how a term sounds is the deterministic ladder in [scripts/podcast/knowledge/term_render.py](../../scripts/podcast/knowledge/term_render.py), compiled into the framing by [scripts/podcast/_pronunciation_block.py](../../scripts/podcast/_pronunciation_block.py): per-book override table (`BOOK_DIR/_system/pronunciation.md`) → loanword allowlist → English exonym table → `confirmed` row in the cross-book ledger (`content/knowledge-base/pronunciations.jsonl`) → gloss → plain transliteration. Settle an unresolved term with `scripts/podcast/run_pronunciation_probe.py`, never by editing a framing directly. (Full detail at line ~183 below — this line exists so the ladder is visible before anything else in the file is read.)
**BOOK_DIR** = `content/<Bucket>/<book-slug>/` (bucket derived from the book's `content_profile` via `_rules.bucket_for_profile`; resolve with `_paths.find_content`). The same directory serves draft and published states — `scripts/podcast/publish_to_library.py` flips the status in place. Has `_README.md` plus four subfolders:
 ...
 - `transcripts/` — holds **two different contracts**, and the pipeline writes both (the note that used to stand here calling this folder "human-input only" was wrong):
   - `EP##-<slug>.transcript.txt` — FLAT prose, no clock, one per episode. Dropped by Asif or written by `transcribe_episode.py` / `transcribe_notebooklm.py`. Read by `scripts/podcast/audit_transcript.py` for the lexical audit and by the `podcast-challenger` Loop M empirical-transcript audit. Nothing ever rewrites an existing one — see the header of `_transcript.py` for why.
   - `ep##.vtt` — TIMED, keyed by episode number. What the Library ships, what the read-along highlights against, and what the Sessions lane reads as a spoken chapter's source. Written only by `ensure_transcripts.py`.
   - `_inbox/` — where externally-produced timed transcripts (`.vtt` / `.srt`) are dropped. `ensure_transcripts.py` looks here BEFORE it spends anything: a file that names its episode (`ep07.vtt`, `[Part 7]`, `Session 7`) and passes its checks is adopted, and Azure is never called for that episode. Measured on lecture audio, the bought transcript is the WORSE artifact — see the header of `_external_transcript.py`. A file that fails is refused BY NAME with its reason; `--adopt-only` refuses to fall back to Azure at all.
   - `_provenance.json` — which route produced each timed transcript, and for an adopted one, how much of the recording it covered. The cost ledger cannot answer this: an adopted transcript is free and so leaves no row at all.

At session start, list `content/*/` (or run `scripts/podcast/cross_book_dashboard.py`) to see in-flight books — those with `status: draft` in `_system/orchestrator-state.json`. If a book is being worked, verify `BOOK_DIR/_system/`, `BOOK_DIR/episodes/`, and `BOOK_DIR/transcripts/` exist. If missing, run the scaffold protocol in Section 1.

**Important — handbook tree retirement (2026-05-23), scope corrected 2026-08-05:** `content/podcast/.skill/handbook/` was retired in the 2026-05-23 restructure and does not exist on disk (confirmed absent 2026-08-05) — **every** `PODCAST_ROOT/.skill/handbook/*` reference anywhere in this file is dead, not just the numbered cold-start list below. An agent MUST NOT try to `Read` one of these paths, and MUST NOT treat a rule stated as "mandatory," "required," or "P0 gate" against one of these paths as blocking — that combination (a real gate pointed at a nonexistent file) is a defect, not a stricter rule. Four of these have a verified current replacement:
...
The rest — `source-distillation.md`, `episode-architecture.md`, `scratchpad-markers.md`, `notebooklm-best-practices.md`, `arabic-tts-protocol.md`, `pre-refined-source-mode.md`, `numeric-symbolic-disambiguation.md`, `extract-capability.md`, `workspace-readme-template.md`, `_learned-addenda/` — have **no rediscovered replacement**. This is stated plainly rather than guessed at: nothing found in `scripts/podcast/` was verified to carry their content forward. Where the file names one of these below, treat the surrounding named rule as historical color only — proceed using this file's own operational instructions in that section instead of trying to look the handbook file up.

Same rule for every `SHARED_ARABIC/*` path below (see the SHARED_ARABIC definition above): dead reference, never a gate.
...
- **Work-level teaching allocator (multi-volume works, 2026-06-18)** — [`allocate_teachings.py`](../../scripts/podcast/allocate_teachings.py) places every teaching of a synthesized work's ledger into exactly one volume+chapter, deduplicated, before any volume's 0d (no-loss/no-repeat GATE that raises on failure; flat-rate `claude -p`; resume-safe `_system/_alloc/`; emits `_system/_volume-split.json`). [`orchestrate_work.py`](../../scripts/podcast/orchestrate_work.py) `_ensure_allocation()` runs it once per synthesized work. 0d ([`_chapter_design.py`](../../scripts/podcast/_authoring/_chapter_design.py) `_volume_allocation()`) injects the per-volume concept list + cross-volume de-dup seed — GATED so single-book 0d is byte-identical; a malformed `_volume-split.json` WARNS, never silently degrades. Per-book `config.episode_max_concepts` (default = `EPISODE_MAX_CONCEPTS` 3) gives curated breathable seasons; the per-source-chapter slicer clamps `end_line` to file length (fixes TOC overshoot for all books).
...
**INVARIANT 3: Chapters are designed, not promoted — and segmented by cognitive load.** When a source PDF is ingested, the source's published section structure is NOT directly promoted to chapters. Phase 0d re-segments by meaningful thematic units, balanced ~2,500–3,500 words per chapter (floor 1,500, ceiling 4,500). Each chapter is itself a NotebookLM source. **One episode carries ONE coherent teaching cluster** — a chapter that packs many distinct teachings (e.g. eight admonitions, a dozen lessons) is several episodes, not one marathon. The wider **"extended" tier (5,500–9,500) is for LOW-density narrative** only; dense doctrinal material (Arabic-scholarly) targets the "longer" tier (≤4,500) and SPLITS rather than balloons. This is enforced, not advisory: Phase 0d halts-and-surfaces (`_chapter_design.py` density guardrail) when any planned episode exceeds the per-episode density ceiling (`EPISODE_DENSITY_CEILING_DENSE`=6,000 for doctrinal, `…_NARRATIVE`=9,500 for narrative — `_validator_constants.py`).
...
**INVARIANT 5: All Islamic Arabic appears in chapters, with phonetics in the library.** Phase 0c builds the per-book phonetics index and glossary. The chapter source persists glossary-backed Arabic as `romanized term (Arabic script)` so Islamic books are never transliteration-only. The **customize prompt's `## Pronunciation` block** (per R-PRONUNCIATION-IMPERATIVE) remains the single surface where phonetic respelling reaches NotebookLM, in imperative form (`Pronounce "Tasawwuf" as "ta-SAW-wuf". Say it as one fluent word.`). Chapter prose still carries **no inline `(PHO-ne-tic; gloss)` parens and no post-transliteration phonetic lines in blockquotes** — per R-PHONETICS-OUT. (Architectural pivot 2026-06-24: Arabic script is required in Islamic chapters; phonetic respelling stays out of prose.)
...
Items 1–6a and 7–14, 18, 20–21 below name paths retired 2026-05-23 (see the notices above). Skip reading them; the annotation on each names its real current authority where one is verified, or says plainly that none was found.

1. `SHARED_ARABIC/00-README.md` — retired. Real authority: the term_render.py ladder (see SHARED_ARABIC definition above).
2. `SHARED_ARABIC/01-tts-pronunciation-key.md` — retired. No rediscovered replacement for the TTS-respelling engineering rules; the ladder above still governs which spelling is used.
3. `SHARED_ARABIC/02-quran-letter-phonetics.md` — retired. No rediscovered replacement.
4. `SHARED_ARABIC/03-arabic-english-manifest.md` — retired. Real authority: the `confirmed` rows in `content/knowledge-base/pronunciations.jsonl`, reached via the term_render.py ladder.
5. `SHARED_ARABIC/04-common-term-substitutions.md` — retired. No rediscovered replacement for the nafs/shaytan/ruh-style substitution table; treat the substitution instructions elsewhere in this file as historical color, not a gate.
6. `SHARED_ARABIC/05-name-alias-policy.md` — retired. No rediscovered replacement; `handbook/worked-examples.md` in the same sentence is also retired.
6a. `SHARED_ARABIC/06-abjad-numerals.md` — retired. No rediscovered replacement for the abjad tables; do not treat a letter-count claim as gated on a file that doesn't exist.
7. `PODCAST_ROOT/.skill/handbook/notebooklm-source-chapter-rules.md` — retired. Real authority: [scripts/podcast/_rules.py](../../scripts/podcast/_rules.py) + podcast-challenger.md Categories (Loops B/C/D/E).
8. `PODCAST_ROOT/.skill/handbook/notebooklm-customize-prompt-rules.md` — retired. Real authority: same as above (Loops F/H/I/J/K).
9. `PODCAST_ROOT/.skill/handbook/notebooklm-source-format.md` — retired. No rediscovered replacement for the file-by-file format notes.
10. `PODCAST_ROOT/.skill/handbook/two-host-framing.md` — retired. Real authority: podcast-challenger.md Category F.
10b. `PODCAST_ROOT/.skill/handbook/debate-framing.md` — retired. Real authority: podcast-challenger.md Category P.
11. `PODCAST_ROOT/.skill/handbook/source-distillation.md` — retired. No rediscovered replacement; use judgment per source type, guided by Phase 0a's normalization step in `scripts/podcast/_authoring.py`.
12. `PODCAST_ROOT/.skill/handbook/episode-architecture.md` — retired. No rediscovered replacement.
13. `PODCAST_ROOT/.skill/handbook/scratchpad-markers.md` — retired. No rediscovered replacement, and no code under `scripts/podcast/` was found generating or consuming an `@@` marker legend — verify with Asif whether the scratchpad step below (step 6 of chapter refinement) is still live machinery before relying on it.
14. `PODCAST_ROOT/.skill/handbook/notebooklm-best-practices.md` — retired. No rediscovered replacement; where this file cites a specific section number below (§3, §5), treat the surrounding instruction as the operative one.
...
18. `PODCAST_ROOT/.skill/handbook/arabic-tts-protocol.md` — retired. No rediscovered replacement.
...
20. `PODCAST_ROOT/.skill/handbook/pre-refined-source-mode.md` — retired. No rediscovered replacement for this document, but its subject (Pre-Refined Source Mode / Mode-3) is described in full inline in this file — see the bypass section below.

21. `PODCAST_ROOT/.skill/handbook/numeric-symbolic-disambiguation.md` — retired. No rediscovered replacement. **Verified 2026-08-05: podcast-challenger.md's Loop N is a different check** (phonetic-guide architecture, not numeric/symbolic disambiguation) — the "enforced at ship time by Loop N" claim this line used to make was false. There is currently no ship-time gate for counts-without-enumeration, abjad ciphers, or anachronistic modern glosses; apply judgment from `_workspace/plan/numeric-symbolic-disambiguation-plan.md` if it still exists, otherwise flag the ambiguity to Asif rather than guessing.
...
**The two NORMATIVE handbook files (notebooklm-*-rules) are mandatory on every run, not optional.** They are the authority for every customize-prompt template and every chapter-as-source constraint. Guidance files explain WHY; normative files state WHAT.

**They are NOT the authority for what a term sounds like.** The SHARED_ARABIC manifest this section named until 2026-08-01 (`content/_shared/arabic/`, files 00-README through 06-abjad-numerals) was RETIRED in the 2026-05-23 restructure and does not exist — pointing prompts at it is how the `## Pronunciation` block ended up authored from nothing. The spoken form of every term is resolved by ONE deterministic ladder, `scripts/podcast/knowledge/term_render.py`, and compiled into the framing by `scripts/podcast/_pronunciation_block.py` at build time (R-PRONUNCIATION-RENDER). Its rungs, in order: the per-book override table at `BOOK_DIR/_system/pronunciation.md` → the loanword allowlist → the English exonym table → a `confirmed` row in the cross-book ledger (`content/knowledge-base/pronunciations.jsonl`) → a gloss → plain transliteration. Change what a term sounds like by editing the override table or by settling it with the probe (`scripts/podcast/run_pronunciation_probe.py`), never by editing a framing.

If `PODCAST_ROOT` is missing the top-level books index, scaffold it before continuing:
 - Create `PODCAST_ROOT/.skill/books.md`. The template this used to cite (`PODCAST_ROOT/.skill/handbook/workspace-readme-template.md`) is retired with no rediscovered replacement — build the index from the schema item 15 above already describes (one row per book, pointing at that book's own registry) rather than trying to read a dead template.
...
**Bypass for single-chapter sources (Extract Mode)**: when the source is already a single chapter `.txt` file (pre-prepared book chapter, single-episode re-run), skip this entire SKILL — Phase 0 pre-reads, all of it (the SHARED_ARABIC index and handbook normative refs it used to also name are retired — see the notices above). Use the `/extract-chapter <chapter-ref>` slash command (wired to the `podcast-extract` agent at `.github/agents/podcast-extract.agent.md`), which drives `scripts/podcast/extract_chapter.py` deterministically from a per-chapter contract under `_workspace/<category>/<book-slug>/chapter-contracts/<slug>.yml`. (`content/podcast/.skill/handbook/extract-capability.md`, this section's former "full spec," is retired with no rediscovered replacement — `extract_chapter.py`'s own contract validator is the operative authority.) Extract Mode is the right answer for single-episode re-runs and any source where chapter design has already been done by hand.

**Bypass for pre-refined multi-chapter sources (Pre-Refined Source Mode)**: when the source is a *multi-chapter book whose prose has already been refined by the user* (per-chapter `.md`/`.txt` files at the book root, with editorial commentary already authored inline), do NOT run Phase 0a–0e. Forcing the orchestrator over pre-refined prose destroys the user's editorial work (Phase 0b re-refines; Phase 0d re-segments). Instead, build a `BOOK_DIR/_notebooklm/` scaffolding directory: master source index, centralized pronunciation guide (with stress cues + do-not-pronounce-as table + honorific protocol), listener glossary, source-integrity notes (every quotation/attribution/theological claim classified), Do Not Say guardrails, episode arc map, human-review checklist, and per-chapter scaffolding files (Source Card + Episode Intelligence + Host Questions + Podcast Factory Library Difficulty + Review Lens + Podcast Factory Library Fit + Episode Opener/Closer + NotebookLM Instruction). The chapter prose is frozen; only scaffolding is added. **Arabic pronunciation in this mode is delivered through two surfaces**: (1) `_notebooklm/01-pronunciation-guide.md` uploaded to NotebookLM as a standing reference, AND (2) per-episode customize-prompt `## Pronunciation` block in R-PRONUNCIATION-IMPERATIVE form (per R-PRONUNCIATION-IMPERATIVE). Lookup order for every Arabic term is the term_render.py ladder described above, the same as Phase 0c — not the retired `SHARED_ARABIC/*` paths this line used to name. (`content/podcast/.skill/handbook/pre-refined-source-mode.md`, this section's former "full spec," is retired with no rediscovered replacement — this paragraph is the operative description of the mode.) Canonical worked example: `content/Islamic/the-master-and-the-disciple/_notebooklm/`.

**Splitting policy**: when a source chapter exceeds the 4,500-word ceiling (Section 0, Invariant 3), split it into derivatives with clean single-noun English titles (kebab-case, no version suffixes) and record provenance via the `derived_from:` field in each derivative's contract. (`content/podcast/.skill/handbook/extract-capability.md`, this line's former "full spec," is retired with no rediscovered replacement — the ceiling and naming rule stated here is the operative one.)
...
What this phase CORRECTS:
 ...
 - **Authorial-apparatus noise** (R-NOISE-AUTHORIAL-APPARATUS, Wave-N): strip the author's own NON-teaching meta about the book-OBJECT — circulation/copyright notices (do-not-email/share/store, copy-is-a-sin, circulation-punishment threats), ijazat-to-record + treasury-deposit provenance, colophons, front-matter/readership/book-description/author-bio/chain-of-narration/permission-to-read apparatus, and chapter-prose reference tails after wisdom/saying blockquotes (`NZ-REFERENCE-TAIL`, e.g. "in Nahj al-Balagha (compiled by al-Sharif al-Radi), Hikam (Saying) 147"). The denoise prompt carries `R_NOISE_APPARATUS_DIRECTIVE` (`_rules.py`) so this is stripped at the root, and Phase 0e runs `strip_reference_attribution_noise.py` as a deterministic backstop. Arabic script in teaching content is preserved for the later Arabic/pronunciation review stage. PROTECT (never strip — it is teaching): the quoted saying itself, the speaker attribution, wilayah/allegiance doctrine, and the inherited-from-prophets/saints epistemics. Cross-surface detection of any that leaks through is the [`noise-auditor`](../../infra/claude-agents/noise-auditor.md) agent's job (`NZ-*` findings).
...
Goal: every Arabic term, Quranic verse rendering, hadith line, dua, name, and honorific is captured in the phonetic library and, for Islamic scholarly books, has Arabic script available for deterministic chapter injection. The hosts must be able to read every Arabic-derived word aloud correctly without inline phonetic clutter.

**Architectural note (2026-06-24).** Phonetic guidance is delivered to NotebookLM exclusively via the customize prompt's `## Pronunciation` block (R-PRONUNCIATION-IMPERATIVE). This phase therefore builds an INDEX of every Arabic term + canonical phonetic + Arabic script for the book, which later phases use to populate each episode's customize prompt and to persist visible Arabic in chapter sources. **The chapter file itself never carries inline `(PHO-ne-tic)` parens or post-transliteration phonetic lines** (R-PHONETICS-OUT). The term appears in the chapter as transliteration plus Arabic script, while the customize prompt teaches the voice model how to say it.
...
Method:
 ...
 2. For every Arabic term, transliteration, name, or honorific, record it in `_phonetics.md` / `glossary.yml` with: canonical spelling, Arabic script, phonetic form, brief gloss, first-appearance citation. **No inline phonetic annotation is written into the chapter** (R-PHONETICS-OUT). The glossary drives both the customize-prompt Pronunciation block and deterministic chapter Arabic injection.
 3. **Lookup order for every Arabic term**: do not draft one. The ladder in `scripts/podcast/knowledge/term_render.py` resolves every term deterministically — per-book override table → loanword → English exonym → `confirmed` ledger row → gloss → plain transliteration — and `_pronunciation_block.py` compiles the result into the framing at build time. A term the ladder can only spell back is REPORTED, not guessed at: settle it by ear with `run_pronunciation_probe.py`, which writes the answer to the cross-book ledger so no later book re-derives it. (Superseded 2026-08-01: this step used to route through `SHARED_ARABIC/`, retired in the 2026-05-23 restructure.)
 4. **Substitution check**: `SHARED_ARABIC/04-common-term-substitutions.md` is retired with no rediscovered replacement, so this is judgment, not a lookup against a table. If a term has an obvious context-driven English substitute (e.g., *nafs → soul / lower soul / irascible soul*; *shaytan → Satan*; *ruh → spirit*), replace the Arabic with the appropriate English form for the surrounding context. Keep the Arabic when it is a technical term the chapter is deliberately teaching, or when a confident substitution isn't available.
 ...
 ```
 6. Cross-link `glossary.yml` with `BOOK_DIR/_system/pronunciation.md` (the book-level user-edited overrides). Conflicts resolve in the ladder's order, and the override table wins: a human who has heard the audio outranks every table below them.

Coverage rules (enforced — fail the phase if any are violated):
 - Islamic scholarly chapters must contain Arabic script from the glossary after deterministic injection; finalize gate G13 blocks zero-Arabic chapters.
 ...
 - Advisory, not a phase-failing gate (the substitution table this rule used to check against is retired — see step 4 above): a common Arabic term with an obvious English substitute should either (a) be substituted or (b) carry a documented justification in `00-framing.md`'s pronunciation block for keeping the Arabic.
...
Sizing and chapter-count rules (per INVARIANT 6 — the retired `notebooklm-best-practices.md` §3 this used to also cite has no rediscovered replacement; the rules stated below are the operative ones):
...
| **Extended Deep Dive** (recommended for dense / philosophical / technical sources) | **~30–45 min** | **5,500–9,500** |
 ...
 - **Concept ceiling (R-MAX-CONCEPTS, 2026-06-10 — `docs/standards/chapter-density.md`).** Each chapter carries AT MOST 3 concept-level `## H2` sections (structural frames excluded), ~1,500–2,500 words each. The word band alone is NOT sufficient: a 5,400-word chapter holding 9 distinct teachings is three episodes, not one. Enforced deterministically: the Phase 0d post-write gate rejects over-dense chapters (`_authoring/_chapter_design.py`), and the $0 preflight smoke gate halts the per-chapter loop for books with `density_standard: 2` in series-config. Keep each concept WHOLE within one episode and order episodes so each landing hands off to the next opening.

Content rules baked into the Phase 0d author prompt (2026-06-10, `CHALLENGER_VERSION = "2.5"`):
 - **R-QURAN-CITATION-FORMAT** — every Quranic quotation carries its inline reference in plain English: `(chapter N, verse M)`. Never `(Q N:M)`, `(Quran N:M)`, or bare `(N:M)` (TTS reads them as opaque numbers). References come from the source; never invented (`_validators.assert_quran_citation_format`, P1 flag).
 - **R-NO-TRANSLIT-FORMULA** — verbatim Arabic formulae/verses/hadiths appear as their accurate English translation ONLY; the `*Arabic translit run* — *translation*` pair form is forbidden (`_validators.assert_no_translit_formula_pairs`, P1 flag; ≥4-token italic runs with diacritics). Short inline Arabic terms without diacritics stay allowed.
 - **R-SERMON-VERBATIM** — sermons/khutbas/formal addresses render WHOLE (refined English, never summarized) as their OWN concept section in exactly one episode; the contract carries `sermon: {present: true, section_title}`; the framing author injects a `## Verbatim Recitation` block instructing the hosts to read the sermon aloud word-for-word before discussing it (post-author gate in `_authoring/_framing.py`; set-level integrity check CS9 in `check_chapter_set.py`).
 - **Chapter-set integrity (CS7–CS11)** — post-0d, `check_chapter_set.py` verifies the split is a PARTITION of the source: coverage (nothing dropped, P7), no overlap/duplication (P8), sermon integrity (P9), set density (P10); the challenger judges flow/conceptual integrity (CS11). For `density_standard: 2` books, P0 findings halt before Phase 0e spend.
...
Allowed enrichment sources, citation format, and tradition-mix principles are codified in the canonical whitelist — retired as a standalone file; inlined into [scripts/podcast/_authoring/_enrichment.py](../../scripts/podcast/_authoring/_enrichment.py)'s Phase 0e prompt. Consult that module before adding any enrichment. The whitelist is tiered:
...
Placement guidance:
 ...
 - Each enrichment carries clear attribution without bibliographic clutter in chapter prose: scripture/hadith may need concise source locators, while wisdom/saying blockquotes should keep the speaker and omit edition/translator tails.
 - Format pattern for wisdom sayings: `> [quote]. — [speaker].` Brief connective tissue before and after weaves it into the argument; detailed source metadata belongs in logs/show notes, not in the chapter text uploaded for audio.
...
- **`0book-compose`** ([_book_pipeline_v2.py](../../scripts/podcast/_book_pipeline_v2.py) `compose_book_v2`) — the ONE compose route, selected by the `book_augmentation` / `book_voice` knobs: a faithful base (`_translation_edition.author_translation_edition_compose`) → optional additive `0book-augment` → optional `0book-voice`, then, in code order: the Composer replay, the transliteration fold, the annotation-policy classification, the once-per-book corpus-grounded etymology step ([_etymology.py](../../scripts/podcast/_etymology.py) `build_etymology_atoms` — roots + derived families from the Quranic morphology corpus at `content/knowledge-base/quranic-corpus/`, meanings joined from the classical lexica via `lexicon.jsonl`; idempotent via `_system/etymology-report.json`), then inline Arabic (glossary script now corpus-filled deterministically where unambiguous and OCR-grounded), American spelling, and first-use honorifics → `book/book.md`. Prose is written **in the book's declared `narrative_frame`**, never automatically first person. Arabic quotations are rendered as SCRIPT with the English beneath, faithful (no abridgement, no teaching lost). A chapter carrying a Book Composer edit is NOT regenerated — see the Composer rule in `framework.md`. (`_book_compose.py` is a helper module only; its standalone whole-book composer was deleted 2026-07-21 because running it clobbered `book.md`.)
...
**Source medium (a SOURCE property, like the narrative frame).** `source_medium` in `_system/series-config.yaml` — `printed_text` (default) or `audio_lecture` — records what the book was made FROM. `al-anwaar-al-lateefah` is 65 mp3 lectures recorded in Urdu and transcribed; there is no Arabic page behind its vocabulary, so REQ-BA-127 scopes itself out and its romanizations are the honest rendering rather than a gap. Set it at intake from how the material actually arrived, never from the product. Everything else — articulation, vowelling, substitution of whatever terms the glossary CAN prove — runs unchanged.

**Narrative frame (every book, every route).** `narrative_frame` in `_system/series-config.yaml` declares who narrates — `transmitted_report` | `external_narrator` | `first_person_author` | `first_person_expository` | `participant_narrator` (registry: `_narrative_frames.NARRATIVE_FRAMES`, re-exported from `_rules`). It is a property of the SOURCE, never of the product: do NOT pick it from the deliverable or the voice knob. Set it at intake by reading how the source actually opens; an Arabic text opening `بلغنا` ("it has reached us") is `transmitted_report` and narrates in the third person. Every prose-rewriting phase both instructs and enforces it through `_narrative.py`, and `book-challenger` closes with it as Pass 3. First-person narration is legitimate ONLY when the source itself speaks that way. **The frame also governs ADDRESS, not only person** (`R-NO-LECTURE-VOICE` / REQ-BA-125, 2026-08-03): under a frame that does not address a reader the narration does not turn to the reader or direct an audience, and a lecture-derived source has those moves RECAST into exposition rather than deleted. **Address is scoped by `addresses_reader`, a separate registry property, since 2026-08-11** — keying it off grammatical person meant every first-person book was treated as a letter, which is right for *Ayyuhal Walad* and wrong for a delivered lecture whose speaker genuinely says "I". Pick `first_person_expository` for a lecture becoming a book (his "I" is kept, the room is not) and `first_person_author` only where the address to a reader IS the form. This is the second half of a frame conversion — changing only the person leaves a lecture still a lecture, which is exactly what `al-anwaar-al-lateefah` shipped before the rule existed. **And NAVIGATION** (`R-NO-NAVIGATION-APPARATUS` / REQ-BA-126): the prose does not locate itself inside the source's division scheme, because the edition has its own numbered chapters and no canopies or gates to turn to.

**Every foreign term is shown in Arabic script at least once** (`R-ARABIC-SCRIPT-SHOWN-ONCE` / REQ-BA-127). The Arabic audit and R-ARABIC-SCRIPT-RETAINED both enumerate Arabic RUNS, so a term that reached the page as `*marifah*` and nothing else produces no run and is invisible to them — `al-anwaar-al-lateefah` shipped 219 such terms reading `arabic_runs_lost: 0`. The cause was upstream: `_gloss_terms.gloss_candidates` only saw parenthetical glosses (`the gnosis (marifah)`) while the articulation pass writes the italic shape (`the *marifah* — the gnosis`), so the harvester found ONE candidate in a book that italicises 261 terms. It now reads both shapes; `bare_term_findings` is the check, `_system/gloss-coverage.json` + ship gate B7 the report, and `tests/test_gloss_terms.py` ratchets a per-book ceiling so a compose cannot reintroduce them.

**Romanization is REPLACED by script, not merely annotated** (`R-ARABIC-SCRIPT-SUBSTITUTED` / REQ-BA-128, 2026-08-03). Compose step `5a-substitute` ([_book_substitution.py](../../scripts/podcast/_book_substitution.py)) finishes Asif's 2026-08-02 rule — "zero English transliteration of Arabic terms in book.md" — which the overlay only half-implemented: it CONVERTS a bracket the author wrote and ANNOTATES a term he did not, and neither reaches `the *mawaddah*`. The Book Composer's **Replace with Arabic** button is the same code path for a chapter edited by hand. Deterministic, glossary-driven, reversible by sidecar. **Eligibility is decided by two POSITIVE conditions, not a denylist** (hardened 2026-08-03): the romanization must be a possible transliteration of its own script ([_translit_skeleton.py](../../scripts/podcast/_translit_skeleton.py) — a structural consonant-skeleton match, no corpus needed, which is what stops the English word `approach` being printed as `الْمُبَاشَرَة`), AND the term's `annotation_class` must be `teach`, because structure cannot catch a wrong word spelled like the right one — `surah` paired with `صُورَة` (*picture*), `adam` with `آدَم` (*Adam the prophet*) in a passage meaning `عَدَم`. On top of those it refuses absorbed English (Allah, imam, shaykh — REQ-BA-070), `familiar`/`silent`, names, work titles, quotations, letters-as-letters, and any term that is only PART of a longer romanized phrase. Reversibility is load-bearing: the sidecar fingerprint is stamped from the FINISHED page by apparatus step 12, an unverified record is kept rather than dropped, and `revert_ineligible` takes back terms the gate no longer allows even when a wholesale restore is impossible.

**The articulate style is the DEFAULT for Islamic books (Asif, 2026-07-31).** `_pipeline_flags._default_knobs` returns `{source_only, faithful}` for `content_profile: islamic_scholarly` — so `0book-fluency`, and with it the full **Book Articulation Standard** ([docs/standards/book-articulation.md](../../docs/standards/book-articulation.md), REQ-BA-010..160), runs on every Islamic reading edition unless the book says otherwise. The reference edition is `the-master-and-the-disciple`. The old default was `author_companion`, a re-voice with far more latitude and no written contract; with `narrative_frame` enforcing who narrates it had little left to do, and on the reference book six of nine chapters came back 92-100% identical to the faithful base. `deliverable_mode` is checked FIRST (so a declared translation edition keeps `{none, faithful}`), the profile comes from the shared `_content_profile.is_islamic_scholarly`, non-Islamic profiles keep `{source_only, author_companion}`, and an explicit knob always wins — **pin both knobs on any book whose route matters**, exactly as `the-master-and-the-disciple` and `ayyuhal-walad` do, so it cannot move if the map is re-tuned again. `book-challenger` check **BK-P8** verifies the result against the standard on every faithful-voice book.
...
The draft folder (mandatory files):
 ...
 Target ~500–1,200 words. The four-part structural skeleton (opening directive, three-part focus, pronunciation hooks, anti-noise rules) maps onto blocks 1–9 above. (The retired `notebooklm-best-practices.md` §5 this used to cite has no rediscovered replacement; blocks 1–9 above are the operative spec.)
 ...
 **Validation gates (chapter — the SOURCE the user uploads):**
 ...
 - Word count ∈ [500, 5,500] hard band (sweet spot 1,800–2,800). (The retired `notebooklm-best-practices.md` §3 this used to cite has no rediscovered replacement; the band stated here is the operative one.)
 ...
 **Post-SHIP-READY learning hook (added v2.0, 2026-05-18 — runs once after the gate clears):**
 ...
 The aggregate + propose pair NEVER modifies normative rule files. Promotion (proposal → handbook rule + `_rules.py` constant list) is human-driven and must pass `scripts/podcast/test_challenger.py --verbose` against the fixture corpus before merging. See `_learning/README.md` for the full pipeline contract.
...
The three-step sequence (fully deterministic when Azure Speech is provisioned; otherwise the transcript drop is the only human-pacing step):
 ...
 - **(b) Batch — the whole book's NotebookLM audio in one command** (preferred when auditing produced episodes): `python3 scripts/podcast/transcribe_notebooklm.py <slug>` transcribes every canonical m4a missing a transcript and writes both transcript contracts (see scripts catalog).
 - **(c) Manual fallback — any external transcription service.** Drop a `.transcript.txt` at `BOOK_DIR/transcripts/EP##-<slug>.transcript.txt` ( export, https://transcripts.ai, manual subscription). The filename contract is fixed by the `transcripts/_README.md` template — both paths write to the same place so downstream tooling is path-blind.
...
Each source type has its own distillation pattern. (`PODCAST_ROOT/.skill/handbook/source-distillation.md`, the former home of the full patterns, is retired with no rediscovered replacement — the quick reference below is what remains.) Format-to-text conversion is done once in Phase 0a. After Phase 0a, all source types flow through the same Phase 0b–0g and Phase 1–4 pipeline. Quick reference:
...
Full spec: retired — real authority is podcast-challenger.md Category F.
...
Full spec: retired — real authority is podcast-challenger.md Category P.
...
**LOOP 2 — NOTEBOOKLM OPTIMIZATION** (`notebooklm-best-practices.md`, this loop's former anchor, is retired with no rediscovered replacement — the checks below are the operative ones)
...
**LOOP 6 — PRONUNCIATION COVERAGE (architecture v3.5)**
 ...
 - **Cross-check every term's canonical phonetic against the term_render.py ladder's output** (`scripts/podcast/knowledge/term_render.py` → `content/knowledge-base/pronunciations.jsonl`'s `confirmed` rows — NOT `SHARED_ARABIC/03-arabic-english-manifest.md`, retired with no rediscovered replacement). A term with a `confirmed` ledger row must use that spelling exactly; drift is a P0 quality-gate failure.
 - For any term with no `confirmed` row, verify the phonetic was drafted per the ladder's fallback rungs (loanword allowlist → English exonym → gloss → plain transliteration) and that the term is recorded in `BOOK_DIR/_system/source/text/_phonetics.md`. (`SHARED_ARABIC/01-tts-pronunciation-key.md`, this step's former citation, is retired with no rediscovered replacement for its specific engineering rules.)
 - **Run the substitution audit** (judgment, not a lookup — `SHARED_ARABIC/04-common-term-substitutions.md` is retired with no rediscovered replacement): any Arabic term with an obvious English substitute (nafs/shaytan/ruh-style) still present in the chapter without a documented framing justification is a Loop 6 failure — either substitute the English form or add the justification to `00-framing.md`'s pronunciation block.
...
Before telling Asif a bundle is ready, silently verify:
 ...
 4. **Chapter is phonetic-clean** (R-PHONETICS-OUT, post-2026-05-17): NO inline `(PHO-ne-tic;...)` parens; NO post-transliteration phonetic blockquote lines. Every transliterated Arabic term used in the chapter has a matching imperative line in `00-framing.md`'s `## Pronunciation` block (R-PRONUNCIATION-IMPERATIVE). Phonetic spellings match the term_render.py ladder's `confirmed` ledger rows exactly (not `SHARED_ARABIC/03-arabic-english-manifest.md`, retired with no rediscovered replacement). Honorifics expanded exactly once per figure per chapter (R-HONORIFIC-ONCE). No abbreviated work titles (R-NO-ABBREVIATION).
...
**Both subsections immediately below (`SHARED_ARABIC/` and `PODCAST_ROOT/.skill/handbook/`) describe a directory tree retired in the 2026-05-23 restructure — confirmed absent 2026-08-05. None of these files can be read or extended as described.** The "intelligence-module layer" extensibility contract this section used to state (drop a new `.md` file under the handbook dir, no `SKILL.md` edit required) has no working equivalent today: extending the challenger's checks currently means editing [scripts/podcast/_rules.py](../../scripts/podcast/_rules.py) and [infra/claude-agents/podcast-challenger.md](../../infra/claude-agents/podcast-challenger.md) directly, which DOES require touching tracked files this section claimed were unnecessary to touch. Both bullet lists below are left in place as a historical record of what the tree used to contain, not as a current file index.

### In `SHARED_ARABIC/` (RETIRED — historical record only):
...
### In `PODCAST_ROOT/.skill/handbook/` (RETIRED — historical record only):
 - `registry.md` — episode index (number, title, slug, book-slug, status, NotebookLM URL)
 - `notebooklm-source-chapter-rules.md` — chapter-as-source contract (R-NOHTML, R-PHONETIC, R-NAMES, R-OPENFRAME, R-ENRICH60, etc.) — current authority: `_rules.py` + podcast-challenger.md
 - `notebooklm-customize-prompt-rules.md` — customize-prompt contract (R-WELCOME, R-NOREPEAT, R-NOBACKGROUND, R-NAMEALIAS, R-NOINTERRUPT, R-SUMMARYTAIL) — current authority: `_rules.py` + podcast-challenger.md
 - `notebooklm-source-format.md` — file-by-file format spec for NotebookLM ingestion — no rediscovered replacement
 - `notebooklm-best-practices.md` — distilled best-practices reference — no rediscovered replacement
 - `enrichment-sources.md` — Phase 0e whitelist + citation format + tradition-mix principles — current authority: `scripts/podcast/_authoring/_enrichment.py`
 - `two-host-framing.md` — host personas + steering language patterns (Deep Dive format) — current authority: podcast-challenger.md Category F
 - `debate-framing.md` — proposition + roles + positions + resolution shapes (Debate format) — current authority: podcast-challenger.md Category P
 - `source-distillation.md` — distillation pattern per source type — no rediscovered replacement
 - `episode-architecture.md` — opening hook, beat shape, landing — no rediscovered replacement
 - `scratchpad-markers.md` — podcast-local `@@` marker vocabulary — no rediscovered replacement; no code found generating or consuming it
 - `workspace-readme-template.md` — used to bootstrap a new book's `_README.md` — no rediscovered replacement
...
### Scripts (all under `<REPO_ROOT>/scripts/podcast/`):
 ...
 - `transcribe_notebooklm.py` — Azure batch transcription of NotebookLM OUTPUT audio (rebuilt 2026-06-12; v1 used local whisper, never installed — TurboScribe was the manual workaround). For a book slug, transcribes every canonical `m4a/ch<NN><s>-<slug>.m4a` missing its transcript via Azure Speech Fast Transcription and writes BOTH transcript contracts from one call: `m4a/transcripts/<ch-stem>.transcript.txt` (source of truth — postprod-review, normalize_m4a) and the derived `transcripts/EP##-<slug>.transcript.txt` copy (audit_transcript Loop M, stitch_video keyword sync). Idempotent; `--only/--force/--dry-run/--locale`; duration-priced `azure-speech-stt-fast` rows appended per file. Non-canonical filenames are skipped with a pointer to normalize_m4a.py. External transcription (TurboScribe etc.) remains a FALLBACK drop path.
 - `normalize_m4a.py` — drop-and-normalize step for NotebookLM downloads. NotebookLM names exports after the Audio Overview's creative title and any manual `NN-` prefix is an unverified CLAIM (a real 19/20 swap shipped 2026-06-12); this script fingerprints each non-canonical `.m4a`/`.txt` under `BOOK_DIR/m4a/` against the episode framings + chapter sources (transcript text > title tokens; numeric prefixes only compared, never trusted), then renames to `ch<NN><s>-<slug>.m4a` / `m4a/transcripts/ch<NN><s>-<slug>.transcript.txt`. Dry-run by default, `--apply` to execute; ambiguous or colliding files are left untouched and surfaced; every verdict (MATCH / SWAP / AMBIGUOUS / COLLISION) appends to the `m4a/_review/prefix-verification.json` ledger. **As of 2026-06-14 this is normally AUTOMATIC: the `audio-ingest` phase (`phases/audio_ingest_driver.py`, in `PHASES` between `finalize` and `0book-design`) calls `normalize_m4a` + `transcribe_notebooklm` on `--resume` after the operator drops the downloaded `.m4a`, gates on completeness (re-halts cleanly until every NotebookLM episode has audio + transcript), and is an engine-gated no-op for ElevenLabs books.** The standalone command remains for manual/debug use; the finalize-halt card + the durable `_system/notebooklm-worklist.md` name it.
 - `_audio_engines.py` — pluggable audio-engine registry (Audio Engine v2, 2026-06-12). A book declares `audio_engine: notebooklm | elevenlabs` in `_system/series-config.yaml`; missing field = `notebooklm` (every pre-existing book byte-identical — golden-fixture regression). Capability flags (`supports_arabic_script`, `supports_audio_tags`, `max_chunk_chars=2000`, `credit_rate`, `render_mode manual|api`), pinned `eleven_v3` model id, default voice casting (per-book override `elevenlabs_voices:`). Adding an engine = one registry entry; validators/cost/orchestrator read capabilities from here, never hardcode.
 - `_dialogue_script.py` + `_authoring/_dialogue.py` — the ElevenLabs path's per-chapter artifact: a COMPLETE two-host dialogue script (`_system/dialogue-scripts/EP##-<slug>.script.md`, `HOST_A:`/`HOST_B:` turn lines, sparse `[tag]` cues) authored on Claude Max with the converged framing as the authoritative steering spec. SOFT char band per length tier is a pacing target only — content completeness outranks the band (never cut a teaching; exceed and flag P2). Pure chunker (turn boundaries, <= 2,000 chars/request), chunk content hashes, hash-derived seeds.
 - `_validators_dialogue.py` + `_dialogue_convergence.py` — the pre-synthesis content gate: deterministic deny-list/doctrine/honorific/host-parity/meta-prose checks, engine-aware Arabic+tag rules (registry-driven), and a deterministic COVERAGE check (every contract tension + concept surfaced — no-teaching-lost analog, P0). Convergence loop (author -> gate -> semantic challenger on Max -> fixer, max 5 iters) with SHIP-READY / SHIP-WITH-CAUTION / FAILED verdicts; findings to `_learning/findings.jsonl` (`source=dialogue-gate`); gate report carries the EXACT credit estimate. NOTHING renders before a passing verdict.
 - `pronunciation_compiler.py` — deterministic `glossary.yml` -> ElevenLabs alias-rule PLS dictionary (phoneme rules unused — legacy-English-only). Uploads once per book, pins `{dictionary_id, version_id}` in `_system/pronunciation-dictionary.json`, recompiles to a new pinned version on glossary change (history ledger). Arabic-script recitation scaffold behind `elevenlabs_arabic_recitation` (default OFF until the H2 audible-sample review).
 - `render_dialogue_audio.py` — production ElevenLabs renderer (render once, cache forever). Refuses without a shippable gate verdict; refuses paid synthesis without approval (orchestrator H1 / `--confirm`). Renders via `_elevenlabs.py` (stdlib client: key env -> .env -> keychain, retries, subscription meter) into the SAME canonical layout as NotebookLM downloads: `m4a/ch<NN><s>-<slug>.m4a` + both transcript contracts (transcript IS the speaker-labeled script — no STT). Render ledger `_system/render-ledger.jsonl` (input-hash -> output-hash) + chunk cache: unchanged chunks never re-render/re-spend. Exact metered credits -> `elevenlabs-eleven-v3` cost rows. Free chars-per-second sanity band; `--deep-verify` optional Azure STT containment report. Orchestrator phases `audio-script` + `audio-render` (phases/audio_driver.py) sit between per-chapter-slides and finalize; notebooklm books skip both.
 - `_quran_recitation.py` — deterministic Quran citation -> verbatim KQur Arabic. A static canonical surah-name table + exact number parser map a citation ("the chapter of Abraham, verse seven" / "(chapter 14, verse 7)") to (surah, ayat); `quran_ayat_lookup` (mirror.db `fts_quran`, read-only) returns the verbatim Arabic. Unresolved/absent verse -> nothing (logged, left in English) — scripture is NEVER model-generated. `inject_recitations(text)` splices the Arabic after each resolvable citation. Consumed by `pronunciation_compiler.compile_turns_for_render` (ElevenLabs render layer only, flag `elevenlabs_arabic_recitation`).
 - `_audio_fingerprint.py` — measurable speaking-style fingerprint (8 metrics: wpm, switches/min, turn length, host balance, pause rate+length, per-host pitch variance) + weighted similarity score vs a per-genre gold standard. `score_against_profile(fp, profile)` loads `content/_shared/audio-style/<profile>.json` (islamic_scholarly fallback). Pure local DSP (ffmpeg decode), no spend. Tracked promotion of the style-match experiment.
 - `build_audio_gold_standard.py` — builds `content/_shared/audio-style/<profile>.json` from the FULL m4a corpus of a profile's NotebookLM-native books (median center + corpus-spread tolerance per metric, word counts from sibling transcripts). Re-run when episodes are added; the JSON diff is the review artifact. `--profile islamic_scholarly [--books ...] [--dry-run]`.
 - `build_voice_samples.py` — renders one short sample clip per approved voice into `plan-dashboard/public/voice-samples/<name>.mp3` (the intake voice-picker "Hear" links). ElevenLabs spend (~one line per voice, tiny); requires `--confirm`, reports metered credits, skips existing clips unless `--force`.
 ...
 - `_rules.py` — canonical rule data (DENY lists, abbreviation map, honorific patterns) imported by the build and audit scripts. Was mirrored FROM `.skill/handbook/notebooklm-customize-prompt-rules.md` before that file's 2026-05-23 retirement — `_rules.py` is now the source itself, nothing to mirror from. Also home of `CHALLENGER_VERSION` (single-source version stamped into every challenger report and ledger record) and `emit_finding()` (the JSONL-ledger writer used by both the challenger agent and `audit_transcript.py`).
...
- `stage_runner.py` — CLI driver for the stage pipeline. Runs the next pending
  ...
  — never hardcode a content path like `content/Islamic/<slug>`.**
