...
**INVARIANT 6: Every chapter has a unique, concise title — chapter count and size are content-depth-driven.** Each `BOOK_DIR/chapters/chNN-<slug>.txt` has its own working title (kept in the matching `chapter-contracts/<slug>.yml` `title:` field and mirrored in the book's `_system/registry.md`). Titles MUST be:
  ...
  - **Descriptive of the chapter's actual content**, not generic ("Chapter Two", "Introduction continued"), not author-name-only ("Ghazali on …"). The title is the listener's first cue about the episode.
...
Chapter count and per-chapter size are determined by the source's content depth against NotebookLM Audio Overview length targets (per `notebooklm-best-practices.md` §3) — **never by arbitrary numbering or by mechanically following the translator's section structure**. Length bands map to target episode shape:
  ...
  - **Default Deep Dive** (~12–15 min audio): 1,800–2,800 words per chapter. This is the recommended default.
  - **Longer Deep Dive** (~18–22 min audio): 2,800–4,500 words per chapter. Use only when the content's argument cannot be honestly split without breaking the thematic unit.
A 30-page epistle that decomposes into 5 strong thematic units yields 5 chapters; a 200-page treatise might yield 8–14 depending on argument density. **Never inflate a thin theme to hit a word target, and never compress two distinct themes to hit a chapter count.**
...
If a new book is being added (PATHS ARE PER-BOOK — never share workspaces across books):
  - Create `library/<category>/<book-slug>/_system/`, `library/<category>/<book-slug>/chapters/`, `library/<category>/<book-slug>/episodes/`, `library/<category>/<book-slug>/turboscribe/`
  ...
  - Create `library/<category>/<book-slug>/turboscribe/_README.md` from the template below
...
| Longer Deep Dive | ~18–22 min | 2,800–4,500 |
  ...
  - **Hard ceiling: 4,500 words** per chapter (`build_episode_txt.py` enforces 500–5,500; the design target stays inside the soft band).
  ...
  - **Content-depth-driven, not count-driven.** Decide chapter count from where the author's argument actually turns, then check the band. If a thematic unit only carries 800 honest words, either merge with an adjacent unit or accept the Brief shape — never inflate to hit Default.
...
Segmentation method:
  ...
  3. Group adjacent thematic units into chapters until each one falls in the chosen band (Brief / Default / Longer). Split units that exceed the ceiling.
  4. Chapter count emerges from the segmentation: a 30-page epistle with 5 clear turns yields 5 chapters; a 200-page treatise might yield 8–14 depending on argument density. There is no fixed minimum or maximum.
  ...
  8. Write `BOOK_DIR/_system/source/text/chapters-rationale.md` explaining the segmentation: which source sections fed which chapter, why two were merged, why one was split, and which length band each chapter targets.
