...
**INVARIANT 5: All Arabic gets phonetics.** Phase 0c ensures every Arabic word, transliteration, Quranic verse rendering, hadith line, dua, name, and honorific carries an inline phonetic guide on first appearance per chapter. No Arabic script or transliteration ships without pronunciation support.

These invariants are what keep the structure honest: episodes are derivative artifacts of enriched, designed chapters — never freestanding NotebookLM bundles, never thin slices of an OCR'd translator's PDF.
...
The book-slug is derived from the source title (kebab-case, ≤ 40 chars). This folder is the persistent home for the cleaned source text — it is NOT cleaned up after delivery.

Files produced in this phase:

  - `raw-extract.md` — verbatim text extraction. For PDFs with a text layer, use the text layer; for image-only PDFs, run OCR (Tesseract via `ocrmypdf` or equivalent). Page breaks marked with `<!-- page N -->`. **One file**, full document. Do not split yet — segmentation comes in Phase 0d.
  ...
  - `_extraction-notes.md` — anything uncertain: low-confidence OCR spans, illegible passages, footnote vs body text ambiguity, translator brackets. This file is the audit trail; nothing gets silently smoothed.
...
Output: phonetic annotations are written **back into `normalized.md`** (single file remains the working surface) and a separate `BOOK_DIR/_system/source/text/_phonetics.md` index records every term with its agreed phonetic form for reuse.
...
Sizing rules (per `PODCAST_ROOT/_system/notebooklm-best-practices.md` §3):
...
Allowed enrichment sources, citation format, and tradition-mix principles are codified in the canonical whitelist: **`PODCAST_ROOT/_system/enrichment-sources.md`**. Consult it before adding any enrichment. The whitelist is tiered:
...
### PHASE 0g: REGISTER THE SERIES
  ...
  1. Reserve a contiguous block of episode numbers in `PODCAST_ROOT/_system/registry.md` — one row per chapter (and therefore per episode), status `draft`, all pointing to the same book-slug.
...
The draft folder (mandatory files):
  ...
  - `00-framing.md` — the NotebookLM "Customize" prompt: audience, angle, central tensions to reach, host dynamic, tone constraints, pronunciation hooks, permission to disagree. This is what Asif copies into NotebookLM's *Customize* prompt box. Target ~500–1,000 words (per `PODCAST_ROOT/_system/notebooklm-best-practices.md` §5; the four-part structure — opening directive, three-part focus, pronunciation hooks, anti-noise rules — must all be present).
...
Each source type has its own distillation pattern. The full patterns live in `PODCAST_ROOT/_system/source-distillation.md`. Quick reference:
  ...
  - **Multi-source synthesis** — 2–4 sources put in conversation. The synthesis lives as a single composed chapter file under `BOOK_DIR/chapters/` with each source's contribution clearly attributed. `04-discussion-spine.md` carries the synthesis lens for the episode framing.
...
Before telling Asif a bundle is ready, silently verify:
  ...
  8. Does `00-framing.md` contain the four-part structure (opening directive, three-part focus, pronunciation hooks, anti-noise rules)?
