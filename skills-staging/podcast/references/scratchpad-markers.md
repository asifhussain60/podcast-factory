...
**Authoritative spec for the `@@` marker vocabulary within the podcast skill.** Podcast owns this copy. The journal skill maintains a parallel copy at `journal/reference/scratchpad-markers.md` for memoir chapters. The two copies are independent; changes to one do not require changes to the other.

---

## The mental model

A **canonical file** is the finished artifact — a refined podcast episode at `podcast/episodes/EP##-<slug>/01-source-primary.md`. The canonical file is what NotebookLM ingests.
...
Lifted into `podcast/series-policies.md` and applied as augmentation to every future refinement pass. Each entry has provenance (which scratchpad, which date), the directive, and an active/inactive flag.
...
|---|---|---|
| podcast | `podcast/episodes/EP01-ayyuhal-walad-ch1/01-source-primary.md` | `podcast/_workspace/EP01-ayyuhal-walad-ch1/01-source-primary.scratch.md` |
...
# Mirror of: podcast/episodes/EP##-<slug>/01-source-primary.md
...
- **Pronunciation entries that apply to the whole series permanently** — edit `podcast/03-pronunciation.md` directly. Use `@@pronounce` only for a per-run override.
...
- `reference/quotes-library.txt` — propose a quote entry if the episode surfaces a passage likely to inform memoir work.
- `reference/clinic-library.txt` — propose a craft observation if the episode's source material contains a technique relevant to memoir craft.

**How to propose:** Write the proposed entry to `podcast/_workspace/EP##-<slug>/proposed-library-entries.md`. Do NOT write directly to `reference/quotes-library.txt` or `reference/clinic-library.txt`. The journal skill owns those files; Asif routes the proposal himself.

The podcast skill reads nothing from `reference/` and reads nothing from `chapters/`. Proposals flow outward only.
