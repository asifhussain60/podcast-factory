...
- **NotebookLM (manual, default).** Unchanged: finalize halt prints the upload table; the human uploads chapter + framing, downloads `.m4a`, then `normalize_m4a.py` + `transcribe_notebooklm.py`.
...
**Dual-path for Islamic content (2026-06-13).** ElevenLabs is now the per-profile NEW-book default for `islamic_scholarly` (cast Eric/Lily), stamped at intake by [scripts/podcast/intake_launch.py](scripts/podcast/intake_launch.py) from the registry in `_rules.py` — never applied retroactively, so the existing NotebookLM-native books (no `audio_engine` field) stay on NotebookLM. NotebookLM remains available **per-episode on demand** via `episode_engine_overrides: {EP##-slug: notebooklm}` in series-config: `engine_for_episode()` ([scripts/podcast/_audio_engines.py](scripts/podcast/_audio_engines.py)) routes each episode, the script + render phases skip overridden episodes (no wasted authorship, no credit estimate, never API-rendered), and the finalize halt prints BOTH the "already rendered" note (API episodes) and the NotebookLM upload table filtered to the overridden episodes. When no overrides exist the upload table is unfiltered — byte-identical to before (golden-test latch).
