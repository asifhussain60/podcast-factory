...
- **Book Composer = the singular chapter-edit path.** Human chapter edits destined for the PDF go through `/studio/<slug>/compose` only. Its save route writes `book/book.md` AND the durable sidecar `_system/composer-edits.json`; [scripts/podcast/_book_edits.py](scripts/podcast/_book_edits.py) `apply_composer_edits` replays that sidecar as the final text step of `compose_book_v2` (step 5b, after voice + seam dedupe, before the Arabic audit). Without the replay a Composer edit lasted only until the next compose regenerated the layers — the reason `book.md` alone was never a durable surface. Idempotent, heading-anchored, conflict- and orphan-reporting into `_system/composer-edits-replay.json`. The `chapters/*.txt` studio routes (`denoise`, `replace`, `save-stage`) belong to the NotebookLM lane and do NOT feed the PDF.
