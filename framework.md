...
- **Augmented PDF route — the companion book.** Phases `0book-design → 0book-compose → 0book-illustrate → 0book-slide-import → 0book-render` (gated by `series.enable_book_branch`, NON-blocking on the podcast ship, run post-finalize inside the publish driver so the book is built from review-approved content) re-segment the source into a book-craft chapter structure, revoice it into modern author-first-person prose with Arabic scripture (script + English beneath) and plain transliteration ([scripts/podcast/_translit.py](scripts/podcast/_translit.py) ↔ [plan-dashboard/src/lib/translit.ts](plan-dashboard/src/lib/translit.ts)), inject teaching diagrams, weave NotebookLM-exported slide decks ([scripts/podcast/_slide_import.py](scripts/podcast/_slide_import.py): per-chapter deck PDFs dropped at `slide-decks/chNN-<slug>.pdf` per the finalize-halt generation card → LLM-authored slide→anchor manifests, sig-cached → every slide placed BEFORE the passage it explains in `book/book-slides.md`; missing drops HALT before publish, `.SKIP` exempts), and render `book/book.pdf` (Playwright; priority book-slides.md > book-illustrated.md > book.md) + the in-site reader view (`/studio/<slug>/book`). Gated by `book-challenger`. The revoice — formerly the retired per-chapter `0literary` step — lives here and NEVER feeds NotebookLM.
- **Articulated translation route — the faithful translation PDF.** A book with `_system/series-config.yaml` `deliverable_mode: translation_edition` uses [generate_translation_edition.py](scripts/podcast/generate_translation_edition.py) or the book driver to run `0a ingest → 0b refine → 0book-design → 0book-compose → 0book-illustrate → book-level slide pair → 0book-render`, while skipping podcast phonetics, gap analysis, enrichment, and audio. Its contract lives in [scripts/podcast/_translation_edition.py](scripts/podcast/_translation_edition.py): `translation_policy.augmentation` must forbid outside-source augmentation; Arabic terms and original-language text are preserved from OCR/refined source; salutation prose is compacted to `(عليهم السلام)`, `(ع)`, and `(رض)`; diagrams/slides are monochrome; and `book/source-crosswalk.json` persists each chapter's source line/page ranges, source headings/excerpt, Arabic page range, and title/source drift findings. This route is not the augmented companion-book route: it articulates and denoises the provided source only, without importing doctrine, modern examples, or external research.

- **Unified book path (Book Pipeline v2 — behind `book_pipeline_v2`, default OFF).** The two routes above are being collapsed into ONE path selected by two orthogonal knobs in `_system/series-config.yaml`: `book_augmentation: none | source_only` and `book_voice: faithful | author_companion` ([scripts/podcast/_pipeline_flags.py](scripts/podcast/_pipeline_flags.py)). `0book-compose` then runs a faithful base (the translation-edition composer, reused with `enforce_contract=False`) → optional additive `0book-augment` ([scripts/podcast/_book_augment.py](scripts/podcast/_book_augment.py): source-grounded, clearly-labeled + doctrinally-gated editorial asides, non-destructive to the base) → optional `0book-voice` ([scripts/podcast/_book_voice.py](scripts/podcast/_book_voice.py): author-companion re-voice, reverted per-chapter on any teaching/Arabic/doctrinal drift), orchestrated by [scripts/podcast/_book_pipeline_v2.py](scripts/podcast/_book_pipeline_v2.py). The knob-default map reproduces current routing (`deliverable_mode: translation_edition → {none, faithful}`; legacy companion → `{source_only, author_companion}`). With the flag OFF the legacy dispatch above runs unchanged; `generate_translation_edition.py` is retired behind the flag (removed once v2 is default).
...
| `book-render-challenger` | [infra/claude-agents/book-render-challenger.md](infra/claude-agents/book-render-challenger.md) | Print-render challenger for the Book Pipeline v2 reading edition — gates the RENDERED `book/book.pdf` against [docs/standards/book-print-quality.md](docs/standards/book-print-quality.md) (REQ-BR-*) via the deterministic probes in [scripts/podcast/_book_render_checks.py](scripts/podcast/_book_render_checks.py). Runs only under `book_pipeline_v2`, wired into 0book-render, NON-blocking. (Added Wave: book-pipeline-v2 merge 2026-07-15.) |
