...
_Theme-parity sustainability (expanded 2026-04-17)_: the token-driven system is guarded by `server/scripts/validate-theme-parity.mjs` (runnable via `npm run validate-themes`), which enforces **8 checks** — token parity across themes, no hex literals in enforced component CSS (auto-discovers all `site/css/*.css` except `floating-chat.css` + `base.css`), no palette rgba in enforced component CSS, every `var(--token)` reference resolves (aggregated from all CSS + auto-discovered dynamic tokens from JS/JSX inline style patterns), zero `<style>` blocks / inline color styles / JSX `style={{…}}` literals in HTML (recursive scan of `site/**/*.html` + `trips/**/*.html` filtered to theme-linked files), switcher-file consistency, font parity (every `--font-*` primary family loaded in Google Fonts `<link>`), and switcher-swatch parity. The `skills-staging/css-theme-sync/` skill wraps the validator with authoritative hex→token + rgba→color-mix mapping and `new-theme` / `propagate-token` sub-commands. A pre-commit hook at `.git/hooks/pre-commit` (installed from `scripts/git-hooks/pre-commit` via `scripts/install-git-hooks.sh`) blocks commits with violations. Invoke after any edit to `site/css/` or when adding a new theme/view.

_In-app theme editor (added 2026-04-17)_: `site/js/tweaker/tweaker.jsx` + `site/css/tweaker.css` ship an element-picker-based theme editor, activated via `?tweak=1` URL flag or persistent `localStorage.journal:tweaker:mode=on`. Hidden from normal readers. Features: hover-highlight element targeting, click-to-select with floating-ui anchored inspector, token-aware editing (global token mutation OR selector-scoped override per edit), AI swatch suggestions per-color (`POST /api/theme-swatches`, Haiku), AI holistic pre-save review (`POST /api/theme-review`, Sonnet), live contrast meter, undo/redo with localStorage session persistence, save to current theme OR new theme (`POST /api/theme-save`, validator-gated). Libraries: react-colorful + @floating-ui/dom + css-selector-generator, all lazy-loaded via esm.sh on activation. Endpoints in `server/src/routes/theme.js`; schema at `server/src/schemas/theme-save.schema.json`.
