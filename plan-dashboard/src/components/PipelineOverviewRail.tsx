...
const kindClass = (kind: string) =>
  `k-${kind in KIND_COLOR ? kind : "mechanical"}`;
    ...
    <div className="por-wrap">
      ...
      <div className="por-svg-frame">
        ...
        >
          ...
          <desc id="pipeline-overview-desc">
            A horizontal rail diagram showing each pipeline phase, how phases
            connect, and which external services they call.
          </desc>
          <defs>
            {/* Arrow marker */}
            <marker
            ...
            {phases.slice(0, -1).map((p, i) => {
              ...
              return (
                ...
                >
                  <stop
                    offset="0%"
                    stopColor={KIND_COLOR[p.kind] || "#87827a"}
                  />
                  <stop
                    offset="100%"
                    stopColor={KIND_COLOR[next.kind] || "#87827a"}
                  />
                </linearGradient>
              );
            })}
          ...
          {phases.slice(0, -1).map((p, i) => {
            ...
            return (
              <line
                ...
                strokeWidth={
                  selectedId
                    ? selectedId === p.id || selectedId === phases[i + 1].id
                      ? 2.5
                      : 0.5
                    : 1.8
                }
          ...
          {tooltip && (
            ...
            >
              <div className="por-tooltip">
                ...
                <span
                  className={`por-tooltip-kind ${kindClass(tooltip.phase.kind)}`}
                >
                  {KIND_LABEL[tooltip.phase.kind]}
                  {tooltip.phase.agent ? ` · ${tooltip.phase.agent}` : ""}
                </span>
                <p>
                  {(tooltip.phase.plain || "").slice(0, 100)}
                  {(tooltip.phase.plain || "").length > 100 ? "…" : ""}
                </p>
              </div>
            </foreignObject>
          )}
          ...
          return (
            <div className={`por-selected-strip ${kindClass(p.kind)}`}>
              <div className="por-strip-header">
                ...
                {p.duration_minutes && (
                  <span className="por-strip-dur">
                    ~{p.duration_minutes} min
                  </span>
                )}
                <button
                  className="por-strip-close"
              ...
              {p.modules && p.modules.length > 0 && (
                <div className="por-strip-modules">
                  {p.modules.map((m) => (
                    <span key={m} className="por-module-chip">
                      {m}
                    </span>
                  ))}
                </div>
