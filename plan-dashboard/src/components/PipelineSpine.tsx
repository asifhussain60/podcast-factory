...
const AZURE_MODULE_CHIPS: Record<string, Omit<SvcChip, "id">> = {
  "azure-document-intelligence": {
    ...
    detail:
      "Reads every page of the source PDF using Azure AI Document Intelligence — recognizes Arabic script, headings, tables, and footnotes with full layout awareness.",
  ...
  "azure-translator": {
    ...
    detail:
      "Translates recognized Arabic text to an English working copy using Azure AI Translator. The original Arabic is preserved alongside it for reference.",
  ...
  P10: [
    {
      ...
      detail:
        "The finished episode bundle is uploaded to Google NotebookLM. Its Audio Overview feature generates the two-host podcast conversation from the source material.",
...
function vendorTagsForChips(
  chips: SvcChip[],
): Array<{ vendor: VendorKey; wordmark: string }> {
  ...
  return (
    <div className={`svc-chip vendor-${chip.vendor} ${open ? "is-open" : ""}`}>
      ...
      >
        ...
        <i
          className={`fa-solid fa-chevron-${open ? "up" : "down"} svc-chip-caret`}
          aria-hidden="true"
        />
      </button>
      {open && (
        <div className="svc-chip-popover" role="tooltip">
          <div className="svc-chip-pop-head">
            <i
              className={`fa-solid fa-${chip.icon} svc-chip-icon ${vendorClass}`}
              aria-hidden="true"
            />
            <div>
              <strong className="svc-chip-pop-title">{chip.label}</strong>
              <span className="svc-chip-pop-wordmark">
                {VENDOR_META[chip.vendor].wordmark}
              </span>
            </div>
          </div>
          {chip.sublabel && (
          ...
          {chip.resource && (
            <div className="svc-chip-pop-row">
              <span className="svc-chip-pop-key">
                {chip.vendor === "azure"
                  ? "Resource"
                  : chip.vendor === "anthropic"
                    ? "Model"
                    : "Product"}
              </span>
    ...
    <div className="pipeline-shell">
        ...
        <div className="pipeline-rail-sticky">
            ...
            {phases.map((p, i) => {
              ...
              return (
                <li
                  key={p.id}
                  ref={(el) => {
                    railItemRefs.current[p.id] = el;
                  }}
                  className={`rail-item kind-${p.kind} ${isActive ? "is-active" : ""}`}
                >
                  <button
                    type="button"
                    onClick={() => jumpTo(p.id)}
                  ...
                  >
                    ...
                    <span className="rail-text">
                      <span className="rail-meta">
                        Step {i + 1} · {p.duration_minutes} min
                      </span>
          ...
          <div className="rail-foot">
            <span className="rail-foot-line">
              Scroll the right column to walk through the pipeline. The active
              step lights up here.
            </span>
          </div>
        </div>
      ...
      <main className="pipeline-stations">
        {phases.map((p, i) => {
          ...
          const phaseModules = p.modules
            .map((m) => moduleMap.get(m))
            ...
            >
                ...
                <div className="station-section-headtext">
                  <div className="row station-section-eyebrow-row">
                    <span className="eyebrow">
                      Station {p.id} · {p.duration_minutes} min
                    </span>
                    <div className="vendor-tags">
                      {vendorTagsForChips(chips).map(({ vendor, wordmark }) => (
                        <span
                          key={vendor}
                          className={`vendor-tag vendor-tag-${vendor}`}
                        >
                          <span
                            className={`vendor-tag-dot vendor-tag-dot--${vendor}`}
                            aria-hidden="true"
                          />
                          <i
                            className={`fa-brands fa-${vendor === "anthropic" ? "aws" : vendor === "azure" ? "microsoft" : "google"} vendor-tag-icon vendor-tag-icon--${vendor}`}
                            aria-hidden="true"
                          />
                          {wordmark}
                        </span>
                      ...
                      {chips.length === 0 && (
                        <span className="vendor-tag vendor-tag-internal">
                          <span
                            className="vendor-tag-dot vendor-tag-dot--muted"
                            aria-hidden="true"
                          />
                          Python script
                        </span>
                    ...
                    <div className="svc-chips-row">
                      {chips.map((chip) => (
                        <ServiceChip
                          ...
                          onToggle={() =>
                            setOpenChip(openChip === chip.id ? null : chip.id)
                          }
                        />
                      ))}
                    </div>
              ...
              {agent && (
                <div className="station-section-agent">
                  <span className="eyebrow section-sub-eyebrow">
                    {p.kind === "hybrid"
                      ? "Claude + Azure runs this step"
                      : "Claude runs this step"}
                  </span>
              ...
              {!agent && p.kind === "mechanical" && (
                <div className="focus-empty">
                  <i
                    className="fa-solid fa-gear focus-empty-icon"
                    aria-hidden="true"
                  ></i>
                  <div>
                    <strong>Python runs this step.</strong>
                    <p className="small muted">
                      A deterministic script does this work the same way for
                      every book. Nothing to reason about, nothing to drift.
                    </p>
                  </div>
                </div>
              )}
