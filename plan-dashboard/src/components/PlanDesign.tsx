...
import {
  STATUS_PILL as STATUS_BADGE,
  STATUS_LABEL,
  STATUS_HEADER,
  STATUS_DOT,
} from "../lib/plan/status-badges";
      ...
      <div key={s.id} className={`step-row${isExp ? " is-expanded" : ""}`}>
            ...
            <div className="step-hover-card" role="tooltip" aria-hidden="true">
              {needs.length > 0 && (
                <div className="hover-section">
                  ...
                  <div className="hover-chips">
                    {needs.map((dep) => (
                      <span key={dep.id} className="hover-chip">
                        {dep.title}
                      </span>
                    ))}
                  </div>
              ...
              {unlocks.length > 0 && (
                <div className="hover-section">
                  ...
                  <div className="hover-chips">
                    {unlocks.map((u) => (
                      <span key={u.id} className="hover-chip">
                        {u.title}
                      </span>
                    ))}
                  </div>
                ...
                <div className="hover-section">
                  ...
                  <div className="hover-chips">
                    {s.tools.map((t) => (
                      <span
                        key={t}
                        className={`hover-chip tool-chip tool-${t.toLowerCase()}`}
                      >
                        {t}
                      </span>
                    ))}
        ...
        >
          <span className="step-id-cell">
            ...
            <span
              className={`tier-badge tier-${s.tier?.toLowerCase() ?? "t1"}`}
            >
              {s.tier}
            </span>
          </span>
          ...
          <span
            className={`step-badge ${STATUS_BADGE[s.status] ?? "is-future"}`}
          >
            {STATUS_LABEL[s.status] ?? s.status}
          </span>
          <span className="small muted step-touched">
            {s.last_touched ?? "—"}
          </span>
        ...
        {isExp && (
          <div className="step-detail">
            <p className="step-detail-what">
              {s.plain ?? "No narrative text yet."}
            </p>
            ...
            <div className="step-detail-meta">
              {needs.length > 0 && (
                <div className="detail-row">
                  ...
                  <div className="detail-chips">
                    {needs.map((dep) => (
                      <span key={dep.id} className="detail-chip">
                        {dep.title}
                      </span>
                    ))}
                  </div>
              ...
              {unlocks.length > 0 && (
                <div className="detail-row">
                  ...
                  <div className="detail-chips">
                    {unlocks.map((u) => (
                      <span key={u.id} className="detail-chip">
                        {u.title}
                      </span>
                    ))}
                  </div>
                ...
                <div className="detail-row">
                  ...
                  <div className="detail-chips">
                    {s.tools.map((t) => (
                      <span
                        key={t}
                        className={`detail-chip tool-chip tool-${t.toLowerCase()}`}
                      >
                        {t}
                      </span>
                    ))}
        ...
        <div className="wave-events-card">
          <div className="row-between">
            ...
            <span className="small muted">
              Iteration {loopState.iteration_count}
            </span>
          </div>
          <div className="stack-tight">
            <div className="wave-event-row">
              <span className={`wave-event-pill s-${loopState.current_status}`}>
                {loopState.current_status}
              </span>
              <span className="wave-event-wave">{loopState.current_wave}</span>
              <span className="wave-event-msg">
                Intent check: {loopState.intent_check_result}
              </span>
            </div>
            ...
            <div className="wave-event-row">
              ...
              <span className="wave-event-msg">
                Applied: {loopState.pattern_tally.applied_this_run} ·
                Discovered: {loopState.pattern_tally.discovered_this_run} ·
                Promoted: {loopState.pattern_tally.promoted_this_run}
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="wave-events-list">
            {waveEvents.map((evt, idx) => (
              <div key={`${evt.ts}-${idx}`} className="wave-event-row">
                <span className={`wave-event-pill s-${evt.status}`}>
                  {evt.status}
                </span>
                <span className="wave-event-time">
                  {evt.ts.replace("T", " ").replace("Z", " UTC")}
                </span>
                <span className="wave-event-wave">
                  Wave {evt.wave_letter ?? evt.wave}
                </span>
      ...
      {waves.flatMap((w) => {
        ...
        if (mobilizationSteps.length > 0) {
          items.push(
            <div
              key={`prewave-${w.id}`}
              className="prewave-band"
              id={`prewave-${w.id}`}
            >
              <div className="row-between">
                <h3>
                  {planVisualManifest.preWaveTerm} · Before Wave {w.id}
                </h3>
                <span className="small muted">
                  {
                    mobilizationSteps.filter((s) => s.status === "complete")
                      .length
                  }{" "}
                  of {mobilizationSteps.length} done
                </span>
              </div>
              <p className="small muted">
                Pre-wave setup that must be in place before core wave execution
                starts.
              </p>
        ...
        items.push(
          <div
            key={w.id}
            className={`wave-band${allComplete ? " is-collapsed" : ""}`}
            id={`wave-${w.id}`}
          >
            <div className="row-between wave-band-header">
              <h3>
                Wave {w.id} — {w.name}
                {allComplete && (
                  <span className="wave-done-stamp" aria-label="wave complete">
                    DONE
                  </span>
                )}
              </h3>
              <span className="small muted">
                {done} of {waveSteps.length} done · {flight} in flight
              </span>
            </div>
            ...
            {isExpanded && (
              <>
                ...
                <p className="small muted">
                  Wave-end cleanup and direct merge to develop are mandatory
                  closeout gates.
                </p>
