...
export default function BookPalette({ books, currentSlug }: { books: Book[]; currentSlug?: string }) {
  ...
  return (
    <>
      <button type="button" className="studio-bookpick-trigger" onClick={() => setOpen(true)} aria-haspopup="dialog">
        <span className="studio-bookpick-eyebrow">Book</span>
        <span className="studio-bookpick-current">{current?.title ?? 'Pick a book'}</span>
        <kbd className="studio-bookpick-kbd">⌘K</kbd>
      </button>

      <Command.Dialog open={open} onOpenChange={setOpen} label="Jump to a book" className="book-palette">
        <Command.Input placeholder="Search books…" className="book-palette-input" />
        <Command.List className="book-palette-list">
          <Command.Empty className="book-palette-empty">No books found.</Command.Empty>
          {books.map((b) => (
            <Command.Item
              key={b.slug}
            ...
            >
              ...
              {b.slug === currentSlug && <span className="book-palette-tag">current</span>}
            </Command.Item>
          ))}
          <Command.Item value="new content create book" onSelect={() => go('/studio/new')} className="book-palette-item book-palette-new">
