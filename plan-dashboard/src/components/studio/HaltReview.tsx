...
export default function HaltReview({ slug, haltId, artifacts, saved }: Props) {
  const [decision, setDecision] = useState<"approved" | "changes">(
  ...
  return (
    <div className="halt-review">
      <div className="halt-review-artifacts">
        <span className="halt-review-artifacts-label">
          ...
          {missing.map((a) => (
            <li key={a.label}>
              <span className="halt-review-artifact is-missing">
                <i className="fa-regular fa-file" aria-hidden="true"></i>{" "}
                {a.label}
                <span className="halt-review-artifact-hint">
                  not produced yet
                </span>
              </span>
            </li>
      ...
      <fieldset className="halt-review-decision">
        ...
        <label
          className={`halt-review-opt${decision === "approved" ? " is-on" : ""}`}
        >
          <input
            type="radio"
            name="halt-decision"
          ...
          />
          <span>
            <strong>Approve &amp; continue</strong> — the pipeline may proceed
            past this gate.
          </span>
        </label>
        <label
          className={`halt-review-opt${decision === "changes" ? " is-on" : ""}`}
        >
          <input
            type="radio"
            name="halt-decision"
          ...
          />
          <span>
            <strong>Request changes</strong> — note what needs correcting before
            continuing.
          </span>
        </label>
        <textarea
          ...
          placeholder={
            decision === "changes"
              ? "What should change before this gate is cleared?"
      ...
      <div className="halt-review-actions">
        ...
        {savedAt && status !== "error" && (
          <span className="halt-review-saved-at">
