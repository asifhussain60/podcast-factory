...
interface Props {
  ...
  onChange?: (patch: { audio_engine: string; voice_cast_host_a: string; voice_cast_host_b: string }) => void;
}
...
export default function VoicePicker({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!alive) return;
        if (!r.ok || !json.ok) { setError(json.error ?? 'Could not load voices'); return; }
        const p: Pools = json.data;
        setPools(p);
        const a = p.males.find((m) => m.name === defaultHostA)?.name ?? p.males[0]?.name ?? '';
        const b = p.females.find((f) => f.name === defaultHostB)?.name ?? p.females[0]?.name ?? '';
        setHostA(a);
        setHostB(b);
      } catch (e) {
  ...
  useEffect(() => {
    onChange?.({ audio_engine: engine, voice_cast_host_a: hostA, voice_cast_host_b: hostB });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [engine, hostA, hostB]);
  ...
  function playSample(entry: VoiceEntry) {
    ...
    if (playing === entry.name) { setPlaying(null); return; }
  ...
  function renderColumn(role: 'A' | 'B', heading: string, sub: string, entries: VoiceEntry[]) {
          ...
          {entries.map((v) => {
            ...
            return (
              <li key={v.voiceId || v.name}>
                <div className={`voice-card${isSel ? ' voice-card--selected' : ''}`}>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <span className="voice-meta">
                      ...
                      <span className="voice-accent">{accentLabel(v.accent)}</span>
                    </span>
                  </button>
                  {v.sample && (
                    <button
  ...
  return (
    <div className="intake-card">
      ...
      <p className="intake-hint">
        ElevenLabs renders the episodes automatically in the voices you pick here. NotebookLM is the
      ...
      <div className="intake-field" role="group" aria-label="Audio engine">
        ...
        <div className="voice-engine">
          {[
            { id: 'elevenlabs', label: 'ElevenLabs', sub: 'Default · auto-rendered' },
      ...
      {engine === 'elevenlabs' && (
        pools ? (
          <div className="voice-grid">
            {renderColumn('A', 'Scholar voice', 'Male · leads the teaching', pools.males)}
            {renderColumn('B', 'Seeker voice', 'Female · asks the questions', pools.females)}
          </div>
        ) : (
          !error && <p className="intake-hint" role="status">Loading voices…</p>
        )
      )}
    </div>
  );
