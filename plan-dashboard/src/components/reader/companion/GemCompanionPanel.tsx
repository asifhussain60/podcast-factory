...
export default function GemCompanionPanel({ slug, bookTitle, proseSelector = '.bookv-body' }: Props) {
  ...
  function useSelection() {
    ...
    const para = anchorNode instanceof Node
      ? (anchorNode.parentElement?.closest('p, li, blockquote') ?? null)
      : null;
    setInput(text);
    setContext((para?.textContent || '').replace(/\s+/g, ' ').trim().slice(0, 600));
    setHint(null);
  }
  ...
  async function submit() {
    ...
    if (!value) {
      setHint(mode === 'explain' ? 'Enter or select a concept to explain.' : 'Enter a question to ask.');
      return;
    }
    ...
    const endpoint = mode === 'explain' ? '/api/ai/gem-explain' : '/api/ai/gem-answer';
    ...
    try {
      ...
      setResult({ body: String(data.body ?? ''), etymology: data.etymology ?? null });
    } catch (e) {
  ...
  if (!open) {
    return (
      <button type="button" className="gcp-launcher" ref={launcherRef} onClick={() => setOpen(true)}>
  ...
  return (
    <aside
      className="gcp" role="complementary" aria-label="AI Companion — Ismaili Scholar"
    ...
    >
      <div className="gcp-head">
        ...
        <button type="button" className="gcp-close" aria-label="Close Companion" onClick={() => setOpen(false)}>
      ...
      <div className="gcp-modes" role="group" aria-label="Companion mode">
        <button
          type="button" aria-pressed={mode === 'explain'}
          className="gcp-mode" data-active={mode === 'explain'}
          onClick={() => setMode('explain')}
        >Explain a concept</button>
        <button
          type="button" aria-pressed={mode === 'ask'}
          className="gcp-mode" data-active={mode === 'ask'}
          onClick={() => setMode('ask')}
      ...
      <textarea
        id="gcp-input" ref={inputRef} className="gcp-input" rows={mode === 'explain' ? 2 : 3}
      ...
      <div className="gcp-actions">
        <button type="button" className="gcp-btn gcp-btn--ghost" onClick={useSelection}>
          <i className="fa-solid fa-highlighter" aria-hidden="true" /> From selection
        </button>
        <button type="button" className="gcp-btn gcp-btn--primary" onClick={() => void submit()} disabled={loading}>
      ...
      {context && (
        <p className="gcp-context" title={context}>
          <i className="fa-solid fa-quote-left" aria-hidden="true" /> Grounding in the selected passage.
        </p>
      )}
      {hint && <p className="gcp-hint">{hint}</p>}
      {error && <p className="gcp-error" role="alert">{error}</p>}

      {loading && !result && (
        <div className="gcp-result gcp-result--loading" aria-busy="true">
          <span className="gcp-skel" /><span className="gcp-skel" /><span className="gcp-skel gcp-skel--short" />
        </div>
      )}

      {result && (
        <div className="gcp-result">
          {paragraphs(result.body).map((p, i) => (<p key={i}>{p}</p>))}
          {result.etymology && (
            <p className="gcp-etym"><span className="gcp-etym-label">Etymology.</span> {result.etymology}</p>
          )}
          <p className="gcp-disclaimer">Generated by AI in a scholarly persona. Verify against the source; AI can make mistakes.</p>
        </div>
      )}
    </aside>
  );
