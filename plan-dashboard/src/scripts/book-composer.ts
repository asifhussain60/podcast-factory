...
function boot(): void {
  ...
  function enterEdit(): void {
    ...
    activeEditor.editor.on("update", () => {
      proseAutosave.markDirty();
      if (readingAssistantEnabled)
        requestAnimationFrame(refreshReadingAssistant);
    });
  ...
  const readingAssistantBtn = root.querySelector<HTMLButtonElement>(
    "#cx-reading-assistant",
  );
  let readingAssistantEnabled = false;

  function refreshReadingAssistant(): void {
    clearReadingAssistant(root);
    clearEditorReadingAssistant();
    root.classList.remove("has-reading-assistant-highlights");
    ...
    if (root.classList.contains("is-editing")) {
      ...
      if (editor) {
        root.classList.toggle(
          "has-reading-assistant-highlights",
          applyEditorReadingAssistant(editor),
        );
      }
      return;
    }
    root
      .querySelectorAll<HTMLElement>(".cx-body")
      .forEach(applyReadingAssistant);
  }

  function setReadingAssistant(enabled: boolean): void {
    readingAssistantEnabled = enabled;
    root.classList.toggle("is-reading-assisted", enabled);
    readingAssistantBtn?.setAttribute("aria-pressed", String(enabled));
    refreshReadingAssistant();
  }
  ...
  function setModeVisual(mode: "read" | "edit"): void {
    ...
    if (readingAssistantBtn) {
      readingAssistantBtn.title =
        "Emphasize the representative sentence in each paragraph";
    }
    if (readingAssistantEnabled) requestAnimationFrame(refreshReadingAssistant);
  ...
  // ── Rearticulate, Replace with Arabic, Paste & Fix ──────────────────────
  // Split into book-composer-chapter-tools.ts and book-composer-paste-fix.ts
  // (DR-005 size ratchet) — all three are whole-chapter, selection-
  // independent tools on this tab. Wired here with getters so neither module
  // ever holds a stale snapshot of the live editor/chapter state this file
  // owns.
  const { runRearticulate, runReplaceArabic } = createChapterToolsController({
    getActiveEditor: () => activeEditor,
    getSelectedChapter: () => selectedChapter,
    getChapterTitle: (key) => chapterByKey.get(key)?.title ?? key,
    getSelectionText: () => selectionText(),
    flushActiveSave: () => activeSaveFlush?.() ?? Promise.resolve(true),
    setAiStatus,
    reloadPreservingChapter,
    slug,
  });
  const pasteFixController = createPasteFixController({
    getActiveEditor: () => activeEditor,
    getSelectedChapter: () => selectedChapter,
    getChapterTitle: (key) => chapterByKey.get(key)?.title ?? key,
    setAiStatus,
  ...
  readingAssistantBtn?.addEventListener("click", () => {
    setReadingAssistant(!readingAssistantEnabled);
  });
