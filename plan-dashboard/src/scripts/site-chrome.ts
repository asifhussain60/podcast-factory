...
function initJumpNav(): void {
  ...
  sections.forEach((s) => {
    ...
    const text = (heading?.textContent ?? s.id).replace(/^\d+\s*/, "").trim();
...
function initNavScroll(): void {
  ...
  const active = document.querySelector<HTMLElement>(
    ".topnav-section.is-active",
  );
  if (!strip || !active) return;
  // Scroll only the tab strip, not the page (scrollIntoView walks all ancestors).
  const sRect = strip.getBoundingClientRect();
  const aRect = active.getBoundingClientRect();
  const tabCenterInStrip =
    aRect.left - sRect.left + strip.scrollLeft + aRect.width / 2;
