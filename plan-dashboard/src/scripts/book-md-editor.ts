...
function serializeInline(node: PMNode): string {
  let out = '';
  node.forEach((child) => {
    if (!child.isText || !child.text) { out += serializeInline(child); return; }
    let text = child.text;
...
function docToMarkdown(editor: Editor): string {
  ...
  editor.state.doc.forEach((node) => {
    ...
    if (type === 'heading') {
      lines.push('#'.repeat(Number(node.attrs.level) || 2) + ' ' + serializeInline(node));
...
export function mountChapterEditor(el: HTMLElement, html: string): ChapterEditor {
  const editor = new Editor({
    element: el,
    extensions: [StarterKit],
    content: html,
    editorProps: { attributes: { class: 'cx-prose', 'aria-label': 'Chapter prose editor' } },
  });
  return {
    editor,
