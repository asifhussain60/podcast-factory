...
const CHECKLIST_REL =
  "_system/probe/EP00-pronunciation-probe/listen-checklist.md";
...
export type AmbiguityType =
  "tension" | "undefined_reference" | "scaffolding_gap" | "structural_absence";
...
export async function getAmbiguityItems(
  slug: string,
): Promise<AmbiguityData | null> {
  const ref = await findContent(slug);
  if (!ref) return null;
  const bookDir = ref.dir;
