...
export function readDraft(slug: string, chapter: string, stage: string): string | null {
...
export function writeDraft(slug: string, chapter: string, stage: string, content: string): void {
  const p = draftPath(slug, chapter, stage);
...
export function deleteDraft(slug: string, chapter: string, stage: string): boolean {
  const p = draftPath(slug, chapter, stage);
  if (!existsSync(p)) return false;
  try { rmSync(p); return true; } catch { return false; }
}

/** Cheap existence check (used to seed the editor's on-load draft state). */
export function hasDraft(slug: string, chapter: string, stage: string): boolean {
