...
export async function loadLiveIndex(): Promise<LiveBucketGroup[]> {
  ...
  const withBook = refs.filter((r) =>
    existsSync(join(r.dir, "book", "book.md")),
  );
  ...
  for (const ref of withBook) {
    ...
    if (m) {
      ...
      const entry = bucketMap.get(workSlug) ?? {
        bucket: ref.bucket,
  ...
  for (const bucket of BUCKET_ORDER) {
    ...
    const entries = [...bucketMap.values()].sort((a, b) =>
      a.title.localeCompare(b.title),
    );
    for (const e of entries)
      e.volumes.sort((a, b) => a.slug.localeCompare(b.slug));
