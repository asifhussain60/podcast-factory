...
export const HALTS: HaltDef[] = [
  {
    ...
    artifacts: [
      { label: 'Open questions', relPath: '_system/source/text/_open-questions.md' },
      { label: 'Teaching-ledger coverage', relPath: '_system/source/text/_teaching-ledger-coverage.md' },
      { label: 'Reorganization map', relPath: '_system/source/text/_reorg-map.md' },
      { label: 'Curation log', relPath: '_system/source/text/_curation-log.md' },
      { label: 'Refined edition', relPath: '_system/source/text/refined-english.md' },
    ],
  },
  {
    id: 'source-review',
  ...
  },
  {
    ...
    phase: '0f',
    label: 'Series plan',
    blurb: 'Confirm the chapter list and length tier before per-chapter authoring begins.',
    artifacts: [
  ...
  },
  {
    ...
    artifacts: [
      { label: 'Final review report', relPath: 'audits/final-review-report.md' },
...
export function resolveArtifacts(bookDir: string, def: HaltDef): ResolvedArtifact[] {
  return def.artifacts.map((a) => ({ ...a, exists: safeIsFile(join(bookDir, a.relPath)) }));
}
