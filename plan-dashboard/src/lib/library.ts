...
async function listFiles(
  ...
  for (const name of names.sort()) {
    ...
    out.push({
      name,
      path: full,
      relPath: full.startsWith(rootForRel)
        ? full.slice(rootForRel.length + 1)
        : full,
      bytes: s.size,
...
export async function summarize(ref: ContentRef): Promise<ContentSummary> {
  ...
  const title = simplifyTransliteration(
    (await readTitleFromMeta(metaPath)) ?? slugToTitle(ref.slug),
  );
    ...
    peqVerdict: bookScore
      ? bookScore.avg >= 85
        ...
        : bookScore.avg >= 70
          ...
          : "FAIL"
      : null,
    peqVerdictLabel: bookScore
      ? verdictLabel(
          bookScore.avg >= 85 ? "PASS" : bookScore.avg >= 70 ? "WARN" : "FAIL",
        )
      : null,
  };
}

async function readPublicationStatusFromMeta(
  metaPath: string,
): Promise<"draft" | "published" | "in_progress" | "unknown"> {
...
export async function loadDetail(slug: string): Promise<DetailView | null> {
  ...
  if (await fileExists(metaPath)) {
    try {
      meta = await parseLooseYaml(await readFile(metaPath, "utf-8"));
    } catch {
      /* noop */
    }
  }
  ...
  const chapters = (await listFiles(join(ref.dir, "chapters"), ref.dir)).filter(
    (f) => !f.isDir && (f.ext === "txt" || f.ext === "md"),
  );
  const episodes = (await listFiles(join(ref.dir, "episodes"), ref.dir)).filter(
    (f) => !f.isDir && f.ext === "txt",
  );
  const audio = (await listFiles(join(ref.dir, "m4a"), ref.dir)).filter(
    (f) => !f.isDir && ["m4a", "mp3", "wav"].includes(f.ext),
  );
  ...
  // audits
  const audits = (await listFiles(join(ref.dir, "audits"), ref.dir)).filter(
    (f) => !f.isDir,
  );

  // source files
  const sourceText = join(ref.dir, "_system", "source", "text");
  const sourceFiles = (await listFiles(sourceText, ref.dir)).filter(
    (f) => !f.isDir && ["md", "txt", "json"].includes(f.ext),
  );

  // glossary
  const glossaryPath = join(ref.dir, "_system", "glossary.yml");
  const glossary = {
    exists: await fileExists(glossaryPath),
    path: glossaryPath,
  };
