...
export const POST: APIRoute = async ({ params }) => {
  ...
  if (!existsSync(gatePath)) {
    return new Response(
      JSON.stringify({
        error: `No review-gate.json found for "${slug}". Run Phase 06a first.`,
      }),
      { status: 404, headers: { "Content-Type": "application/json" } },
    );
  }
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        error: `Failed to read review-gate.json: ${e.message}`,
      }),
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        error: `Failed to write review-gate.json: ${e.message}`,
      }),
      { status: 500, headers: { "Content-Type": "application/json" } },
    );
  }
