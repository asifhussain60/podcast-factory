...
export const PATCH: APIRoute = async ({ request }) => {
  ...
  try {
    const atom = updateAtom(id, {
      ...
      content_level: content_level === null ? null : (typeof content_level === 'string' ? content_level : undefined),
...
export const POST: APIRoute = async ({ request }) => {
  let body: unknown;
  try { body = await request.json(); } catch { return apiError('Invalid JSON'); }
  const { type, text_en, tradition, content_level, tags } = body as Record<string, unknown>;
  if (typeof type !== 'string') return apiError('type is required');
  if (typeof text_en !== 'string' || !text_en.trim()) return apiError('text_en must be a non-empty string');
  ...
  try {
    const atom = createAtom({
      ...
      text_en,
      tradition,
      content_level: typeof content_level === 'string' ? content_level : undefined,
