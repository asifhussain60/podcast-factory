...
export const GET: APIRoute = async ({ url }) => {
  ...
  if (TEXT_EXTS.has(ext) && s.size < 5 * 1024 * 1024) {
    ...
    if (ext === "md") {
      ...
      const escaped = text
        ...
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;");
      const body = `<!doctype html><html><head><meta charset="utf-8"><title>${target.split("/").pop()}</title><style>body{font-family:ui-monospace,Menlo,monospace;font-size:18px;line-height:1.65;background:#f7f4ee;color:#1f1d18;padding:24px;max-width:920px;margin:0 auto;white-space:pre-wrap;}</style></head><body>${escaped}</body></html>`;
  ...
  return new Response(buf, {
    status: 200,
    headers: { "content-type": mime, "content-length": String(buf.byteLength) },
  });
};
