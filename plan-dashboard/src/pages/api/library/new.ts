...
export const POST: APIRoute = async ({ request, redirect }) => {
  ...
  if (!isValidSlug(slug)) {
    return new Response(
      `invalid slug: must be kebab-case (lowercase a-z, 0-9, hyphens). Got ${JSON.stringify(slug)}`,
      { status: 400 },
    );
  }
  if (!isValidCategory(category)) {
    return new Response(
      `invalid category: must be one of ${ACTIVE_CATEGORIES.join(", ")}`,
      { status: 400 },
    );
  }
  ...
  if (existing) {
    return new Response(
      `a piece of content with slug ${slug} already exists at ${existing.dir}`,
      { status: 409 },
    );
  }
