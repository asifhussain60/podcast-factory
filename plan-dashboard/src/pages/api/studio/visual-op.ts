...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (action === "generate") {
    ...
    if (!prompt)
      return apiError("A description is required to generate an image");
  ...
  try {
    const result = await new Promise<Record<string, unknown>>(
      (resolve, reject) => {
        ...
        proc.on("close", (code) => {
          ...
          reject(
            new Error(
              `visual-op exited ${code}: ${stderr.slice(0, 300) || stdout.slice(0, 300)}`,
            ),
          );
        });
      },
    ...
    if (result.ok === false)
      return apiServerError(String(result.error ?? "visual-op failed"));
    return apiOk(result);
  } catch (e) {
