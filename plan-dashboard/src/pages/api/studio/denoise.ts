...
function denoiseText(text: string, re: RegExp): { text: string; count: number; samples: string[] } {
  let count = 0;
...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (pattern.trim() === '') return json({ ok: false, error: 'pattern is required' }, 400);
  ...
  if (new RegExp(pattern).test('')) {
    return json({ ok: false, error: 'pattern matches an empty string — make it more specific' }, 400);
  }
