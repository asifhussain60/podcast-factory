...
export const POST: APIRoute = async ({ request }) => {
  ...
  const resolveTarget = (): string | null => {
    if (stage === 'narrator') {
      ...
      if (existsSync(stagesChapterDir)) return join(stagesChapterDir, 'additions-narrator.md');
      return null;
    }
    ...
    if (existsSync(stagesChapterDir)) return join(stagesChapterDir, `${stage}.md`);
    return null;
  };
  ...
  try {
    ...
    if (!targetPath) {
      return apiError(`No writable artifact for stage "${stage}" in chapter "${chapter}"`, 404);
    }
