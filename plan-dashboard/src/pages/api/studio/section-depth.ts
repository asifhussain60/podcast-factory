 ...
 *   Body: { book, chapter, ordinal, slug, depth_level }
 *   Returns { ok, section: SectionDepth }
 *
 * Wave N: persistent section-level depth assignments for the Studio editor.
 * Ordinal is the stable key; slug is the display label (may be updated as
 * the pipeline renames sections). Source defaults to 'human' on PATCH.
 */
...
export const PATCH: APIRoute = async ({ request }) => {
  try {
    ...
    const { book, chapter, ordinal, slug, depth_level } = body as Record<string, unknown>;
    ...
    const section = upsertSectionDepth(book, chapter, ordinal, slug, depth_level, 'human');
