...
export const POST: APIRoute = async ({ request }) => {
  let body: { slug?: string; haltId?: string; decision?: string; notes?: string };
  try {
  ...
  if (decision !== 'approved' && decision !== 'changes') return apiError('decision must be "approved" or "changes"');
  if (decision === 'changes' && !notes.trim()) return apiError('Add a note describing the changes you want');
  if (notes.length > 8000) return apiError('Note is too long (max 8000 characters)');
  ...
  try {
    ...
    reviews[haltId] = { decision: decision as 'approved' | 'changes', notes: notes.trim(), ts: new Date().toISOString() };
