...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (rule.kind === "enum") {
    if (!rule.values.has(value))
      return apiError(`Invalid value "${value}" for ${field}`);
  } else {
    if (!value && !rule.allowEmpty)
      return apiError(`${field.replace(/_/g, " ")} cannot be empty`);
    if (value.length > rule.maxLen)
      return apiError(
        `${field.replace(/_/g, " ")} is too long (max ${rule.maxLen})`,
      );
    if (/[\r\n]/.test(value))
      return apiError(`${field.replace(/_/g, " ")} must be a single line`);
  }
  ...
  if (!existsSync(metaPath))
    return apiError("meta.yml not found for this book", 404);
