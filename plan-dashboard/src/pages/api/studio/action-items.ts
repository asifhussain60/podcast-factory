...
import { getActionItems, upsertActionItem, deleteActionItem } from '../../../lib/db/knowledge';
...
export const GET: APIRoute = ({ request }) => {
  try {
    ...
    if (!book || !chapter) return json({ ok: false, error: 'book and chapter params required' }, 400);
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const { book, chapter, scope, paraOrdinal, termText, anchorText, actionKind, note } = body as Record<string, unknown>;
    if (typeof book !== 'string' || !book) return json({ ok: false, error: 'book required' }, 400);
    if (typeof chapter !== 'string' || !chapter) return json({ ok: false, error: 'chapter required' }, 400);
    if (typeof scope !== 'string' || !ALLOWED_SCOPES.has(scope)) return json({ ok: false, error: 'scope must be paragraph|term' }, 400);
    if (typeof paraOrdinal !== 'number') return json({ ok: false, error: 'paraOrdinal (number) required' }, 400);
    if (typeof actionKind !== 'string' || !ALLOWED_KINDS.has(actionKind)) return json({ ok: false, error: `unknown actionKind: ${String(actionKind)}` }, 400);
...
export const DELETE: APIRoute = async ({ request }) => {
  try {
    ...
    if (typeof book !== 'string' || !book) return json({ ok: false, error: 'book required' }, 400);
    if (typeof chapter !== 'string' || !chapter) return json({ ok: false, error: 'chapter required' }, 400);
    if (typeof id !== 'number') return json({ ok: false, error: 'id (number) required' }, 400);
