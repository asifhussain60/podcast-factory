...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (!SLUG_RE.test(newSlug))
    return apiError(
      "New slug must be lowercase-kebab-case (a-z, 0-9, hyphens)",
    );
  if (oldSlug === newSlug)
    return apiError("New slug is the same as the current slug");
  ...
  try {
    ...
    return apiOk({
      oldSlug,
      newSlug,
      redirect: `/studio/${newSlug}`,
      log: stdout.trim(),
    });
  } catch (e: unknown) {
    const err = e as { stderr?: string; message?: string };
    const msg = (err.stderr || err.message || "Rename failed")
      .toString()
      .trim()
