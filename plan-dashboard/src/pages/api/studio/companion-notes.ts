...
export const GET: APIRoute = ({ request }) => {
  ...
  try {
    ...
    if (!chapter || !CHAPTER_KEY_RE.test(chapter)) return apiError('Missing or invalid chapter');
...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (!slug || !SLUG_RE.test(slug)) return apiError('Missing or invalid slug');
  if (!chapter || !CHAPTER_KEY_RE.test(chapter)) return apiError('Missing or invalid chapter');
  if (!note || typeof note.body !== 'string' || !note.body.trim()) return apiError('Missing note body');
  if (!note.kind || typeof note.kind !== 'string') return apiError('Missing note kind');
  try {
