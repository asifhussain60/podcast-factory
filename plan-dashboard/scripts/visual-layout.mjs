...
export function normalizePlacement(raw) {
  ...
  let align = String(raw.align || "center")
    .trim()
    .toLowerCase();
  if (!ALIGNS.includes(align)) {
    warnings.push(`${vid || "?"}: unknown align -> center`);
    align = "center";
  }

  let flow = String(raw.flow || "standalone")
    .trim()
    .toLowerCase();
  if (!FLOWS.includes(flow)) {
    warnings.push(`${vid || "?"}: unknown flow -> standalone`);
    flow = "standalone";
  }
  ...
  if (align === "center" && flow === "wrap") {
    warnings.push(`${vid}: center forces standalone`);
    flow = "standalone";
  }
  if (flow === "wrap" && widthPct > WRAP_MAX_WIDTH_PCT) {
    warnings.push(
      `${vid}: wrap width ${widthPct}%>${WRAP_MAX_WIDTH_PCT}% -> standalone`,
    );
    flow = "standalone";
  }

  let pageFit = String(raw.page_fit || "avoid")
    .trim()
    .toLowerCase();
  if (!PAGE_FITS.includes(pageFit)) {
    warnings.push(`${vid}: unknown page_fit -> avoid`);
    pageFit = "avoid";
  }
  ...
  if (anchorPara === null || anchorPara === undefined || anchorPara === "")
    anchorPara = null;
  else {
...
export function validateLayout(data) {
  ...
  if (!data || typeof data !== "object")
    return { placements: [], warnings: ["layout is not an object"] };
...
export function loadLayout(bookContentDir) {
  ...
  try {
    ...
    return {
      placements: [],
      warnings: [`visual-layout.json unreadable: ${e.message}`],
    };
  }
}
...
export function figureHtml(placement, assetSrc, embeddedTitle = "") {
  ...
  const dupCaption =
    caption &&
    embeddedTitle &&
    caption.trim().toLowerCase() === embeddedTitle.trim().toLowerCase();
  const cap =
    caption && !dupCaption ? `<figcaption>${esc(caption)}</figcaption>` : "";
...
export function applyLayout(bodyHtml, placements, assetsById) {
  ...
  for (const p of placements) {
    ...
    byKey
      .get(key)
      .push({ p, fig: figureHtml(p, asset.src, asset.embeddedTitle || "") });
  }
  ...
  const emit = (idx) => {
    if (byPara.has(idx)) {
      out.push(...byPara.get(idx));
      byPara.delete(idx);
    }
  };
  const flush = () => {
    if (byPara.size) {
      for (const figs of byPara.values()) out.push(...figs);
      byPara.clear();
    }
  };
