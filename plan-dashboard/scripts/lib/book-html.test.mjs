...
test('default render never emits the editorial fence as visible text', () => {
  ...
  assert.ok(!html.includes('editorial:begin'), 'fence must not appear (raw or escaped)');
  assert.ok(!html.includes('&lt;!--'), 'no escaped comment leaks into prose');
  assert.ok(html.includes('A grounding note for the reader'), 'the note text still renders');
  assert.ok(!html.includes('study-note'), 'no self-study aside in default mode');
});
...
test('self-study render turns editorial fences into a styled Contextual-note aside', () => {
  ...
  assert.ok(html.includes('A grounding note for the reader'), 'body carried through');
  ...
  assert.ok(!html.includes('<strong>Editorial note'), 'inner bold label dropped');
});
...
test('self-study parses bullet lists and study-summary fences; default does not', () => {
  ...
  assert.ok(on.includes('<aside class="study-summary">'), 'summary aside present');
  assert.ok(on.includes('Study summary</p>'), 'summary labeled');

  const off = renderMd(md);
  assert.ok(!off.includes('<ul class="study-list">'), 'default has no list parser');
...
test('default render of ordinary prose is structurally unchanged', () => {
  ...
  assert.ok(html.includes('<section class="chapter-open'), 'chapter open preserved');
  assert.ok(html.includes('<p class="ch-first">A paragraph.</p>'), 'first paragraph preserved');
