#!/usr/bin/env node
// layout-invariants.mjs — the deterministic DOM assertions behind `npm run smoke`.
//
// Split out of site-health-smoke.mjs on 2026-08-16 because that file had grown
// past its size ratchet, and this was the half that could leave: every invariant
// runs inside `page.evaluate`, so the whole section closes over nothing but the
// Playwright page handed to it. The smoke runner keeps what it is actually about
// — booting a server, sweeping routes, judging responses.
//
// These cover defect classes that are VISUAL but MEASURABLE, so a regression
// cannot reship on the strength of a screenshot nobody looked at. Add new ones
// here as the site-health-sentinel agent finds recurring, measurable defects.
...
// measurable defects.
export async function checkLayoutInvariants(page) {
