...
const BASE = (process.env.SITE_HEALTH_BASE_URL || 'http://localhost:4322').replace(/\/$/, '');
const route = opt('route');
const outDir = resolve(opt('out', '.visual-qa'));
const label = opt('label', (route || 'root').replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '') || 'root');
...
const viewports = [];
if (only !== 'mobile') viewports.push({ name: 'desktop', width: desktopW, height: 900 });
if (only !== 'desktop') viewports.push({ name: 'mobile', width: mobileW, height: 844 });
...
async function shoot(browser, vp) {
  ...
  if (evalJs) {
    try { await page.evaluate(evalJs); } catch (e) { errors.push(`eval: ${e.message}`); }
  }
  ...
  console.log(file);
  if (errors.length) console.error(`  [pageerror on ${vp.name}] ${errors.join(' | ')}`);
  await context.close();
}
