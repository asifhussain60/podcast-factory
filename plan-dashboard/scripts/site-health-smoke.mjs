...
function buildRoutes(slug) {
  const tier1 = [
    ...
    '/security', '/plan', '/about', '/annotation-ops',
    '/library', '/studio', '/studio/new', '/pronunciation', '/pre-upload',
    '/wisdom',
...
async function ensureServer() {
  ...
  if (provided) {
    ...
    if (!ok) throw new Error(`SITE_HEALTH_BASE_URL ${provided} is not reachable`);
...
async function probeRoute(context, baseUrl, route) {
  ...
  page.on('pageerror', (err) => {
    findings.push({ kind: 'uncaught-exception', detail: String(err && err.message || err) });
  });
  page.on('requestfailed', (req) => {
    ...
    findings.push({ kind: 'request-failed', detail: `${req.url()} — ${reason}` });
  });
...
async function main() {
  ...
  if (!routes.length) {
    console.error('site-health-smoke: no routes to probe (bad --route filter?)');
    process.exit(1);
  }
  ...
  if (JSON_OUT) {
    console.log(JSON.stringify(
      { baseUrl, slug, total: results.length, clean: clean.length, skipped: skipped.length, failed: failed.length, results },
      null, 2));
  } else {
    console.log(`\nsite-health-smoke — ${baseUrl}  (fixture slug: ${slug || 'none'})`);
    console.log(`  ${clean.length} clean · ${skipped.length} skipped (fixture gap) · ${failed.length} FAILED\n`);
    for (const r of clean) console.log(`  ✓ ${r.route}  [${r.navStatus ?? '—'}]`);
    for (const r of skipped) console.log(`  – ${r.route}  [${r.navStatus}] skipped: fixture mismatch`);
