...
function pageStructureFromText(pagesText, captions) {
  return pagesText.map((raw, i) => {
    const lines = String(raw || '')
      ...
      .map((l) => l.trim())
      .filter(Boolean);
    const openMatch = (lines[0] || '').match(CHAPTER_OPEN_RE) || raw.match(CHAPTER_OPEN_RE);
  ...
  try {
    for (let p = 1; p <= maxPages; p++) {
      ...
      try {
        ...
        execFileSync(pdftotext, ['-f', String(p), '-l', String(p), '-layout', pdfPath, out], {
...
export function diffPageStructures(pdfPages, previewPages) {
  const findings = [];
  const add = (id, severity, detail, page) => findings.push({ id, severity, detail, page });
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (norm(a.firstLine) !== norm(b.firstLine) || norm(a.lastLine) !== norm(b.lastLine)) {
      add(
        'PF-002',
        'P1',
    ...
    if (misplaced.length) {
      add('PF-003', 'P0', `page ${a.page} figure set differs: ${misplaced.join(', ')}`, a.page);
    }
  ...
  findings.sort((x, y) => (x.severity !== 'P0') - (y.severity !== 'P0') || (x.page || 0) - (y.page || 0));
  return findings;
}
...
function parseArgs(argv) {
  const args = { json: false, baseUrl: process.env.SITE_HEALTH_BASE_URL || 'http://localhost:4322' };
...
async function main() {
  ...
  if (!args.slug) {
    console.error('usage: node scripts/preview-fidelity-check.mjs --slug <slug> [--json] [--base-url URL]');
    process.exit(1);
  }
  const bookDir = resolveBookDir(args.slug);
  if (!bookDir) {
    console.error(`No book/ dir found for slug "${args.slug}" under any content bucket.`);
    process.exit(1);
  }
  ...
  if (pdfPages === null) {
    console.error('pdftotext (Poppler) unavailable — deferring parity check (no failure).');
    process.exit(0);
  }

  let previewPages;
  try {
    previewPages = await extractPreviewPageStructure(args.baseUrl, args.slug, captions);
  } catch (err) {
    ...
    if (args.json) console.log(JSON.stringify(payload, null, 2));
      ...
      console.log(`(PDF side ready: ${pdfPages.length} pages extracted for "${args.slug}".)`);
    }
    process.exit(0);
  }
  ...
  if (args.json) {
    ...
    for (const f of findings) console.log(`  ${f.severity} ${f.id} — ${f.detail}`);
  }
