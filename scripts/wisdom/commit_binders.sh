...
for entry in "${BINDERS[@]}"; do
  ...
  DIR="_workspace/wisdom-ksessions/extracted/wisdom/${PREFIX}"
  if [[ ! -d "$DIR" ]]; then
  ...
  git commit -m "feat(wisdom): binder ${BID} — ${NAME} (autonomous run)" \
    -m "Bundles prepared, vision pass deferred to human reviewer (stub sidecars — flagged needs_human_review for image content), finalized, reviewed against curated Ismaili glossary, and sealed. See _workspace/plan/wisdom-rollout-failures.log for the chapters whose images are flagged for human vision review." \
