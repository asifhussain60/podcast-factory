...
def main() -> int:
    ...
    for n in names:
        f = lec_dir / f"{n}.txt"
        if not f.exists():
            raise SystemExit(f"missing lecture {f}")
        parts.append(f"## {n}\n{f.read_text()}")
    raw = "\n\n".join(parts)
    out = gemini(a.model, SYS, raw)
    dst.parent.mkdir(parents=True, exist_ok=True)
    header = (
        f"# Narrator additions — {a.chapter} (Shaykh Abdullah Misra, attributed commentary)\n\n"
    ...
    append_gemini_cost(
        book, phase="wc8/narrator-additions", step=a.chapter, model=a.model, in_chars=len(raw), out_chars=len(out)
    )
