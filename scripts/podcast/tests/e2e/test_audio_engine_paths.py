...
import _dialogue_convergence as dconv
...
class AudioEnginePathE2E(unittest.TestCase):
    ...
    def setUp(self):
        ...
        framing = (self.book / "_system" / "episode-drafts" / EPISODE_ID / "00-framing.md").read_text(encoding="utf-8")
        (self.book / "episodes" / f"{EPISODE_ID}.txt").write_text(framing, encoding="utf-8")
    ...
    def _drive(self, *, approve_audio_render=False, converge_side_effect=None, render_side_effect=None):
        ...
        if converge_side_effect is not None:
            patches.append(
                mock.patch("_dialogue_convergence.converge_dialogue_script", side_effect=converge_side_effect)
            )
        if render_side_effect is not None:
            patches.append(mock.patch("render_dialogue_audio.render_episode", side_effect=render_side_effect))
        with redirect_stdout(out), redirect_stderr(io.StringIO()):
            for p in patches:
                p.start()
                self.addCleanup(p.stop)
            try:
                rc = chapter_driver._drive_per_chapter_and_after(self.book, approve_audio_render=approve_audio_render)
            finally:
                for p in patches:
                    p.stop()
    ...
    def test_elevenlabs_path_halts_at_h1_then_renders_to_finalize(self):
        ...
        def fake_converge(book_dir, slug, **kw):
            ...
            return dconv.DialogueConvergenceResult(
                chapter_slug=slug, episode_id=EPISODE_ID, verdict="SHIP-READY", credit_estimate=200
            )
        ...
        def fake_render(book_dir, ep, **kw):
            ...
            return RenderResult(episode_id=ep, ch_stem=stem, verdict="SHIP-READY", rendered=True, credits_metered=187)

        rc2, stdout2 = self._drive(
            approve_audio_render=True, converge_side_effect=fake_converge, render_side_effect=fake_render
        )
    ...
    def test_elevenlabs_script_failure_halts_before_any_render(self):
        ...
        def failing_converge(book_dir, slug, **kw):
            return dconv.DialogueConvergenceResult(chapter_slug=slug, episode_id=EPISODE_ID, verdict="FAILED")

        render_spy = mock.Mock()
        rc, _ = self._drive(converge_side_effect=failing_converge, render_side_effect=render_spy)
    ...
    def test_episode_override_skips_render_and_emits_notebooklm_table(self):
        ...
        cfg = self.book / "_system" / "series-config.yaml"
        cfg.write_text(
            cfg.read_text(encoding="utf-8") + f"episode_engine_overrides:\n  {EPISODE_ID}: notebooklm\n",
            encoding="utf-8",
        )
