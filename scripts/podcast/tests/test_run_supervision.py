...
class CostCeilingTests(unittest.TestCase):
    def _book_with_ledger(self, tmp: Path, rows: list[dict], config: dict | None = None) -> Path:
        ...
        (book / "_system").mkdir(parents=True)
        (book / "_system" / "orchestrator-state.json").write_text(
            json.dumps({"phase": "0e", "phase_status": "running", "phases": {}, "config": config or {}}),
            encoding="utf-8",
        )
    ...
    def test_under_soft_is_ok(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            book = self._book_with_ledger(
                Path(tmp), [{"ts": "t", "phase": "0e", "step": "x", "model": "g", "cost_usd": 5.0, "engine": "api"}]
            )
    ...
    def test_soft_warns_hard_halts(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            book = self._book_with_ledger(
                Path(tmp), [{"ts": "t", "phase": "0e", "step": "x", "model": "g", "cost_usd": 25.0, "engine": "api"}]
            )
            self.assertEqual(cost_ceiling_check(book)["action"], "warn")
            (book / "_system" / "cost-ledger.jsonl").write_text(
                json.dumps({"ts": "t", "phase": "0e", "step": "x", "model": "g", "cost_usd": 55.0, "engine": "api"})
                + "\n",
                encoding="utf-8",
            )
...
class CircuitBreakerTests(unittest.TestCase):
    ...
    def test_first_chapter_deterministic_failure_halts_after_one(self):
        ...
        rc, calls, state = self._drive(slugs, lambda s: self._failed(s, "extract failed: not under canonical root"))
...
class SuperviseHelperTests(unittest.TestCase):
    ...
    def test_systemic_classification(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (book / "_system").mkdir(parents=True)
            (book / "_system" / "orchestrator-state.json").write_text(
                json.dumps({"phase": "per-chapter", "phase_status": "running", "phases": {}, "config": {}}),
                encoding="utf-8",
            )
            self.assertIsNotNone(s._systemic_reason({"last_error": {"message": "CIRCUIT-BREAKER: x"}}, book))
            self.assertIsNotNone(
                s._systemic_reason({"phase": "per-chapter", "phase_status": "failed", "last_error": None}, book)
            )
            self.assertIsNone(
                s._systemic_reason({"phase": "per-chapter", "phase_status": "running", "last_error": None}, book)
            )
