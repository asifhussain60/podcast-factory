...
import _tighten_helpers  # noqa: E402
...
class BoundaryCheckTests(unittest.TestCase):
    def test_every_bucket_accepted(self):
        for bucket in BUCKETS:
            with self.subTest(bucket=bucket):
                _tighten_helpers.boundary_check(
                    REPO_ROOT / "content" / bucket / "some-book"
                )

    def test_nested_series_volume_accepted(self):
        _tighten_helpers.boundary_check(
            REPO_ROOT / "content" / "Islamic" / "asaas-al-taveel" / "vol-01"
        )
    ...
    def test_legacy_published_accepted(self):
        _tighten_helpers.boundary_check(
            REPO_ROOT / "content" / "published" / "books" / "some-book"
        )
