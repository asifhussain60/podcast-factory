...
import dedup_cross_chapter as dd  # noqa: E402

DOCTRINE = ("The Imam's substance dissolves at each succession and the ranks return that "
...
def _book(tmp: Path, chapters: dict[str, str]) -> Path:
    ...
    for i, (slug, body) in enumerate(chapters.items(), 1):
        (book / "chapters" / f"ch{i:02d}-{slug}.txt").write_text(
            f"# {slug.title()}\n\n{body}\n", encoding="utf-8")
    return book
...
class DedupTests(unittest.TestCase):
    ...
    def test_embedded_repeat_is_flagged_not_cut(self):
        unique_pre = ("The lecture opened on the believer's own place and the discipline of "
                      "being from a place, returning always to one ground and no other ground. ")
        unique_post = (" The teacher then turned to geography and the soil of one's own life, "
        ...
        collapses = dd.plan(book)
        self.assertEqual(len(collapses), 1)
        self.assertTrue(collapses[0].embedded,
                        "a repeat woven into unique prose must be flagged for rewrite")
    ...
    def test_shared_citation_is_not_a_repeat(self):
        cite = ("(Farhad Daftary, The Ismailis: Their History and Doctrines, "
