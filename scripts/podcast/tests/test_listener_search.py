...
FIXTURES = Path(__file__).resolve().parents[3] / "listener" / "test" / "fixtures" / "search-fold.fixtures.json"
...
def test_headings_are_not_indexed_as_passages():
    book = FakeBook(chapters=[FakeChapter("ch-1", 1, "One", "<h2>One</h2><p>The body.</p>")])
...
def test_ordinal_follows_document_order():
    book = FakeBook(chapters=[FakeChapter("ch-1", 1, "One", "<p>first</p><p>second</p><p>third</p>")])
    rows, _ = passages_for(book)
...
def test_sibling_arabic_runs_are_not_glued_together():
    ...
    """
    html = '<p class="ar"><span class="ar-inline">ولي</span><span class="ar-inline">موالات</span></p>'
    (block,) = blocks_of(html)
