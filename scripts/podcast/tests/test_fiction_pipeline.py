...
class TestConsolidationFlag(unittest.TestCase):
    def test_fiction_triggers_consolidation(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            book = _make_book(Path(tmp), content_profile="fiction", episode_planning_mode="chronological")
...
class TestScenicStyleConfigDriven(unittest.TestCase):
    def test_scenic_style_reads_config_override(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            book = _make_book(
                Path(tmp), content_profile="fiction", scenic_style="Ming-dynasty ink-and-color shan-shui landscape."
            )
...
class TestCostEstimatorSingleSourceOfTruth(unittest.TestCase):
    def test_estimator_reuses_ledger_rate_constants(self):
        ...
        self.assertIs(estimate_cost.PRICING_USD_PER_MILLION_TOKENS, _cost_ledger.PRICING_USD_PER_MILLION_TOKENS)

    def test_estimate_separates_notional_from_real(self):
        import estimate_cost

        with tempfile.TemporaryDirectory() as tmp:
            book = _make_book(Path(tmp), content_profile="fiction", source_language="zh-Hant", enable_video=True)
