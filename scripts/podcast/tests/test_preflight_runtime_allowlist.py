...
class RuntimeAllowlistTests(unittest.TestCase):
    ...
    def test_the_translation_edition_compose_outputs_are_allowlisted(self) -> None:
        """isaf-al-talib, 2026-09-18: compose and its gates wrote NEW untracked files, and the
        next --resume/--retry-phase refused with "working tree not clean" three times — one
        stall cost ~50 minutes because nothing reported it. All of these are derived output
        the pipeline regenerates, never hand-authored."""
        for artifact in (
            ...
            self.assertIn(f'"{artifact}"', self.text, f"{artifact} is generated by compose but would strand a resume")
        self.assertIn("book/_chapters/", self.text, "per-chapter compose output would strand a resume")
