...
def test_against_the_live_spiritual_ethos_book_dir():
    """Runs the classifier against the actual book on disk, right now, as a
    live smoke test that the fixture-based unit tests above match reality."""
    book_dir = Path(__file__).resolve().parents[3] / "content" / "Islamic" / "spiritual-ethos"
    if not (book_dir / "book" / "book.md").exists():
        return  # not present in this checkout/CI slice — fixture tests above still cover the logic
    assert needs_model_recompose(book_dir) is False, (
        "spiritual-ethos book.md should not need a model recompose right now — "
        "if this fails, a real model-governing input changed and a full "
        "--retry-phase 0book-compose is genuinely warranted"
    )
