...
from _authoring._chapter_design import (
    _concept_inventory,
...
def _sc(idx, ep_count, topics=None, title="Part"):
    sc = {"sc_index": idx, "source_title": f"{title} {idx}", "episode_count": ep_count}
...
class TopicFloorTest(unittest.TestCase):
    def test_under_split_chapter_violates(self):
        ...
        v = _topic_floor_violations(plan, None, max_concepts=3, enforce=True, consolidate=False)
    ...
    def test_missing_topics_tolerated_when_not_enforced(self):
        plan = [_sc(1, 1)]
        v = _topic_floor_violations(plan, None, max_concepts=3, enforce=False, consolidate=False)
        self.assertEqual(v, [])

    def test_missing_topics_tolerated_in_consolidation_mode(self):
        plan = [_sc(1, 1)]
        v = _topic_floor_violations(plan, None, max_concepts=3, enforce=True, consolidate=True)
        self.assertEqual(v, [])
    ...
    def test_global_inventory_floor_catches_marathon_plan(self):
        ...
        plan = [_sc(i, 1, topics=[f"t{i}a", f"t{i}b", f"t{i}c"]) for i in range(1, 6)]
        v = _topic_floor_violations(plan, inventory, max_concepts=3, enforce=True, consolidate=False)
    ...
    def test_global_inventory_floor_holds_in_consolidation_mode(self):
        ...
        plan = [_sc(1, 1)]
        v = _topic_floor_violations(plan, inventory, max_concepts=3, enforce=True, consolidate=True)
...
class ConceptInventoryTest(unittest.TestCase):
    def test_reads_archive_and_excludes_frames(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(inv[0]["topics"], ["First teaching", "Second teaching"])
