...
import build_episode_txt as bld
...
class EpisodeSourceWiringTests(unittest.TestCase):
    def test_source_is_author_voice_never_literary(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                mock.patch.object(bld, "find_chapter_by_slug", side_effect=fake_find),
        ...
        self.assertTrue(
            any(c.rstrip("/").endswith("chapters") for c in captured),
            f"expected the source lookup in <book>/chapters: {captured}",
        )
        self.assertFalse(
            any("literary" in c for c in captured),
