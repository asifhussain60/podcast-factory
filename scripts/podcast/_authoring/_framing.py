...
def _build_islamic_framing_prompt(
    ...
    return (
        ...
        f"back-to-back.\n"
        f"Then list each term, one per line, as `- Term: form`.\n"
        f"R-PRONUNCIATION-RENDER (2026-08-01): you choose WHICH terms need help; you do NOT "
        f"choose what the hosts say. `build_episode_txt.py` recompiles every value from the "
        f"shared term ladder (per-book override -> loanword -> exonym -> confirmed ledger "
        f"form -> gloss -> plain transliteration) before the framing is written, so whatever "
        f"follows the colon is DISCARDED — write the plain transliteration and move on. "
        f"NEVER write an English translation there: `- arkan: the pillars` is the defect this "
        f"rule exists for, since the block's own instruction calls that slot a phonetic, so "
        f"the hosts were told to pronounce a translation and said 'Archon' across five of six "
        f"episodes of the 2026-08-01 run. The gate rejects it.\n"
...
def author_framing(book_dir: Path, chapter_slug: str, timeout: int = FRAMING_TIMEOUT) -> str:
    ...
    if framing_chars > _CHAR_MAX:
        ...
        compress_prompt = (
            ...
            f"     'Say each term ONCE. Never say the original spelling and the English form\n"
            f"     back-to-back.'\n"
            f"     Then bullet entries only: '- Term: form' (one per term, no prose). KEEP\n"
            f"     EVERY TERM — dropping one is how the three most-mangled words of the\n"
            f"     2026-08-01 run reached the audio with no entry at all. Values are\n"
            f"     recompiled from the shared term ladder after you finish\n"
            f"     (R-PRONUNCIATION-RENDER), so shorten by trimming the NOTES beside an\n"
            f"     entry, never the entry. Never write an English translation as the value.\n"
