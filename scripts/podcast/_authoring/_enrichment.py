...
def author_phase_0e(book_dir: Path,
    ...
    for chapter_file in chapter_files:
        ...
        if _use_technical_enrichment:
            ...
            f"- Apply R-PRESERVE-ARABIC-SOURCE (2026-06-16; supersedes R-NO-ARABIC-NAMES "
            f"/ R-SURAH-ENGLISH-ONLY / R-ALQAAB for Arabic-scholarly content under the "
            f"human phonetic-alignment workflow). PRESERVE every transliterated Arabic / "
            f"Islamic technical term, proper name, book title, surah name, and "
            f"honorific-title (alqaab) EXACTLY as it appears in the source. Do NOT "
            f"translate, anglicize, paraphrase, or replace them with English role-labels "
            f"or audio substitutes: KEEP tawhid, hudud, da'wa, natiq, ta'wil, al-hayuli, "
            f"riyat, nasut, lahut, the Arabic surah names, book titles, and personal "
            f"names in transliteration. The reader edition AND the per-term phonetic-"
            f"alignment review in the Astro site ('keep Arabic / fix spelling / use "
            f"English translation') depend on these terms surviving intact; the English-"
            f"vs-Arabic decision is made LATER by the human, NOT baked in here. Audio "
            f"anglicization for NotebookLM TTS is applied downstream FROM the human's "
            f"phonetic-alignment decisions (the glossary/exonym sanitize layer), never "
            f"in the persisted chapter source. Use plain ASCII transliteration without "
            f"diacritics; established English exonyms for the major prophets remain "
