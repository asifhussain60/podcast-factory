...
def transcribe(wav: Path) -> str:
    ...
    body = (
        part("audio", wav.read_bytes(), filename=wav.name, ctype="audio/mpeg")
    ...
    req = urllib.request.Request(
        ...
        data=body,
        method="POST",
        headers={"Ocp-Apim-Subscription-Key": c.key, "Content-Type": f"multipart/form-data; boundary={boundary}"},
    )
...
def main() -> int:
    ...
    out.write_text(text + "\n")
    append_azure_stt_cost(content_dir(a.slug), phase="wc8/transcribe", step=Path(a.video).name, duration_seconds=dur)
