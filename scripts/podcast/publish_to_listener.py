...
def build_statements(book: Book, *, published_at: str, commit: str | None) -> list[str]:
    ...
    add(
        ...
        "(slug, author, author_short, title_arabic, title_language, study_track, blurb_html, edition_note, cover_key, pdf_key, published_at, source_commit) "
        f"VALUES ({sql_str(book.slug)}, {sql_str(book.author)}, {sql_str(book.author_short)}, {sql_str(book.title_arabic)}, {sql_str(book.title_language)}, {sql_str(book.study_track)}, "
