...
Usage:
    python new_episode.py <BOOK_DIR> <slug> [--title "Episode Title"] [--registry <BOOK_DIR>/_system/registry.md]

What it does:
    1. Reads <BOOK_DIR>/_system/registry.md to find the next monotonic episode
       number for the book (registries are per-book; there is no shared registry).
    ...
    4. Adds a draft row to the per-book registry
...
def main() -> int:
    ...
    registry_path = args.registry if args.registry else (book_dir / "_system" / "registry.md")
