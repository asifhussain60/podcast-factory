    ...
    if not is_autonomous(engine):
        for phase in ("audio-script", "audio-render"):
            if _phase_status(book_dir, phase) not in ("completed", "skipped"):
                update_phase(
                    book_dir, phase=phase, status="skipped", extras={"reason": f"audio_engine={engine.name} (manual)"}
                )
        return "skipped", 0
...
def _drive_audio_script(book_dir: Path, book_slug: str) -> int:
    from _audio_engines import ENGINE_NOTEBOOKLM, engine_for_episode
    ...
    if not slugs:
        update_phase(
            book_dir, phase="audio-script", status="failed", error="no chapter contracts — Phase 0d must run first"
        )
        return 2
    ...
    for slug in slugs:
        ...
        if prior in ("SHIP-READY", "SHIP-WITH-CAUTION") and script_path_for(book_dir, episode_id).exists():
    ...
    update_phase(book_dir, phase="audio-script", status="completed", extras={"verdicts": verdicts})
    phase_git_commit(book_dir, f"podcast({book_slug}): phase audio-script ({len(verdicts)} scripts gated)")
    return 0
...
def _render_plan(book_dir: Path) -> tuple[list[str], int]:
    ...
    from _dialogue_script import parse_dialogue_script, script_char_count, script_path_for
    from render_dialogue_audio import chapter_stem_for_episode, episodes_with_scripts
    ...
    for ep in episodes_with_scripts(book_dir):
        ...
        if (book_dir / "m4a" / f"{stem}.m4a").exists():
            continue
        pending.append(ep)
        turns = parse_dialogue_script(script_path_for(book_dir, ep).read_text(encoding="utf-8"))
...
def _carryover_summary(book_dir: Path, episodes: list[str]) -> list[tuple[str, str, int]]:
    ...
    try:
        from _dialogue_convergence import gate_report_path, read_verdict
    except Exception:
...
def _drive_audio_render(book_dir: Path, book_slug: str, *, approve_render: bool) -> tuple[str, int]:
    pending, estimate = _render_plan(book_dir)
    if not pending:
        update_phase(
            book_dir, phase="audio-render", status="completed", extras={"note": "all episodes already rendered"}
        )
        return "done", 0
    ...
    if not approve_render:
        ...
        update_phase(
            ...
            phase="audio-render",
            status="halted",
            extras={"credit_estimate": estimate, "episodes_pending": pending, "ship_with_caution": caution},
        )
        _info("")
        _info("─" * 72)
        ...
        if caution:
            ...
            _info("        review them in the reader before approving.")
        _info("  Engine             : elevenlabs (eleven_v3, final quality — no Flash)")
    ...
    update_phase(book_dir, phase="audio-render", status="running", extras={"credit_estimate": estimate})
    metered_total = 0
    sanity_flags: dict[str, list[str]] = {}
    for ep in pending:
        try:
            result = render_episode(book_dir, ep, approved=True, style_gate=True, log=_info)
        except Exception as e:
            update_phase(book_dir, phase="audio-render", status="failed", error=f"[{ep}] {e}")
    ...
    update_phase(
        ...
        phase="audio-render",
        status="completed",
        extras={"credits_metered": metered_total, "credit_estimate": estimate, "sanity_flags": sanity_flags},
    )
    phase_git_commit(
        book_dir, f"podcast({book_slug}): phase audio-render ({len(pending)} episodes, {metered_total:,} credits)"
    )
    if sanity_flags:
        _err(
            f"audio-render: sanity flags on {len(sanity_flags)} episode(s) — "
