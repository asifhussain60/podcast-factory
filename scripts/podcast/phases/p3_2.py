...
DETECT_FILES = (REPO_ROOT / "scripts" / "podcast" / "_authoring" / "capstone.py",)
DETECT_MARKERS = ("CrossTierRead", "full_brethren")
DOR = DoR(
    blockers=("Dedicated capstone module with recursion-invariant enforcement is not yet detected.",),
    assumptions=("Tier-2 capstone reads only tier-1 bundle + chapter abstracts.",),
    ambiguities=("Capstone mode defaults must match per-book meta.yml schema.",),
    operator_action=(
