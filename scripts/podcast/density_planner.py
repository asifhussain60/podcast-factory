...
def _glossary_patterns(book_dir: Path) -> list[tuple[str, re.Pattern]]:
    ...
    for e in gl.get("entries") or []:
        ...
        if len(term) < 3 or term.lower() in seen:
            continue
        seen.add(term.lower())
        pats.append((term, re.compile(r"(?<![\w'])" + re.escape(term) + r"(?![\w])", re.IGNORECASE)))
    return pats
...
def compression_risk(
    words: int, concepts: int, glossary_per_1000: float, citations_per_1000: float, profile: DensityProfile
) -> float:
    ...
    raw = 0.40 * concept_load + 0.25 * word_load + 0.20 * vocab_load + 0.15 * cite_load
    ...
    for p in files:
        ...
        out.append(
            ChapterMetrics(
                ...
                session_title=contract.get("session_title") if isinstance(contract.get("session_title"), str) else None,
                words=words,
                ...
                underutilization=round(_clamp01((dd.min_words_soft - words) / dd.min_words_soft), 3),
...
def _group_overlap(members: list[ChapterMetrics]) -> float:
    ...
    if len(members) < 2:
        return 1.0
    return min(
        members[i].term_set and _jaccard(members[i].term_set, members[i + 1].term_set) or 0.0
...
def _best_mode(
    ...
    for mode in (MODE_DEFAULT, MODE_LONGER):
        ...
        if best is None or cand[:3] > best[:3]:
            best = cand
    return best[3], best[4], best[0], compression_risk(words, concepts, g1000, c1000, best[4])


def _score_group(members: list[ChapterMetrics], content_profile: str, book_dir: Path) -> tuple[float, dict] | None:
    ...
    mode, prof, fit, risk = _best_mode(words, concepts, g1000, c1000, content_profile, book_dir)
    incoherence = 0.0
    if k >= 2:
        ...
        per_member = fit - W_RISK * risk - incoherence - PAIRING_COST + prof.combine_bias * thin_relief
    else:
...
def _reason_solo(c: ChapterMetrics, info: dict, prof: DensityProfile, action: str) -> str:
    bits = [f"{c.words:,} words, {c.concepts} concept section(s), ~{c.read_minutes:.0f} min spoken"]
    bits.append(f"band fit {info['fit']:.2f} vs {prof.min_words_soft:,}-{prof.max_words_soft:,} ({info['mode']})")
    drivers = []
    if c.glossary_per_1000 >= VOCAB_REF_PER_1000:
        drivers.append(f"vocabulary-heavy ({c.glossary_terms} glossary terms, {c.glossary_per_1000}/1,000 words)")
    ...
    if action == "flag_thin":
        bits.append("below the minimum solo band - thin; no adjacent same-arc partner improved the score")
    elif action == "flag_dense":
        bits.append("risk exceeds the profile ceiling even standalone - split candidate on the next authoring pass")
    return "; ".join(bits) + "."
...
def _reason_combine(members: list[ChapterMetrics], info: dict) -> str:
    ...
    overlap = _group_overlap(members)
    arc = (
        f"same source Part (session {members[0].session_index})"
        if _same_session(members)
        else "adjacent Parts with strong term continuity"
    )
    return (
        f"{names}: combined {info['words']:,} words / {info['concepts']} "
...
def _framing_impact(members: list[ChapterMetrics], action: str, pacing: bool) -> str:
    if action == "combine":
        eps = ", ".join(f"EP{c.ep_num:02d}" for c in members)
        base = members[0]
        return (
            f"Re-author ONE merged framing covering {eps} (base: "
    ...
    if action == "flag_dense":
        return (
            "Keep the existing framing; add the pacing directive block "
            "(depth over breadth, define terms once, skip minor detail) and "
            "set Length to Long to give the material more airtime."
        )
    if pacing:
        return (
            "Keep the existing framing; add the pacing directive block - the chapter sits near the compression ceiling."
        )
...
def decide(
    ...
    for gi, (members, info, score) in enumerate(partition(chapters, content_profile, book_dir, max_group), start=1):
        ...
        if len(members) == 1:
            ...
            length = "Long" if action == "flag_dense" else MODE_TO_LENGTH[info["mode"]]
...
def build_plan(book_dir: Path, *, mode: str = "audit", max_group: int = DEFAULT_MAX_GROUP) -> dict:
    ...
    return {
        ...
        "profiles_used": {m: asdict(get_profile(content_profile, m, book_dir)) for m in (MODE_DEFAULT, MODE_LONGER)},
        "summary": summary,
...
def pacing_block_for_episode(book_dir: Path, ep_num: int) -> str | None:
    ...
    lines = [
        ...
        "Prioritize the main doctrinal movement; minor details may be skipped entirely rather than rushed.",
        "Pick the strongest example for each teaching and let it breathe; do not race to cover everything.",
    ]
...
def phase_0d_advisory_block(book_dir: Path) -> str:
    ...
    for g in plan.get("groups", []):
        ...
        if g.get("action") == "combine":
            lines.append(
                f"   - consider keeping {' + '.join(g['members'])} as ONE "
                f"episode unit (prior plan combined {eps}: "
                f"{g['combined_words']:,}w, arc-coherent)"
            )
        elif g.get("action") == "flag_dense":
            lines.append(
                f"   - {g['members'][0]} ran over-dense last render "
                f"(risk {g['compression_risk']:.2f}) - prefer a split "
                f"at the strongest thematic seam"
            )
        elif g.get("action") == "flag_thin":
            lines.append(
                f"   - {g['members'][0]} ran thin last render - prefer merging it into an adjacent same-arc unit"
            )
    if not lines:
        return ""
    return (
        "3e. DENSITY PLANNER ADVISORY (non-binding, measured from the prior "
