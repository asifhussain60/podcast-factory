...
class ElevenLabsClient:
    ...
    *transport* (tests): callable(method, url, headers, body, timeout) ->
    ...
    def __init__(self, api_key: str | None = None, *, transport=None, retries: int = 3, retry_sleep_s: float = 10.0):
        self._key = api_key
    ...
    def _request(
        self, method: str, path: str, *, body: bytes | None = None, content_type: str | None = None, timeout: int = 300
    ) -> bytes:
        url = f"{API_BASE}{path}"
        ...
        for attempt in range(self._retries):
            try:
                if self._transport is not None:
                    ...
                    if status >= 400:
                        raise ElevenLabsError(f"{method} {path} -> HTTP {status}: {data[:300]!r}")
                    return data
                ...
                if 400 <= e.code < 500 and e.code != 429:
                    raise ElevenLabsError(f"{method} {path} -> HTTP {e.code}: {detail!r}") from e
                last_err = e
            except Exception as e:
                last_err = e
            if attempt < self._retries - 1:
                print(
                    f"  [elevenlabs] retry {attempt + 1}/{self._retries - 1} on {method} {path}: {last_err}",
                    file=sys.stderr,
                )
                time.sleep(self._retry_sleep_s)
        raise ElevenLabsError(f"{method} {path} failed after {self._retries} attempts: {last_err}")
    ...
    def text_to_dialogue(
        ...
        *,
        ...
        return self._request(
            "POST",
            f"/v1/text-to-dialogue?output_format={output_format}",
            body=json.dumps(payload).encode("utf-8"),
            content_type="application/json",
            timeout=timeout,
        )

    def create_pronunciation_dictionary(self, *, name: str, pls_text: str, description: str = "") -> tuple[str, str]:
        ...
        if not dict_id or not version_id:
            raise ElevenLabsError(f"dictionary upload response missing ids: {data}")
