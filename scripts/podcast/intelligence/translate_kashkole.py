...
# Split out under the DR-005 600-line gate and re-exported so every name stays
# importable from here — the pattern ADR-0001 established. The seam is real: a
# judgement ABOUT a rendering is not the pass that produces one, which is what
# lets `--rescore` re-judge stored English without re-translating it.
from _kashkole_gates import (  # noqa: E402,F401
    SHORT_RATIO,
    check,
    quranic_runs,
)
