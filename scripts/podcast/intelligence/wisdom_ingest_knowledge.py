"""intelligence/wisdom_ingest_knowledge.py — Kashkole corpus ingestion driver (B0).
...
CLI usage:
    python3 scripts/podcast/intelligence/wisdom_ingest_knowledge.py --status
    python3 scripts/podcast/intelligence/wisdom_ingest_knowledge.py --dry-run
    python3 scripts/podcast/intelligence/wisdom_ingest_knowledge.py --chapter musawwadat/x8114-musawwadat
    python3 scripts/podcast/intelligence/wisdom_ingest_knowledge.py --re-ingest --chapter <binder/chapter>
    python3 scripts/podcast/intelligence/wisdom_ingest_knowledge.py

Python API:
    from intelligence.wisdom_ingest_knowledge import (
...
def _parse_verdict(chapter_dir: Path) -> str | None:
    ...
    report = chapter_dir / "_system" / "source" / "text" / "wisdom-challenger-report.md"
...
def seed_lookup_tables(conn) -> None:
    ...
    conn.execute(
        ...
        " VALUES ('wisdom', 'Kashkole Corpus', 'scholarly')"
    )
    conn.commit()
