...
def aggregate(records: list[dict[str, Any]]) -> str:
    ...
    if not groups:
        return "# Podcast Learning — Patterns\n\n_No findings in the ledger yet._\n"
    ...
    lines.append("")
    lines.append(
        f"**Generated by:** `scripts/podcast/learn_aggregate.py` (ledger: `{DEFAULT_LEDGER.relative_to(REPO_ROOT)}`)"
    )
    ...
    for sig, count, n_books, n_eps, ts_first, ts_last, sev, check in rows:
        ...
        lines.append(f"| `{sig}` | {count} | {n_books} | {n_eps} | {sev} | {check} | {ts_first} | {ts_last} | {flag} |")
    lines.append("")
...
def main() -> int:
    ...
    p.add_argument(
        "--ledger", type=Path, default=DEFAULT_LEDGER, help=f"Path to findings.jsonl (default: {DEFAULT_LEDGER})"
    )
    p.add_argument("--output", type=Path, default=OUTPUT_PATH, help=f"Path to patterns.md (default: {OUTPUT_PATH})")
    p.add_argument(
        "--since",
        type=str,
        default=None,
