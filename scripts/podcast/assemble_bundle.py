...
def build_upload_rows(book_dir: Path, mapping: list[dict], filter_episode_ids=None) -> list:
...
def assemble_bundle(slug: str, *, run_score: bool = False, as_json: bool = False) -> int:
    ...
    for entry in mapping:
        ...
        row: dict = {
            ...
            "session_index": contract.get("session_index") if isinstance(contract.get("session_index"), int) else None,
            "session_title": contract.get("session_title") if isinstance(contract.get("session_title"), str) else None,
        }
    ...
    from _notebooklm_table import (
        ...
        length_for_episode,
        load_density_lengths,
        render_upload_table_lines,
        repo_rel_href,
    )

    _overrides = episode_engine_overrides(book_dir)
    _table_rows = (
        [r for r in rows if engine_for_episode(book_dir, r["episode"]) == ENGINE_NOTEBOOKLM] if _overrides else rows
    )
    _suffix = " — NotebookLM-routed only" if _overrides else ""
    print(f"\nNOTEBOOKLM UPLOAD TABLE — {slug} ({len(_table_rows)} episodes){_suffix}")
    print("  Click the CHAPTER cell to open the SOURCE to upload; the EPISODE cell")
    print("  to open the FRAMING to paste into NotebookLM's Customize box.")
    print("  (skip the '# Framing: …' H1 title line when pasting)")
    print()
