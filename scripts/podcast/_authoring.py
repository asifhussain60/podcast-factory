...
def author_framing(book_dir: Path, chapter_slug: str,
    ...
    prompt = (
        ...
        f"- Apply R-NAMEDISCIPLINE STRICTLY (F14 framework guard 2026-05-21): "
        f"NotebookLM's TTS empirically mangles repeated Arabic names into many "
        f"inconsistent garbled pronunciations per episode (observed in KaR Ch07 audio: "
        f"`al-Kirmani` mangled into 8+ variants in a single 30-min episode; `al-Hayuli` "
        f"into 7+ variants). To minimize this, the framing's `## Name discipline` "
        f"section MUST include a per-figure ROTATION SET of 3-4 English aliases for "
        f"every figure that appears in the chapter file at `{chapter_file}`. Pattern:\n"
        f"    1. First mention of any figure uses Arabic + English appositive "
        f"(e.g., `Hamid al-Din Ahmad al-Kirmani, the author of Kitab al-Riyad`).\n"
        f"    2. Subsequent mentions in the host conversation MUST rotate among the "
        f"English aliases — never repeat the Arabic name. The Driver and Color hosts "
        f"each pick a different alias from the rotation set per turn, cycling through "
        f"the set across the episode.\n"
        f"    3. Same discipline for book titles: first mention with English translation "
        f"(e.g., `al-Mahsul (The Harvest)`); thereafter ONLY the English title.\n"
        f"    4. Concept-words (tawhid, hudud, da'wa, ta'wil, ma'lul, nāṭiq): keep "
        f"Arabic on first mention with English gloss; thereafter English gloss "
        f"(e.g., 'monotheism' instead of 'tawhid' by third occurrence).\n"
        f"  Per-figure rotation set source: if `{book_dir}/_system/name-aliases.yml` "
        f"exists, read it; otherwise generate appropriate rotation sets from the "
        f"chapter contract's `audience` + `central_tensions` + the chapter file's "
        f"figure references. Include the full rotation set in the framing's "
        f"`## Name discipline` section so the hosts have the rotation visible.\n"
        f"- Apply R-DRAMATIC-ARC (F15 framework guard 2026-05-21 — empirical: GPT "
        f"review of KaR Ch07 audio found over-explanation, premature resolution of "
        f"central tension, and absence of narrative-arc structure). For debate-format "
        f"chapters (those whose contract sets `episode_format: debate`), the framing's "
        f"`## Three-part focus` MUST follow this 6-beat arc:\n"
        ...
        f"    Beat 2 — Failed answer A: present the first reformer's position; let it "
        f"sound reasonable; do NOT immediately critique.\n"
        f"    Beat 3 — Failed answer B: present the second reformer's position; same "
        f"discipline.\n"
        f"    Beat 4 — Al-Kirmani's pivot: the move that escapes both. This is the "
        f"central settled formula from `contract.anchor_passages`.\n"
        f"    Beat 5 — Non-bodily correction: why the reformers' category mistakes "
        f"were made; what categories actually apply.\n"
        f"    Beat 6 — Human/political stakes + unresolved listener question.\n"
        ...
        f"- Apply R-CHALLENGER-FRICTION (F15 same source). The Color host (or "
        f"scholar_companion or advocate_b) MUST push back genuinely throughout the "
        f"episode, not chorus. Required pushback patterns to use at least 3 times in "
        f"the conversation: 'I don't buy that yet…', 'That sounds like wordplay…', "
        f"'Isn't this just replacing X with Y…', 'How is this different from hiding "
        f"the problem under a different word…'. FORBIDDEN as the Color host's "
        f"FIRST SENTENCE in any turn (these reduce dramatic tension): 'That is a "
        f"remarkably precise…', 'That beautifully maps…', 'That is the perfect "
        f"translation…', 'Exactly', 'Perfect way to put it', 'That captures it "
        f"perfectly'. The Color host's role is FRICTION, not chorus.\n"
        f"- Apply R-ANALOGY-CAP (F15 same source). Enumerate 3-5 GOVERNING ANALOGIES "
        f"upfront in `## Tone constraints` for the entire episode. Hosts ELABORATE on "
        f"these across the conversation but CANNOT introduce new analogies mid-"
        f"episode. GPT review of KaR Ch07 found 14+ competing analogies; the "
        f"cumulative effect was fatigue. Target 5 (soft) governing analogies for "
        f"dense philosophical content; 3 (hard min) for shorter or less-dense "
        f"chapters.\n"
        f"- Apply R-RECURRING-THESIS (F15 same source). The chapter's central settled "
        f"formula (the most quotable line from `contract.anchor_passages`) MUST be "
        f"repeated VERBATIM 3 times across the episode: once at the open (Beat 0 / "
        f"## Opening directive), once at the pivot (Beat 4 / ## Three-part focus), "
        f"once at the close (Beat 7 / ## Landing). This anchors the listener's grasp "
        f"of the central claim. Reference this repetition rule explicitly in each of "
        f"those three sections of the framing.\n"
        f"- Length-tier-specific Opening directive — if Extended tier, include the exact "
        f"phrase: \"target a 45 to 60 minute deep-dive conversation\" (X18 framework "
        f"adjustment 2026-05-21 — bumped from 30-45 to 45-60 min for dense scholarly "
        f"content where the chapter file exceeds ~7,000 words; gives hosts breathing "
        f"room to develop each beat without rushing).\n"
